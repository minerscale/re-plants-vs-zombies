...
bool LawnGetCloseRequest() {
    if (gLawnApp == nullptr) return false;

    return gLawnApp->mCloseRequest;
}

// 0x44E8C0
bool LawnHasUsedCheatKeys() { return gLawnApp && gLawnApp->mPlayerInfo && gLawnApp->mPlayerInfo->mHasUsedCheatKeys; }
...
void LawnApp::PreNewGame(GameMode theGameMode, bool theLookForSavedGame) {
    ...
    if (theLookForSavedGame && TryLoadGame()) return;
...
StoreScreen *LawnApp::ShowStoreScreen() {
    ...
    auto aStoreScreen = new StoreScreen(this);
...
void LawnApp::DoNewOptions(bool theFromGameSelector) {
    ...
    auto aDialog = new NewOptionsDialog(this, theFromGameSelector);
...
AlmanacDialog *LawnApp::DoAlmanacDialog(SeedType theSeedType, ZombieType theZombieType) {
    ...
    auto aDialog = new AlmanacDialog(this);
...
void LawnApp::DoContinueDialog() {
    auto aDialog = new ContinueDialog(this);
...
int LawnApp::LawnMessageBox(
    ...
    auto aDialog = static_cast<LawnDialog *>(
        DoDialog(theDialogId, true, theHeaderName, theLinesName, theButton1Name, theButtonMode)
    );
...
Dialog *LawnApp::DoDialogDelay(
    int theDialogId, bool isModal, const SexyString &theDialogHeader, const SexyString &theDialogLines,
    const SexyString &theDialogFooter, int theButtonMode
) {
    auto aDialog = static_cast<LawnDialog *>(
        SexyAppBase::DoDialog(theDialogId, isModal, theDialogHeader, theDialogLines, theDialogFooter, theButtonMode)
    );
...
void LawnApp::DoUserDialog() {
    ...
    auto aDialog = new UserDialog(this);
    CenterDialog(aDialog, aDialog->mWidth, aDialog->mHeight);
...
void LawnApp::FinishUserDialog(bool isYes) {
    auto aUserDialog = static_cast<UserDialog *>(GetDialog(Dialogs::DIALOG_USERDIALOG));
    if (aUserDialog) {
        if (isYes) {
...
void LawnApp::DoCreateUserDialog() {
    ...
    auto aDialog = new NewUserDialog(this, false);
    CenterDialog(aDialog, aDialog->mWidth, aDialog->mHeight);
...
void LawnApp::FinishCreateUserDialog(bool isYes) {
    auto aNewUserDialog = static_cast<NewUserDialog *>(GetDialog(Dialogs::DIALOG_CREATEUSER));
    if (aNewUserDialog == nullptr) return;

    SexyString aName = aNewUserDialog->GetName();

    if (isYes && aName.empty()) {
        DoDialog(
            Dialogs::DIALOG_CREATEUSERERROR, true, _S("Enter Your Name"),
        ...
        DoDialog(
            Dialogs::DIALOG_CREATEUSERERROR, true, _S("Enter Your Name" /*"[ENTER_YOUR_NAME]"*/),
            _S("Please enter your name to create a new user profile for storing high score data and game progress"
               /*"[ENTER_NEW_USER]"*/),
            _S("OK" /*"[DIALOG_BUTTON_OK]"*/), Dialog::BUTTONS_FOOTER
        );
    } else if (!isYes) {
        KillDialog(Dialogs::DIALOG_CREATEUSER);
    } else {
        ...
        if (aProfile == nullptr) {
            DoDialog(
                Dialogs::DIALOG_CREATEUSERERROR, true, _S("Name Conflict" /*"[NAME_CONFLICT]"*/),
                _S("The name you entered is already being used.  Please enter a unique player name"
...
void LawnApp::DoRenameUserDialog(const SexyString &theName) {
    ...
    auto aDialog = new NewUserDialog(this, true);
    CenterDialog(aDialog, aDialog->mWidth, aDialog->mHeight);
...
void LawnApp::FinishRenameUserDialog(bool isYes) {
    ...
    auto aNewUserDialog = static_cast<NewUserDialog *>(GetDialog(Dialogs::DIALOG_RENAMEUSER));
    if (aUserDialog == nullptr || aNewUserDialog == nullptr) return;

    SexyString anOldName = aUserDialog->GetSelName();
    SexyString aNewName = aNewUserDialog->GetName();
    if (aNewName.empty()) return;

    bool isCurrentUser = mProfileMgr->GetProfile(anOldName) == mPlayerInfo;
    if (!mProfileMgr->RenameProfile(anOldName, aNewName)) {
        DoDialog(
            Dialogs::DIALOG_RENAMEUSERERROR, true, _S("Name Conflict" /*"[NAME_CONFLICT]"*/),
            _S("The name you entered is already being used.  Please enter a unique player name"
               /*"[ENTER_UNIQUE_PLAYER_NAME]"*/),
            _S("OK" /*"[DIALOG_BUTTON_OK]"*/), Dialog::BUTTONS_FOOTER
        );
...
void LawnApp::FinishNameError(int theId) {
    ...
    auto aNewUserDialog = static_cast<NewUserDialog *>(
        GetDialog(theId == Dialogs::DIALOG_CREATEUSERERROR ? Dialogs::DIALOG_CREATEUSER : Dialogs::DIALOG_RENAMEUSER)
    );
    if (aNewUserDialog) {
...
void LawnApp::DoCheatDialog() {
    ...
    auto aDialog = new CheatDialog(this);
    CenterDialog(aDialog, aDialog->mWidth, aDialog->mHeight);
    AddDialog(Dialogs::DIALOG_CHEAT, aDialog);
}

void LawnApp::FinishCheatDialog(bool isYes) {
    auto aCheatDialog = static_cast<CheatDialog *>(GetDialog(Dialogs::DIALOG_CHEAT));
    if (aCheatDialog == nullptr) return;

    if (isYes && !aCheatDialog->ApplyCheat()) return;
...
void LawnApp::FinishTimesUpDialog() { KillDialog(Dialogs::DIALOG_TIMESUP); }
...
void LawnApp::DoConfirmPurchaseDialog(const SexyString &theMessage) {
    auto aComfirmDialog = static_cast<LawnDialog *>(
        DoDialog(Dialogs::DIALOG_STORE_PURCHASE, true, _S("买下这个物品？"), theMessage, _S(""), Dialog::BUTTONS_YES_NO)
    );
...
// 0x451580
Dialog *LawnApp::NewDialog(
    int theDialogId, bool isModal, const SexyString &theDialogHeader, const SexyString &theDialogLines,
    const SexyString &theDialogFooter, int theButtonMode
) {
    auto aDialog = new LawnDialog(
        this, theDialogId, isModal, SexyStringToStringFast(theDialogHeader), SexyStringToStringFast(theDialogLines),
        SexyStringToStringFast(theDialogFooter), theButtonMode
    );
...
bool LawnApp::KillNewOptionsDialog() {
    auto aNewOptionsDialog = static_cast<NewOptionsDialog *>(GetDialog(Dialogs::DIALOG_NEWOPTIONS));
    if (aNewOptionsDialog == nullptr) return false;
...
// 0x4522C0
bool LawnApp::DebugKeyDown(int theKey) { return SexyAppBase::DebugKeyDown(theKey); }
...
void LawnApp::LoadGroup(const char *theGroupName, int theGroupAveMsToLoad) {
    ...
    if (mShutdown || mCloseRequest) return;
...
void LawnApp::LoadingThreadProc() {
    if (!TodLoadResources("LoaderBar")) return;
    ...
    constexpr std::tuple<const char *, int> groups[] = {
        {"LoadingImages", 9 },
        {"LoadingFonts",  54},
        {"LoadingSounds", 54}
    };
    ...
    for (const auto &[group, ms] : groups) {
        mNumLoadingThreadTasks += mResourceManager->GetNumResources(group) * ms;
    }
...
void LawnApp::URLOpenFailed(const std::string &theURL) {
    ...
    DoDialog(
        Dialogs::DIALOG_OPENURL_WAIT, true, _S("Open Browser"), _S("OK"), StringToSexyStringFast(aString),
...
void LawnApp::ConfirmQuit() {
    ...
    auto aDialog = static_cast<LawnDialog *>(
        DoDialog(Dialogs::DIALOG_QUIT, true, aHeader, aBody, _S(""), Dialog::BUTTONS_OK_CANCEL)
    );
...
void LawnApp::ButtonDepress(int theId) {
    ...
    // 按钮编号 theId ∈ [2000, 3000) 时，表示按下 theId - 2000 编号的对话中的“是”按钮
    {
        ...
        case Dialogs::DIALOG_NEWOPTIONS: KillNewOptionsDialog(); return;

        case Dialogs::DIALOG_PREGAME_NAG: DoRegister(); return;

        case Dialogs::DIALOG_LOAD_GAME: return;
        ...
        case Dialogs::DIALOG_INFO: KillDialog(Dialogs::DIALOG_INFO); return;

        case Dialogs::DIALOG_PAUSED: KillDialog(Dialogs::DIALOG_PAUSED); return;
        ...
        case Dialogs::DIALOG_BONUS: KillDialog(Dialogs::DIALOG_BONUS); return;
        ...
        case Dialogs::DIALOG_USERDIALOG: FinishUserDialog(true); return;

        case Dialogs::DIALOG_CREATEUSER: FinishCreateUserDialog(true); return;

        case Dialogs::DIALOG_CONFIRMDELETEUSER: FinishConfirmDeleteUserDialog(true); return;

        case Dialogs::DIALOG_RENAMEUSER: FinishRenameUserDialog(true); return;

        case Dialogs::DIALOG_CREATEUSERERROR:
        case Dialogs::DIALOG_RENAMEUSERERROR: FinishNameError(theId - 2000); return;

        case Dialogs::DIALOG_CHEAT: FinishCheatDialog(true); return;

        case Dialogs::DIALOG_RESTARTCONFIRM: FinishRestartConfirmDialog(); return;

        case Dialogs::DIALOG_TIMESUP: FinishTimesUpDialog(); return;
        ...
        default: KillDialog(theId - 2000); return;
        }
    }
    ...
    // 按钮编号 theId ∈ [3000, 4000) 时，表示按下 theId - 3000 编号的对话中的“否”按钮
    {
        ...
        case Dialogs::DIALOG_LOAD_GAME: KillDialog(Dialogs::DIALOG_LOAD_GAME); return;

        case Dialogs::DIALOG_USERDIALOG: FinishUserDialog(false); return;

        case Dialogs::DIALOG_CREATEUSER: FinishCreateUserDialog(false); return;

        case Dialogs::DIALOG_CONFIRMDELETEUSER: FinishConfirmDeleteUserDialog(false); return;

        case Dialogs::DIALOG_RENAMEUSER: FinishRenameUserDialog(false); return;

        case Dialogs::DIALOG_CHEAT: FinishCheatDialog(false); return;
        ...
        default: KillDialog(theId - 3000); return;
        }
    }
}
...
bool LawnApp::IsWhackAZombieLevel() {
    ...
    if (mGameMode == GameMode::GAMEMODE_CHALLENGE_WHACK_A_ZOMBIE) return true;
...
bool LawnApp::IsLittleTroubleLevel() {
    return (
        mBoard && (mGameMode == GameMode::GAMEMODE_CHALLENGE_LITTLE_TROUBLE ||
...
bool LawnApp::IsScaryPotterLevel() {
    if (mGameMode >= GameMode::GAMEMODE_SCARY_POTTER_1 && mGameMode <= GameMode::GAMEMODE_SCARY_POTTER_9) return true;
...
bool LawnApp::IsStormyNightLevel() {
    ...
    if (mGameMode == GameMode::GAMEMODE_CHALLENGE_STORMY_NIGHT) return true;
...
bool LawnApp::IsBungeeBlitzLevel() {
    ...
    if (mGameMode == GameMode::GAMEMODE_CHALLENGE_BUNGEE_BLITZ) return true;
...
bool LawnApp::IsFinalBossLevel() {
    ...
    if (mGameMode == GameMode::GAMEMODE_CHALLENGE_FINAL_BOSS) return true;
...
bool LawnApp::IsNight() {
    if (IsIceDemo() || mPlayerInfo == nullptr) return false;
...
ChallengeDefinition &LawnApp::GetCurrentChallengeDef() { return GetChallengeDefinition(GetCurrentChallengeIndex()); }
...
SeedType LawnApp::GetAwardSeedForLevel(int theLevel) {
    ...
    return static_cast<SeedType>(aSeedsHasGot);
}
...
ParticleSystemID LawnApp::ParticleGetID(TodParticleSystem *theParticle) {
    return static_cast<ParticleSystemID>(mEffectSystem->mParticleHolder->mParticleSystems.DataArrayGetID(theParticle));
}
...
bool LawnApp::CanShowAlmanac() {
    if (IsIceDemo()) return false;

    if (mPlayerInfo == nullptr) return false;
...
bool LawnApp::CanShowZenGarden() {
    ...
    if (IsTrialStageLocked()) return false;
...
void LawnApp::UpdateCrazyDave() {
    Reanimation *aCrazyDaveReanim = ReanimationTryToGet(mCrazyDaveReanimID);
    if (aCrazyDaveReanim == nullptr) return;
...
void LawnApp::DrawCrazyDave(Graphics *g) {
    ...
    if (mCrazyDaveMessageText.size()) {
        ...
        TodDrawStringWrapped(
            g, aBubbleText, aRect, FONT_BRIANNETOD16, Color::Black,
            DrawStringJustification::DS_ALIGN_CENTER_VERTICAL_MIDDLE
        );
        if (clickToContinue) {
            TodDrawString(
                g, _S("click to continue"), aPosX + 139, aPosY + 140, FONT_PICO129, Color::Black,
...
int LawnApp::GetNumPreloadingTasks() {
    ...
    if (mPlayerInfo) {
        for (SeedType i = SeedType::SEED_PEASHOOTER; i < SeedType::NUM_SEED_TYPES;
             i = static_cast<SeedType>((int)i + 1)) {
        ...
        for (ZombieType i = ZombieType::ZOMBIE_NORMAL; i < ZombieType::NUM_ZOMBIE_TYPES;
             i = static_cast<ZombieType>((int)i + 1)) {
...
bool LawnApp::IsTrialStageLocked() {
    if (mDebugTrialLocked) return true;
...
bool LawnApp::CanDoPinataMode() {
    ...
    return mPlayerInfo->mChallengeRecords
               [static_cast<int>(GameMode::GAMEMODE_TREE_OF_WISDOM) -
                static_cast<int>(GameMode::GAMEMODE_SURVIVAL_NORMAL_STAGE_1)] >= 1000;
}
...
bool LawnApp::CanDoDanceMode() {
    ...
    return mPlayerInfo->mChallengeRecords
               [static_cast<int>(GameMode::GAMEMODE_TREE_OF_WISDOM) -
                static_cast<int>(GameMode::GAMEMODE_SURVIVAL_NORMAL_STAGE_1)] >= 500;
}
...
bool LawnApp::CanDoDaisyMode() {
    ...
    return mPlayerInfo->mChallengeRecords
               [static_cast<int>(GameMode::GAMEMODE_TREE_OF_WISDOM) -
                static_cast<int>(GameMode::GAMEMODE_SURVIVAL_NORMAL_STAGE_1)] >= 100;
}
...
bool LawnApp::CanDoRegisterDialog() { return false; }

void LawnApp::DoNeedRegisterDialog() {}

void LawnApp::FinishModelessDialogs() {}

bool LawnApp::NeedRegister() { return false; }
