...
MemoryImage::MemoryImage(const MemoryImage &theMemoryImage)
    : Image(theMemoryImage), mBitsChangedCount(theMemoryImage.mBitsChangedCount), mD3DData(NULL),
      mD3DFlags(theMemoryImage.mD3DFlags), mHasTrans(theMemoryImage.mHasTrans), mHasAlpha(theMemoryImage.mHasAlpha),
      mIsVolatile(theMemoryImage.mIsVolatile), mPurgeBits(theMemoryImage.mPurgeBits), mWantPal(theMemoryImage.mWantPal),
      mBitsChanged(theMemoryImage.mBitsChanged), mApp(theMemoryImage.mApp) {
...
void MemoryImage::NormalDrawLine(
    ...
    if (true) //(mLockedSurfaceDesc.ddpfPixelFormat.dwRGBBitCount == 32)
    {
        if (theColor.mAlpha == 255) {
            uint32_t aColor = 0xFF000000 | ((((theColor.mRed * aRMask) + aRRoundAdd) >> 8) & aRMask) |
    ...
    if ((mBitsChanged) && (!mForcedMode)) {
        ...
        if (mBits != NULL) {
            ...
            for (int i = 0; i < aSize; i++) {
                ...
                if (anAlpha == 0) mHasTrans = true;
                else if (anAlpha != 255) mHasAlpha = true;
            }
...
void MemoryImage::SetVolatile(bool isVolatile) { mIsVolatile = isVolatile; }

void *MemoryImage::GetNativeAlphaData(NativeDisplay *theDisplay) {
    if (mNativeAlphaData != NULL) return mNativeAlphaData;
...
uchar *MemoryImage::GetRLAlphaData() {
    ...
    if (mRLAlphaData == NULL) {
        ...
        if (mColorTable == NULL) {
            uint32_t *aSrcPtr;
            if (mNativeAlphaData != NULL) aSrcPtr = (uint32_t *)mNativeAlphaData;
            else aSrcPtr = GetBits();
                ...
                for (int aRow = 0; aRow < mHeight; aRow++) {
                    ...
                    while (aRCount < mWidth) {
                        ...
                        if ((aThisAClass != aLastAClass) || (aRCount == mWidth)) {
                            if (aThisAClass == aLastAClass) aRLCount++;
                            ...
                            if ((aRCount == mWidth) && (aThisAClass != aLastAClass)) *aWPtr++ = 1;
...
void MemoryImage::PurgeBits() {
    ...
    if (mApp->Is3DAccelerated()) {
        ...
        if (mD3DData == NULL) return;
    } else {
        if ((mBits == NULL) && (mColorIndices == NULL)) return;
        unreachable();
        /* FIXME
...
void MemoryImage::DeleteSWBuffers() {
    if ((mBits == NULL) && (mColorIndices == NULL)) GetBits();
...
void MemoryImage::Delete3DBuffers() { mApp->Remove3DData(this); }
...
void MemoryImage::ReInit() {
    ...
    if (mWantPal) Palletize();

    if (mPurgeBits) PurgeBits();
}

void MemoryImage::DeleteNativeData() {
    if ((mBits == NULL) && (mColorIndices == NULL)) GetBits(); // We need to keep the bits around
...
void MemoryImage::NormalBlt(Image *theImage, int theX, int theY, const Rect &theSrcRect, const Color &theColor) {
    ...
    if (aSrcMemoryImage != NULL) {
        if (aSrcMemoryImage->mColorTable == NULL) {
            uint32_t *aSrcPixelsRow =
                ((uint32_t *)aSrcMemoryImage->GetBits()) + (theSrcRect.mY * theImage->mWidth) + theSrcRect.mX;
...
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
bool MemoryImage::BltRotatedClipHelper(
    float &theX, float &theY, const Rect &theSrcRect, const Rect &theClipRect, double theRot, FRect &theDestRect,
    ...
    for (int i = 0; i < 4; i++) {
        ...
        if (px[i] < aMinX) aMinX = px[i];
        if (px[i] > aMaxX) aMaxX = px[i];
        if (py[i] < aMinY) aMinY = py[i];
        if (py[i] > aMaxY) aMaxY = py[i];
    }
...
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
bool MemoryImage::StretchBltClipHelper(
    const Rect &theSrcRect, const Rect &theClipRect, const Rect &theDestRect, FRect &theSrcRectOut, Rect &theDestRectOut
) {
    theDestRectOut =
    ...
    theSrcRectOut = FRect(
        theSrcRect.mX + (theDestRectOut.mX - theDestRect.mX) * aXFactor,
...
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
bool MemoryImage::StretchBltMirrorClipHelper(
    const Rect &theSrcRect, const Rect &theClipRect, const Rect &theDestRect, FRect &theSrcRectOut, Rect &theDestRectOut
) {
    theDestRectOut =
        Rect(theDestRect.mX, theDestRect.mY, theDestRect.mWidth, theDestRect.mHeight).Intersection(theClipRect);
    ...
    theSrcRectOut = FRect(
        theSrcRect.mX + (aRightClip)*aXFactor, theSrcRect.mY + (theDestRectOut.mY - theDestRect.mY) * aYFactor,
        theSrcRect.mWidth + (theDestRectOut.mWidth - theDestRect.mWidth) * aXFactor,
        theSrcRect.mHeight + (theDestRectOut.mHeight - theDestRect.mHeight) * aYFactor
    );
...
void MemoryImage::StretchBlt(
    ...
    if (!StretchBltClipHelper(theSrcRect, theClipRect, theDestRect, aSrcRect, aDestRect)) return;

    if (fastStretch) FastStretchBlt(theImage, aDestRect, aSrcRect, theColor, theDrawMode);
    else SlowStretchBlt(theImage, aDestRect, aSrcRect, theColor, theDrawMode);
}

void MemoryImage::BltMatrixHelper(
    Image *theImage, float x, float y, const SexyMatrix3 &theMatrix, const Rect &theClipRect, const Color &theColor,
    int theDrawMode, const Rect &theSrcRect, void *theSurface, int theBytePitch, int thePixelFormat, bool blend
) {
...
void MemoryImage::BltMatrix(
    ...
    int aPitch = mWidth * 4;
    int aFormat = 0x8888;
    if (mForcedMode && !mHasAlpha && !mHasTrans) aFormat = 0x888;
...
void MemoryImage::BltTrianglesTexHelper(
    Image *theTexture, const TriVertex theVertices[][3], int theNumTriangles, const Rect &theClipRect,
    const Color &theColor, int theDrawMode, void *theSurface, int theBytePitch, int thePixelFormat, float tx, float ty,
    bool blend
) {
    ...
    for (int i = 0; i < theNumTriangles; i++) {
        ...
        for (int j = 0; j < 3; j++) {
            ...
            if (aVerts[j].mDiffuse != 0) vertexColor = true;
        }

        SWHelper::SWDrawShape(
            aVerts, 3, anImage, theColor, theDrawMode, theClipRect, theSurface, theBytePitch, thePixelFormat, blend,
            vertexColor
        );
    }
}

void MemoryImage::FillScanLinesWithCoverage(
...
void MemoryImage::BltTrianglesTex(
    Image *theTexture, const TriVertex theVertices[][3], int theNumTriangles, const Rect &theClipRect,
    ...
    BltTrianglesTexHelper(
        theTexture, theVertices, theNumTriangles, theClipRect, theColor, theDrawMode, aSurface, aPitch, aFormat, tx, ty,
        blend
    );
    BitsChanged();
}
...
bool MemoryImage::Palletize() {
    ...
    if (mColorTable != NULL) return true;

    GetBits();

    if (mBits == NULL) return false;
