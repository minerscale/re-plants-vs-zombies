...
void ScrollbuttonWidget::Draw(Graphics *g) {
    ...
    if (!mDisabled) g->SetColor(Color::Black);
    else g->SetColor(Color(132, 132, 132));
    ...
    if (mHorizontal || (mType == 3 || mType == 4)) {
        for (int i = 0; i < 4; i++) {
            if (mId == 0 || mType == 3)
                ...
                g->FillRect(
                    (3 - i) + (mWidth - 4) / 2 + anArrowOffset, mHeight / 2 - i - 1 + anArrowOffset, 1, 1 + i * 2
                );
        }
    } else {
        for (int i = 0; i < 4; i++) {
            if (mId == 0 || mType == 1)
                ...
                g->FillRect(
                    mWidth / 2 - i - 1 + anArrowOffset, (3 - i) + (mHeight - 4) / 2 + anArrowOffset, 1 + i * 2, 1
                );
        }
    }
}
