...
static void png_pak_read_data(png_structp png_ptr, png_bytep data, png_size_t length) {
     ...
     */
    check = (png_size_t)p_fread(data, (png_size_t)1, length, (PFILE *)png_get_io_ptr(png_ptr));
    ...
    if (theDoImageSanding && anImage != NULL) {
        {
            ...
            size_t imageSize = anImage->mHeight * anImage->mWidth * sizeof(uint32_t);

            uint32_t* aTempBuffer = (uint32_t *)alloca(imageSize);
            memcpy(aTempBuffer, aBitsPtr, imageSize);

            for (int y = 0; y < anImage->mHeight; y++)
            {
                for (int x = 0; x < anImage->mWidth; x++)
                {
                    if ((*aTempBuffer & 0xFF000000UL) == 0)  // 如果像素的不透明度为 0
                    {
                        *aBitsPtr = AverageNearByPixels(anImage.get(), aTempBuffer, x, y);  //
    计算该点周围非透明像素的平均颜色
                    }
