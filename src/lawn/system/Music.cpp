...
void Music::SetupMusicFileForTune(MusicFile theMusicFile, MusicTune theMusicTune) {
    ...
    const HMUSIC aHMusic = GetBassMusicHandle(theMusicFile);
...
void Music::LoadSong(MusicFile theMusicFile, const std::string &theFileName) {
    ...
    if (!TodLoadMusic(theMusicFile, theFileName)) {
        ...
        BASS_ChannelSetAttribute(
            GetBassMusicHandle(theMusicFile), BASS_ATTRIB_MUSIC_PSCALER, 4
        ); // 设置音乐定位精确度属性
...
void Music::StopAllMusic() {
    if (mMusicInterface != nullptr) {
        if (mCurMusicFileMain != MusicFile::MUSIC_FILE_NONE) mMusicInterface->StopMusic(mCurMusicFileMain);
        if (mCurMusicFileDrums != MusicFile::MUSIC_FILE_NONE) mMusicInterface->StopMusic(mCurMusicFileDrums);
        if (mCurMusicFileHihats != MusicFile::MUSIC_FILE_NONE) mMusicInterface->StopMusic(mCurMusicFileHihats);
    }
...
void Music::PlayMusic(MusicTune theMusicTune, int theOffset, int theDrumsOffset) {
    if (mMusicDisabled) return;

    mCurMusicTune = theMusicTune;
    mCurMusicFileMain = MusicFile::MUSIC_FILE_NONE;
    mCurMusicFileDrums = MusicFile::MUSIC_FILE_NONE;
    mCurMusicFileHihats = MusicFile::MUSIC_FILE_NONE;
    const bool aRestartingSong = theOffset != -1;
    ...
    if (aRestartingSong) {
        if (mCurMusicFileMain != MusicFile::MUSIC_FILE_NONE) {
            const HMUSIC aHMusic = GetBassMusicHandle(mCurMusicFileMain);
        ...
        if (mCurMusicFileDrums != -1) {
            const HMUSIC aHMusic = GetBassMusicHandle(mCurMusicFileDrums);
            BASS_ChannelSetAttribute(aHMusic, BASS_ATTRIB_MUSIC_BPM, mBaseBPM);
            BASS_ChannelSetAttribute(aHMusic, BASS_ATTRIB_MUSIC_SPEED, mBaseModSpeed);
        }
        if (mCurMusicFileHihats != -1) {
            const HMUSIC aHMusic = GetBassMusicHandle(mCurMusicFileHihats);
            BASS_ChannelSetAttribute(aHMusic, BASS_ATTRIB_MUSIC_BPM, mBaseBPM);
            BASS_ChannelSetAttribute(aHMusic, BASS_ATTRIB_MUSIC_SPEED, mBaseModSpeed);
        }
    } else {
...
void Music::MusicResyncChannel(MusicFile theMusicFileToMatch, MusicFile theMusicFileToSync) {
    ...
    {
        const HMUSIC aHMusic = GetBassMusicHandle(theMusicFileToSync);

        int aBPM = mBaseBPM;
        if (aDiff > 2) aBPM -= 2;
        else if (aDiff > 0) aBPM -= 1;
        else if (aDiff < -2) aBPM += 2;
        else if (aDiff < 0) aBPM -= 1;
...
void Music::UpdateMusicBurst() {
    ...
    if (mCurMusicTune == MusicTune::MUSIC_TUNE_DAY_GRASSWALK ||
        ...
        aBurstScheme = 1;
    else if (mCurMusicTune == MusicTune::MUSIC_TUNE_NIGHT_MOONGRAINS) aBurstScheme = 2;
    else return;

    const int aPackedOrderMain = GetMusicOrder(mCurMusicFileMain);
    if (mBurstStateCounter > 0) mBurstStateCounter--;
    if (mDrumsStateCounter > 0) mDrumsStateCounter--;
    ...
    case MusicBurstState::MUSIC_BURST_OFF:
        if (mApp->mBoard->CountZombiesOnScreen() >= 10 || mBurstOverride == 1) StartBurst();
        break;
    case MusicBurstState::MUSIC_BURST_STARTING:
        if (aBurstScheme == 1) {
            ...
            if (mMusicDrumsState == MusicDrumsState::MUSIC_DRUMS_OFF) {
                ...
                mQueuedDrumTrackPackedOrder = aPackedOrderMain;
                mBurstStateCounter = 400;
            } else if (mMusicDrumsState == MusicDrumsState::MUSIC_DRUMS_ON_QUEUED) mBurstStateCounter = 400;
            else {
                aMainTrackVolume =
    ...
    case MusicBurstState::MUSIC_BURST_ON:
        ...
        if (aBurstScheme == 2) aMainTrackVolume = 0.0f;
        if (mBurstStateCounter == 0 &&
            ((mApp->mBoard->CountZombiesOnScreen() < 4 && mBurstOverride == -1) || mBurstOverride == 2)) {
            if (aBurstScheme == 1) {
    ...
    case MusicBurstState::MUSIC_BURST_FINISHING:
        ...
        else aMainTrackVolume = TodAnimateCurveFloat(400, 0, mBurstStateCounter, 0.0f, 1.0f, TodCurves::CURVE_LINEAR);
    ...
    if (aBurstScheme == 1) {
        ...
        if (HIWORD(aPackedOrderMain) > 252) aOrderMain++;
        if (HIWORD(mQueuedDrumTrackPackedOrder) > 252) aOrderDrum++;
    }
    ...
    case MusicDrumsState::MUSIC_DRUMS_ON_QUEUED:
        if (aOrderMain != aOrderDrum) {
            ...
            if (aBurstScheme == 2) aDrumsJumpOrder = (aOrderMain % 2 == 0) ? 76 : 77;
        }
        break;
    case MusicDrumsState::MUSIC_DRUMS_ON: aDrumsVolume = 1.0f; break;
    ...
    case MusicDrumsState::MUSIC_DRUMS_FADING:
        ...
        else aDrumsVolume = TodAnimateCurveFloat(50, 0, mDrumsStateCounter, 1.0f, 0.0f, TodCurves::CURVE_LINEAR);
        if (mDrumsStateCounter == 0) mMusicDrumsState = MusicDrumsState::MUSIC_DRUMS_OFF;
        break;
    case MusicDrumsState::MUSIC_DRUMS_OFF: break;
    }
...
void Music::MusicUpdate() {
    if (mFadeOutCounter > 0) {
        mFadeOutCounter--;
        if (mFadeOutCounter == 0) StopAllMusic();
        else {
            const float aFadeLevel =
    ...
    if (mApp->mBoard == nullptr || !mApp->mBoard->mBoardData.mPaused) {
        UpdateMusicBurst();
        MusicResync();
    }
}
...
void Music::StartGameMusic() {
    ...
    else if (mApp->mBoard->StageHasFog()) MakeSureMusicIsPlaying(MusicTune::MUSIC_TUNE_FOG_RIGORMORMIST);
    else if (mApp->mBoard->StageIsNight()) MakeSureMusicIsPlaying(MusicTune::MUSIC_TUNE_NIGHT_MOONGRAINS);
    else if (mApp->mBoard->StageHas6Rows()) MakeSureMusicIsPlaying(MusicTune::MUSIC_TUNE_POOL_WATERYGRAVES);
    else if (mApp->mBoard->StageHasRoof()) MakeSureMusicIsPlaying(MusicTune::MUSIC_TUNE_ROOF_GRAZETHEROOF);
    else MakeSureMusicIsPlaying(MusicTune::MUSIC_TUNE_DAY_GRASSWALK);
}
