...
SeedChooserScreen::SeedChooserScreen() {
    ...
    for (SeedType aSeedType = SEED_PEASHOOTER; aSeedType < NUM_SEEDS_IN_CHOOSER;
        ...
        aChosenSeed.mSeedState =
            aSeedType == SEED_IMITATER ? ChosenSeedState::SEED_PACKET_HIDDEN : ChosenSeedState::SEED_IN_CHOOSER;
    ...
    if (mApp->mGameMode == GAMEMODE_CHALLENGE_SEEING_STARS) {
        ...
        aStarFruit.mSeedState = ChosenSeedState::SEED_IN_BANK;
        aStarFruit.mSeedIndexInBank = 0;
        mSeedsInBank++;
    }
    if (mApp->IsAdventureMode() && !mApp->IsFirstTimeAdventureMode()) CrazyDavePickSeeds();
    UpdateImitaterButton();
}

template <typename T>
T SeedChooserScreen::PickFromWeightedArrayUsingSpecialRandSeed(
    TodWeightedArray<T> theArray[], const int theCount, MTRand &theLevelRNG
) {
    int aTotalWeight = 0;
    ...
    const int aRndResult = theLevelRNG.Next(static_cast<unsigned long>(aTotalWeight));
    int aWeight = 0;
...
void SeedChooserScreen::CrazyDavePickSeeds() {
    ...
    for (SeedType aSeedType = SEED_PEASHOOTER; aSeedType < NUM_SEEDS_IN_CHOOSER;
        ...
        // uint32_t aRecFlags = SeedNotRecommendedToPick(aSeedType);
    ...
    for (int i = 0; i < 3; i++) {
        const SeedType aPickedSeed =
            PickFromWeightedArrayUsingSpecialRandSeed(aSeedArray, NUM_SEEDS_IN_CHOOSER, aLevelRNG);
        aSeedArray[aPickedSeed].mWeight = 0;
        ChosenSeed &aChosenSeed = mChosenSeeds[aPickedSeed];

        const int aPosX = mBoard->GetSeedPacketPositionX(i);
        ...
        aChosenSeed.mEndY = 8;
        aChosenSeed.mSeedState = ChosenSeedState::SEED_IN_BANK;
...
// 0x484220
bool SeedChooserScreen::Has7Rows() const {
...
// 0x484400
void SeedChooserScreen::GetSeedPositionInChooser(const int theIndex, int &x, int &y) const {
    if (theIndex == SEED_IMITATER) {
        x = mImitaterButton->mX;
        y = mImitaterButton->mY;
    } else {
        const int aRow = theIndex / 8;
        const int aCol = theIndex % 8;
...
// 0x484480
void SeedChooserScreen::GetSeedPositionInBank(const int theIndex, int &x, int &y) {
...
// 0x4845E0
unsigned int SeedChooserScreen::SeedNotRecommendedToPick(const SeedType theSeedType) {
    uint32_t aRecFlags = mBoard->SeedNotRecommendedForLevel(theSeedType);
...
// 0x484620
bool SeedChooserScreen::SeedNotAllowedToPick(const SeedType theSeedType) {
...
// 0x484640
bool SeedChooserScreen::SeedNotAllowedDuringTrial(const SeedType theSeedType) {
...
void SeedChooserScreen::Draw(Graphics *g) {
    ...
    if (!mBoard->ChooseSeedsOnCurrentLevel() || (mBoard->mCutScene && mBoard->mCutScene->IsBeforePreloading())) return;
    ...
    TodDrawString(
        g, _S("[CHOOSE_YOUR_PLANTS]"), 229, 110, Sexy::FONT_DWARVENTODCRAFT18YELLOW, Color::White,
        DrawStringJustification::DS_ALIGN_CENTER
    );

    const int aNumSeeds = Has7Rows() ? 48 : 40;
    for (SeedType aSeedShadow = SEED_PEASHOOTER; aSeedShadow < aNumSeeds;
         aSeedShadow = static_cast<SeedType>(aSeedShadow + 1)) {
        int x, y;
        ...
        if (mApp->SeedTypeAvailable(aSeedShadow)) {
            const ChosenSeed &aChosenSeed = mChosenSeeds[aSeedShadow];
    ...
    const int aNumSeedsInBank = mBoard->mSeedBank->mNumPackets;
    ...
    for (SeedType aSeedType = SEED_PEASHOOTER; aSeedType < NUM_SEEDS_IN_CHOOSER;
         aSeedType = static_cast<SeedType>(aSeedType + 1)) {
        const ChosenSeed &aChosenSeed = mChosenSeeds[aSeedType];
        const ChosenSeedState aSeedState = aChosenSeed.mSeedState;
        if (mApp->SeedTypeAvailable(aSeedType) && aSeedState != ChosenSeedState::SEED_FLYING_TO_BANK &&
            aSeedState != ChosenSeedState::SEED_FLYING_TO_CHOOSER &&
            aSeedState != ChosenSeedState::SEED_PACKET_HIDDEN &&
            (aSeedState == ChosenSeedState::SEED_IN_CHOOSER || mBoard->mCutScene->mSeedChoosing)) {
            bool aGrayed = false;
            if (((SeedNotRecommendedToPick(aSeedType) || SeedNotAllowedToPick(aSeedType)) &&
                 aSeedState == ChosenSeedState::SEED_IN_CHOOSER) ||
                SeedNotAllowedDuringTrial(aSeedType))
                aGrayed = true;

            int aPosX = aChosenSeed.mX;
            int aPosY = aChosenSeed.mY;
            if (aSeedState == ChosenSeedState::SEED_IN_BANK) {
                aPosX -= mX;
                aPosY -= mY;
            }
            DrawSeedPacket(
    ...
    for (SeedType aSeedType = SEED_PEASHOOTER; aSeedType < NUM_SEEDS_IN_CHOOSER;
        ...
        if (mApp->SeedTypeAvailable(aSeedType) && (aSeedState == ChosenSeedState::SEED_FLYING_TO_BANK ||
                                                   aSeedState == ChosenSeedState::SEED_FLYING_TO_CHOOSER)) {
            DrawSeedPacket(
                g, aChosenSeed.mX, aChosenSeed.mY, aChosenSeed.mSeedType, aChosenSeed.mImitaterType, 0, 255, true, false
            );
        }
    }
...
void SeedChooserScreen::UpdateViewLawn() {
    if (mChooseState != SeedChooserState::CHOOSE_VIEW_LAWN) return;
    mViewLawnTime++;
    if (mViewLawnTime == 100)
        mBoard->DisplayAdviceAgain(
            "[CLICK_TO_CONTINUE]", MESSAGE_STYLE_HINT_STAY, AdviceType::ADVICE_CLICK_TO_CONTINUE
        );
    else if (mViewLawnTime == 251) mViewLawnTime = 250;

    const int aBoardX = BOARD_IMAGE_WIDTH_OFFSET - mApp->mWidth;
    const int aSeedChooserY = SEED_CHOOSER_OFFSET_Y - Sexy::IMAGE_SEEDCHOOSER_BACKGROUND->mHeight;
    if (mViewLawnTime <= 100) {
...
void SeedChooserScreen::UpdateCursor() {
    ...
    if (aMouseSeedType != SEED_NONE) {
        const ChosenSeed &aMouseChosenSeed = mChosenSeeds[aMouseSeedType];
        if (aMouseChosenSeed.mSeedState == ChosenSeedState::SEED_IN_BANK && aMouseChosenSeed.mCrazyDavePicked)
...
void SeedChooserScreen::Update() {
    ...
    for (SeedType aSeedType = SEED_PEASHOOTER; aSeedType < NUM_SEEDS_IN_CHOOSER;
        ...
        if (mApp->SeedTypeAvailable(aSeedType)) {
            ChosenSeed &aChosenSeed = mChosenSeeds[aSeedType];
            if (aChosenSeed.mSeedState == ChosenSeedState::SEED_FLYING_TO_BANK ||
                aChosenSeed.mSeedState == ChosenSeedState::SEED_FLYING_TO_CHOOSER) {
                const int aTimeStart = aChosenSeed.mTimeStartMotion;
                const int aTimeEnd = aChosenSeed.mTimeEndMotion;
...
bool SeedChooserScreen::DisplayRepickWarningDialog(const SexyChar *theMessage) {
    return mApp->LawnMessageBox(
               Dialogs::DIALOG_CHOOSER_WARNING, _S("[DIALOG_WARNING]"), theMessage, _S("[DIALOG_BUTTON_YES]"),
               _S("[REPICK_BUTTON]"), Dialog::BUTTONS_YES_NO
           ) == Dialog::ID_YES;
}
...
bool SeedChooserScreen::FlyersAreComming() {
    for (int aWave = 0; aWave < mBoard->mBoardData.mNumWaves; aWave++) {
        for (int anIndex = 0; anIndex < MAX_ZOMBIES_IN_WAVE; anIndex++) {
            const ZombieType aZombieType = mBoard->mBoardData.mZombiesInWave[aWave][anIndex];
...
// 0x485420
bool SeedChooserScreen::CheckSeedUpgrade(const SeedType theSeedTypeTo, const SeedType theSeedTypeFrom) {
    if (mApp->IsSurvivalMode() || !PickedPlantType(theSeedTypeTo) || PickedPlantType(theSeedTypeFrom)) return true;
...
void SeedChooserScreen::PickRandomSeeds() {
    for (int anIndex = mSeedsInBank; anIndex < mBoard->mSeedBank->mNumPackets; anIndex++) {
        ...
        do {
            aSeedType = static_cast<SeedType>(Rand(mApp->GetSeedsAvailable()));
                 ...
                 mChosenSeeds[aSeedType].mSeedState != ChosenSeedState::SEED_IN_CHOOSER);
        ChosenSeed &aChosenSeed = mChosenSeeds[aSeedType];
        aChosenSeed.mTimeStartMotion = 0;
        aChosenSeed.mTimeEndMotion = 0;
        aChosenSeed.mStartX = aChosenSeed.mX;
    ...
    for (SeedType aSeedFlying = SEED_PEASHOOTER; aSeedFlying < NUM_SEEDS_IN_CHOOSER;
         aSeedFlying = static_cast<SeedType>(aSeedFlying + 1))
        LandFlyingSeed(mChosenSeeds[aSeedFlying]);
    CloseSeedChooser();
}

// 0x485BB0
void SeedChooserScreen::ButtonDepress(const int theId) {
    if (mSeedsInFlight > 0 || mChooseState == SeedChooserState::CHOOSE_VIEW_LAWN || !mMouseVisible) return;
    ...
    if (theId == SeedChooserScreen::SeedChooserScreen_ViewLawn) {
        ...
        if (theId == SeedChooserScreen::SeedChooserScreen_Start) OnStartButton();
        else if (theId == SeedChooserScreen::SeedChooserScreen_Random) PickRandomSeeds();
    }
}

// 0x485D80
SeedType SeedChooserScreen::SeedHitTest(const int x, const int y) {
    if (mMouseVisible) {
        for (SeedType aSeedType = SEED_PEASHOOTER; aSeedType < NUM_SEEDS_IN_CHOOSER;
             aSeedType = static_cast<SeedType>(aSeedType + 1)) {
            const ChosenSeed &aChosenSeed = mChosenSeeds[aSeedType];
            if (!mApp->SeedTypeAvailable(aSeedType) || aChosenSeed.mSeedState == ChosenSeedState::SEED_PACKET_HIDDEN)
                continue;
...
// 0x485E20
SeedType SeedChooserScreen::FindSeedInBank(const int theIndexInBank) {
    for (SeedType aSeedType = SEED_PEASHOOTER; aSeedType < NUM_SEEDS_IN_CHOOSER;
         aSeedType = static_cast<SeedType>(aSeedType + 1)) {
        if (mApp->SeedTypeAvailable(aSeedType)) {
            const ChosenSeed &aChosenSeed = mChosenSeeds[aSeedType];
...
void SeedChooserScreen::EnableStartButton(const bool theEnabled) {
...
void SeedChooserScreen::ClickedSeedInBank(ChosenSeed &theChosenSeed) {
    for (int anIndex = theChosenSeed.mSeedIndexInBank + 1; anIndex < mBoard->mSeedBank->mNumPackets; anIndex++) {
        const SeedType aSeedType = FindSeedInBank(anIndex);
...
void SeedChooserScreen::ClickedSeedInChooser(ChosenSeed &theChosenSeed) {
    if (mSeedsInBank == mBoard->mSeedBank->mNumPackets) return;
    ...
    RemoveToolTip();
    mApp->PlaySample(Sexy::SOUND_TAP);
    if (mSeedsInBank == mBoard->mSeedBank->mNumPackets) EnableStartButton(true);
}
...
void SeedChooserScreen::ShowToolTip() {
    if (!mApp->mWidgetManager->mMouseIn || !mApp->mActive || mApp->GetDialogCount() > 0 ||
        ...
        if (mImitaterButton->IsMouseOver() && mMouseVisible) {
            ...
            const SeedType aSeedType = SeedHitTest(mLastMouseX, mLastMouseY);
            if (aSeedType == SEED_NONE) {
                RemoveToolTip();
            } else if (aSeedType != mToolTipSeed) {
                RemoveToolTip();
                const ChosenSeed &aChosenSeed = mChosenSeeds[aSeedType];
                const uint32_t aRecFlags = SeedNotRecommendedToPick(aSeedType);
                ...
                } else if (aChosenSeed.mSeedState == ChosenSeedState::SEED_IN_BANK && aChosenSeed.mCrazyDavePicked) {
...
// 0x486630
void SeedChooserScreen::MouseUp(const int x, const int y, const int theClickCount) {
    (void)x;
    (void)y;
    if (theClickCount == 1) {
...
void SeedChooserScreen::UpdateImitaterButton() {
    if (!mApp->SeedTypeAvailable(SEED_IMITATER)) {
        ...
        mImitaterButton->mDisabled = mChosenSeeds[SEED_IMITATER].mSeedState != ChosenSeedState::SEED_PACKET_HIDDEN;
    }
}

// 0x486770
void SeedChooserScreen::MouseDown(const int x, const int y, const int theClickCount) {
    ...
    if (mChooseState == SeedChooserState::CHOOSE_VIEW_LAWN) {
        ...
        if (mSeedsInBank != mBoard->mSeedBank->mNumPackets) {
            mApp->PlaySample(Sexy::SOUND_TAP);
            const auto aDialog = new ImitaterDialog();
        ...
        if (!mBoard->mSeedBank->ContainsPoint(x, y) && !mAlmanacButton->IsMouseOver() && !mStoreButton->IsMouseOver() &&
            mApp->CanShowAlmanac()) {
            const Zombie *aZombie = mBoard->ZombieHitTest(x - mBoard->mX, y - mBoard->mY);
        ...
        if (aSeedType != SEED_NONE && !SeedNotAllowedToPick(aSeedType)) {
            if (SeedNotAllowedDuringTrial(aSeedType)) {
                ...
                } else if (aChosenSeed.mSeedState == ChosenSeedState::SEED_IN_CHOOSER)
...
// 0x486CE0
bool SeedChooserScreen::PickedPlantType(const SeedType theSeedType) {
    for (SeedType aSeedType = SEED_PEASHOOTER; aSeedType < NUM_SEEDS_IN_CHOOSER;
         aSeedType = static_cast<SeedType>(aSeedType + 1)) {
        const ChosenSeed &aChosenSeed = mChosenSeeds[aSeedType];
        if (aChosenSeed.mSeedState == ChosenSeedState::SEED_IN_BANK) {
...
// 0x486E80
void SeedChooserScreen::KeyDown(const KeyCode theKey) { mBoard->DoTypingCheck(theKey); }

// 0x486EA0
void SeedChooserScreen::KeyChar(const SexyChar theChar) {
    if (mChooseState == SeedChooserState::CHOOSE_VIEW_LAWN &&
        (theChar == ' ' || theChar == '\r' || theChar == '\u001B'))
        CancelLawnView();
    else if (mApp->mTodCheatKeys && theChar == '\u001B') PickRandomSeeds();
