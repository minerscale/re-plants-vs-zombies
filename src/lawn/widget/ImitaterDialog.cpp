...
ImitaterDialog::ImitaterDialog()
    : LawnDialog(
          gLawnApp, Dialogs::DIALOG_IMITATER, true, _S("[CHOOSE_SEED_TO_COPY]"), _S(""), _S("[DIALOG_BUTTON_OK]"),
...
void ImitaterDialog::ShowToolTip() {
    ...
    if (aSeedType == SeedType::SEED_NONE) {
        ...
        RemoveToolTip();
        const uint32_t aRecFlags = mApp->mSeedChooserScreen->SeedNotRecommendedToPick(aSeedType);
