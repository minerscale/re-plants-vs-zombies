...
void CreditsOverlay::Draw(Graphics *g) { mParent->DrawOverlay(g); }
...
CreditScreen::CreditScreen(LawnApp *theApp) {
    ...
    mReplayButton = MakeNewButton(
        CreditScreen::Credits_Button_Replay, this, _S("[CREDITS_REPLAY_BUTTON]"), FONT_HOUSEOFTERROR16,
        IMAGE_CREDITS_PLAYBUTTON, nullptr, nullptr
    );
...
void CreditScreen::DrawFinalCredits(Graphics *g) {
    TodDrawString(
        g, _S("[CREDITS_GAMENAME]"), BOARD_WIDTH / 2, 60, FONT_HOUSEOFTERROR28, Color::White,
    ...
    TodDrawString(
        g, _S("[CREDITS_THANKS]"), BOARD_WIDTH / 2, 530, FONT_HOUSEOFTERROR16, Color::White,
        DrawStringJustification::DS_ALIGN_CENTER
    );
}
...
void CreditScreen::Draw(Graphics *g) {
    ...
    if (aDrawClippedBackgrounds1) {
        ...
        if (aTransformBackground2.mFrame != -1.0f) {
            aBackground2G.ClipRect(
                aTransformBackground2.mTransX, aTransformBackground2.mTransY, aTransformBackground2.mImage->mWidth - 1,
        ...
        if (aTransformBackground3.mFrame != -1.0f) {
            ...
            aBackground3G.ClipRect(
                aTransformBackground3.mTransX, aTransformBackground3.mTransY, aTransformBackground3.mImage->mWidth - 1,
        ...
        if (aTransformBackground4.mFrame != -1.0f) {
            ...
            aBackground4G.ClipRect(
                aTransformBackground4.mTransX, aTransformBackground4.mTransY, aTransformBackground4.mImage->mWidth - 1,
    ...
    if (aDrawClippedBackgrounds2) {
        ...
        if (aTransformBackground1.mFrame != -1.0f) {
            ...
            aBackground1G.ClipRect(
                aTransformBackground1.mTransX, aTransformBackground1.mTransY, aTransformBackground1.mImage->mWidth - 1,
    ...
    if (aDrawFog) {
        float aPercent = TodAnimateCurveFloatTime(
            aFrameFactor * 189.0f, aFrameFactor * 249.0f, aCreditsReanim->mAnimTime, 0.0f, 1.0f, TodCurves::CURVE_LINEAR
        );
...
void CreditScreen::PauseCredits() {
    ...
    if (mApp->LawnMessageBox(
            Dialogs::DIALOG_MESSAGE, _S("[CREDITS_PAUSE_HEADER]"), _S("[CREDITS_PAUSE_BODY]"),
            _S("[CREDITS_RESUME_BUTTON]"), _S("[MAIN_MENU_BUTTON]"), Dialog::BUTTONS_YES_NO
        ) == Dialog::ID_NO) {
