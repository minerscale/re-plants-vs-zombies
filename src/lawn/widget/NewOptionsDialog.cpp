...
NewOptionsDialog::NewOptionsDialog(LawnApp *theApp, bool theFromGameSelector)
    ...
    mRestartButton = MakeButton(
        NewOptionsDialog::NewOptionsDialog_Restart, this, _S("[RESTART_LEVEL_BUTTON]")
    ...
    mBackToGameButton = MakeNewButton(
        Dialog::ID_OK, this, _S("[BACK_TO_GAME]"), nullptr, IMAGE_OPTIONS_BACKTOGAMEBUTTON0,
...
void NewOptionsDialog::Draw(Sexy::Graphics *g) {
    ...
    TodDrawString(
        g, _S("Sound FX"), 186, 167 + aSfxOffset, FONT_DWARVENTODCRAFT18, aTextColor,
        DrawStringJustification::DS_ALIGN_RIGHT
    );
    TodDrawString(
        g, _S("3D Acceleration"), 274, 197 + a3DAccelOffset, FONT_DWARVENTODCRAFT18, aTextColor,
        DrawStringJustification::DS_ALIGN_RIGHT
    );
    TodDrawString(
        g, _S("Full Screen"), 274, 229 + aFullScreenOffset, FONT_DWARVENTODCRAFT18, aTextColor,
        DrawStringJustification::DS_ALIGN_RIGHT
    );
}
...
void NewOptionsDialog::CheckboxChecked(int theId, bool checked) {
    ...
    case NewOptionsDialog::NewOptionsDialog_Fullscreen:
        if (!checked && mApp->mForceFullscreen) {
            mApp->DoDialog(
                Dialogs::DIALOG_COLORDEPTH_EXP, true, _S("No Windowed Mode"),
                _S("Windowed mode is only available if your desktop was running in either\n"
                   ...
                   "If you'd like to run in Windowed mode then you need to quit the game and switch your desktop to 16 "
...
void NewOptionsDialog::ButtonDepress(int theId) {
    ...
    case NewOptionsDialog::NewOptionsDialog_Update: mApp->CheckForUpdates(); break;
    }
}
