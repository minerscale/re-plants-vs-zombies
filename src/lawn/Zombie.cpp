...
void Zombie::ZombieInitialize(
    ...
    case ZombieType::ZOMBIE_BALLOON: // 0x523397
    {
        ...
        if (IsOnBoard()) {
            ...
            const float aAnimRate = RandRangeFloat(8.0f, 10.0f);
    ...
    case ZombieType::ZOMBIE_JALAPENO_HEAD: // 0x523977
    {
        ...
        mVariant = false;
        mBodyHealth = 500;
        const int aDistance = 275 + Rand(175);
        mPhaseCounter = static_cast<int>(aDistance / mVelX) * ZOMBIE_LIMP_SPEED_FACTOR;
        break;
    }
    ...
    case ZombieType::ZOMBIE_INVALID:                      break;
    }
...
// 0x524280
void Zombie::ReanimIgnoreClipRect(const char *theTrackName, bool theIgnoreClipRect) const {
...
void Zombie::PickBungeeZombieTarget(int theColumn) {
    ...
    const TodWeightedGridArray *aGrid = TodPickFromWeightedGridArray(aPicks, aPickCount);
    mTargetCol = aGrid->mX;
    SetRow(aGrid->mY);
...
void Zombie::BungeeStealTarget() {
    ...
    if (aPlant && !aPlant->NotOnGround()) {
        ...
        if (aPlant->mSeedType != SeedType::SEED_COBCANNON && aPlant->mSeedType != SeedType::SEED_GRAVEBUSTER) {
            mTargetPlantID = static_cast<PlantID>(mBoard->mPlants.DataArrayGetID(aPlant));
...
void Zombie::UpdateZombieBungee() {
    if (IsDeadOrDying() || IsImmobilizied()) return;

    if (mZombiePhase == ZombiePhase::PHASE_BUNGEE_DIVING ||
        mZombiePhase == ZombiePhase::PHASE_BUNGEE_DIVING_SCREAMING) {
        const float aOldAltitude = mAltitude;
        mAltitude -= 8.0f;
...
void Zombie::UpdateZombiePogo() {
    ...
    if (mZombiePhase == ZombiePhase::PHASE_POGO_FORWARD_BOUNCE_2 && mPhaseCounter == 70) {
        const Plant *aPlant = FindPlantTarget(ZombieAttackType::ATTACKTYPE_VAULT);
    ...
    if (mPhaseCounter != 0) return;

    const Plant *aPlant = nullptr;
    if (IsOnBoard()) {
...
// 0x525730
void Zombie::ZombieCatapultFire(const Plant *thePlant) {
    const float aOriginX = mPosX + 113.0f;
    const float aOriginY = mPosY - 44.0f;
    ...
    const float aRangeY = aTargetY - aOriginY;
    if (aRangeX < 40.0f) {
        aRangeX = 40.0f;
    }
...
void Zombie::UpdateZombieCatapult() {
    if (mZombiePhase == ZombiePhase::PHASE_ZOMBIE_NORMAL) {
        ...
        const Plant *aPlant = FindCatapultTarget();
        if (aPlant) {
            mZombiePhase = ZombiePhase::PHASE_CATAPULT_LAUNCHING;
            mPhaseCounter = 300;
    ...
    if (mZombiePhase == ZombiePhase::PHASE_POLEVAULTER_PRE_VAULT && mHasHead &&
        ...
        if (aPlant) {
            if (mBoard->GetLadderAt(aPlant->mPlantCol, aPlant->mRow)) {
                const float aPlantX = mBoard->GridToPixelX(aPlant->mPlantCol, aPlant->mRow) + 40;
            ...
            const Reanimation *aBodyReanim = mApp->ReanimationGet(mBodyReanimID);
            const float aAnimDuration = aBodyReanim->mFrameCount / aBodyReanim->mAnimRate * 100.0f;
...
void Zombie::UpdateZombieDolphinRider() {
    if (IsTangleKelpTarget()) return;

    const bool aBackwards = IsWalkingBackwards();
...
void Zombie::UpdateZombieJackInTheBox() {
    if (mZombiePhase == ZombiePhase::PHASE_JACK_IN_THE_BOX_RUNNING) {
        ...
        if (mPhaseCounter <= 0) {
            ...
            mApp->AddTodParticle(
                aPosX, aPosY, Board::MakeRenderOrder(RenderLayer::RENDER_LAYER_TOP, 0, 0),
...
void Zombie::UpdateZombieGargantuar() {
    ...
    if (IsImmobilizied() || !mHasHead) return;
...
void Zombie::UpdateZombieImp() {
    if (mZombiePhase == ZombiePhase::PHASE_IMP_GETTING_THROWN) {
        ...
        const float aDiffY = GetPosYBasedOnRow(mRow) - mPosY;
        mPosY += aDiffY;
        mAltitude += aDiffY;
        if (mAltitude <= 0.0f) {
            mAltitude = 0.0f;
...
void Zombie::UpdateZombiePeaHead() {
    ...
    if (mPhaseCounter == 35) {
        ...
        Reanimation *aBodyReanim = mApp->ReanimationGet(mBodyReanimID);
        const int aTrackIndex = aBodyReanim->FindTrackIndex("anim_head1");
...
void Zombie::UpdateZombieSquashHead() {
...
#endif
        const int aPosX = TodAnimateCurve(50, 20, mPhaseCounter, 0, aDestX - mPosX, TodCurves::CURVE_EASE_IN_OUT);
        const int aPosY = TodAnimateCurve(50, 20, mPhaseCounter, 0, -20, TodCurves::CURVE_EASE_IN_OUT);
    ...
    if (mZombiePhase == ZombiePhase::PHASE_SQUASH_FALLING) {
        const int aPosY = TodAnimateCurve(10, 0, mPhaseCounter, -20, 74, TodCurves::CURVE_LINEAR);
        int aDestX = mBoard->GridToPixelX(mBoard->PixelToGridXKeepOnBoard(mX, mY), mRow);
#ifdef DO_FIX_BUGS
        if (mMindControlled) {
            Zombie *aZombie = FindZombieTarget();
...
void Zombie::BobsledCrash() {
    ...
    const Reanimation *aLeaderReanim = mApp->ReanimationGet(mBodyReanimID);
...
void Zombie::UpdateZombieBobsled() {
    ...
    if (mZombiePhase == ZombiePhase::PHASE_BOBSLED_SLIDING) {
        ...
        if (mZombiePhase != ZombiePhase::PHASE_BOBSLED_BOARDING) return;

        const Reanimation *aBodyReanim = mApp->ReanimationGet(mBodyReanimID);
        const int aCounter = aBodyReanim->mAnimTime * 50.0f;
        const int aPosition = GetBobsledPosition();
...
ZombieID Zombie::SummonBackupDancer(int theRow, int thePosX) {
    if (!mBoard->RowCanHaveZombieType(theRow, ZombieType::ZOMBIE_BACKUP_DANCER)) return ZombieID::ZOMBIEID_NULL;

    Zombie *aZombie = mBoard->AddZombie(ZombieType::ZOMBIE_BACKUP_DANCER, mFromWave);
    if (aZombie == nullptr) return ZombieID::ZOMBIEID_NULL;
    ...
    const int aParticleX = static_cast<int>(aZombie->mPosX) + 60;
    int aParticleY = static_cast<int>(aZombie->mPosY) + 110;
    if (aZombie->IsOnHighGround()) {
        aParticleY -= HIGH_GROUND_HEIGHT;
    }
    const int aRenderOrder = Board::MakeRenderOrder(RenderLayer::RENDER_LAYER_PARTICLE, theRow, 0);
...
void Zombie::UpdateZombieBackupDancer() {
    ...
    const ZombiePhase aDancerPhase = GetDancerPhase();
...
void Zombie::UpdateZombieDancer() {
    ...
    if (mZombiePhase == ZombiePhase::PHASE_DANCER_DANCING_IN) {
        ...
        const Reanimation *aBodyReanim = mApp->ReanimationTryToGet(mBodyReanimID);
        if (aBodyReanim->mLoopCount > 0) {
...
bool Zombie::ZombiquariumFindClosestBrain() {
    if (mBoard->HasLevelAwardDropped() || mBodyHealth > 150) return false;
    ...
    if (aBrainClosest) {
        ...
        const float aRangeY = aBrainClosest->mPosY + 15.0f - (mPosY + 40.0f);
        const float aRangeX = aBrainClosest->mPosX + 15.0f - (mPosX + 50.0f);
...
void Zombie::UpdateZombiePool() {
    if (mZombieHeight == ZombieHeight::HEIGHT_OUT_OF_POOL) {
        ...
        mAltitude--;
        const int aDepth = -40 * mScaleZombie;
        if (mAltitude <= aDepth) {
            mAltitude = aDepth;
            mZombieHeight = ZombieHeight::HEIGHT_ZOMBIE_NORMAL;
            StartWalkAnim(0);
...
void Zombie::DropPole() {
    if (mZombieType != ZombieType::ZOMBIE_POLEVAULTER) return;
...
void Zombie::DropHead(unsigned int theDamageFlags) {
    ...
    if (mBoard->mBoardData.mFutureMode) {
        const Image *aHeadImage = aBodyReanim->GetImageOverride("anim_head1");
        int aFrame = -1;
        if (aHeadImage) {
    ...
    if (mBoard->mBoardData.mPinataMode && mZombiePhase != ZombiePhase::PHASE_ZOMBIE_MOWERED) {
        const TodParticleSystem *aPinataParticle =
    ...
    if (aBodyReanim) {
        ...
        case ZombieType::ZOMBIE_POLEVAULTER:
            ...
            aBodyReanim->SetImageOverride(
                "Zombie_polevaulter_outerarm_upper", IMAGE_REANIM_ZOMBIE_POLEVAULTER_OUTERARM_UPPER2
            );
            break;
        ...
        case ZombieType::ZOMBIE_BOBSLED:
            GetTrackPosition("Zombie_outerarm_lower", aPosX, aPosY);
            aBodyReanim->SetImageOverride(
                "Zombie_dolphinrider_outerarm_upper", IMAGE_REANIM_ZOMBIE_BOBSLED_OUTERARM_UPPER2
            );
            break;
        ...
        case ZombieType::ZOMBIE_DOLPHIN_RIDER:
            GetTrackPosition("Zombie_outerarm_lower", aPosX, aPosY);
            aBodyReanim->SetImageOverride(
                "Zombie_dolphinrider_outerarm_upper", IMAGE_REANIM_ZOMBIE_DOLPHINRIDER_OUTERARM_UPPER2
            );
            break;
    ...
    if (!mInPool && !TestBit(theDamageFlags, static_cast<int>(DamageFlags::DAMAGE_DOESNT_LEAVE_BODY))) {
        ...
        if (aParticle) {
            ...
            case ZombieType::ZOMBIE_BOBSLED:
                ...
                break;
            case ZombieType::ZOMBIE_IMP: aParticle->OverrideImage(nullptr, IMAGE_REANIM_ZOMBIE_IMP_ARM2); break;
            case ZombieType::ZOMBIE_YETI:
                aParticle->OverrideImage(nullptr, IMAGE_REANIM_ZOMBIE_YETI_OUTERARM_HAND);
                break;
            case ZombieType::ZOMBIE_JACK_IN_THE_BOX: aParticle->OverrideImage(nullptr, IMAGE_ZOMBIEJACKBOXARM); break;
            case ZombieType::ZOMBIE_DIGGER:          aParticle->OverrideImage(nullptr, IMAGE_ZOMBIEDIGGERARM); break;
            case ZombieType::ZOMBIE_POLEVAULTER:
            case ZombieType::ZOMBIE_BALLOON:
            case ZombieType::ZOMBIE_DOLPHIN_RIDER:
            case ZombieType::ZOMBIE_POGO:
            case ZombieType::ZOMBIE_LADDER:          aParticle->OverrideImage(nullptr, IMAGE_REANIM_ZOMBIE_OUTERARM_HAND); break;
            default:                                 break;
            }
        }
    }
...
float Zombie::ZombieTargetLeadX(float theTime) {
    ...
    const float aCurrentPosX = aZombieRect.mX + aZombieRect.mWidth / 2.0;
    const float aDisplacementX = aSpeed * theTime;
...
void Zombie::UpdateYeti() {
    if (mMindControlled || !mHasHead || IsDeadOrDying()) return;
...
void Zombie::UpdateLadder() {
    ...
    if (mZombiePhase == ZombiePhase::PHASE_LADDER_CARRYING && mZombieHeight == ZombieHeight::HEIGHT_ZOMBIE_NORMAL) {
        ...
        const Reanimation *aBodyReanim = mApp->ReanimationTryToGet(mBodyReanimID);
        if (aBodyReanim->mLoopCount > 0) {
            const Plant *aPlant = FindPlantTarget(ZombieAttackType::ATTACKTYPE_LADDER);
            if (aPlant) {
...
Plant *Zombie::IsStandingOnSpikeweed() {
    if (mZombieType == ZombieType::ZOMBIE_ZAMBONI || mZombieType == ZombieType::ZOMBIE_CATAPULT) return nullptr;
...
void Zombie::UpdateZombiePosition() {
    ...
    if (mZombieHeight == ZombieHeight::HEIGHT_ZOMBIE_NORMAL) {
        const float aDesiredY = GetPosYBasedOnRow(mRow);
        if (mPosY < aDesiredY) {
...
void Zombie::UpdateClimbingLadder() {
    ...
    const int aLadderOriginX = mBoard->PixelToGridXKeepOnBoard(mX + 5 + aDistOffGround * 0.5f, mY);
...
void Zombie::UpdatePlaying() {
    ...
    mGroanCounter--;
    const int aZombiesCount = mBoard->mZombies.mSize;
...
void Zombie::UpdateYuckyFace() {
    ...
    if (mYuckyFaceCounter == 170) {
        ...
        const bool aIsPool = mBoard->mBoardData.mPlantRow[mRow] == PlantRowType::PLANTROW_POOL;
...
void Zombie::AnimateChewSound() {
    if (mZombiePhase == ZombiePhase::PHASE_SNORKEL_UP_TO_EAT) return;
...
void Zombie::AnimateChewEffect() {
    ...
    if (aPlant) {
        if (aPlant->mSeedType == SeedType::SEED_WALLNUT || aPlant->mSeedType == SeedType::SEED_TALLNUT) {
            const int aRenderOrder = Board::MakeRenderOrder(RenderLayer::RENDER_LAYER_PROJECTILE, mRow, 0);
            ZombieDrawPosition aDrawPos;
            GetDrawPos(aDrawPos);
...
void Zombie::DrawZombie(Graphics* g, const ZombieDrawPosition& theDrawPos)
{
    ...
    //    DrawZombiePart(g, IMAGE_ZOMBIEDOG, mIsEating ? ZombieParts::PART_HEAD : ZombieParts::PARTS_BODY, mFrame,
theDrawPos);
    //    break;
...
bool Zombie::IsWalkingBackwards() {
    if (mMindControlled) return true;
...
void Zombie::DrawZombiePart(
    ...
    const int aCelWidth = theImage->GetCelWidth();
    const int aCelHeight = theImage->GetCelHeight();
    ...
    if (mZombiePhase == ZombiePhase::PHASE_DANCER_DANCING_IN ||
        mZombiePhase == ZombiePhase::PHASE_DANCER_DANCING_LEFT) {
        const int aFrame = GetDancerFrame();
        if (!mIsEating &&
    ...
    const Rect aSrcRect(theFrame * aCelWidth, theRow * aCelHeight, aCelWidth, aDrawHeight);
    const Rect aDestRect(anOffsetX, anOffsetY, aCelWidth, aDrawHeight);
    if (mZombiePhase == ZombiePhase::PHASE_ZOMBIE_BURNED) {
        if (mMindControlled) {
            aMirror = true;
        }
    ...
    if (mJustGotShotCounter > 0) {
        g->SetDrawMode(Graphics::DRAWMODE_ADDITIVE);
        g->SetColorizeImages(true);
        const int aGrayness = mJustGotShotCounter * 10;
...
void Zombie::DrawBobsledReanim(Graphics *g, const ZombieDrawPosition &theDrawPos, bool theBeforeZombie) {
    ...
    if (mZombiePhase == ZombiePhase::PHASE_BOBSLED_CRASHING) {
        ...
        const int aAlpha = TodAnimateCurve(30, 0, mPhaseCounter, 255, 0, TodCurves::CURVE_LINEAR);
    ...
    if (aZombieLeader->mJustGotShotCounter > 0) {
        g->SetDrawMode(Graphics::DRAWMODE_ADDITIVE);
        g->SetColorizeImages(true);
        const int aGrayness = aZombieLeader->mJustGotShotCounter * 10;
        g->SetColor(Color(aGrayness, aGrayness, aGrayness, 255));
...
void Zombie::DrawBungeeTarget(Graphics *g) {
    if (!IsOnBoard() || mApp->IsFinalBossLevel()) return;

    if (mZombiePhase == ZombiePhase::PHASE_BUNGEE_HIT_OUCHY || mZombiePhase == ZombiePhase::PHASE_BUNGEE_RISING) return;

    if (mRelatedZombieID != ZombieID::ZOMBIEID_NULL) return;
    ...
    if (mZombiePhase == ZombiePhase::PHASE_BUNGEE_DIVING ||
        ...
        aTargetX += TodAnimateCurveFloat(
            BUNGEE_ZOMBIE_HEIGHT, BUNGEE_ZOMBIE_HEIGHT - 400, static_cast<int>(mAltitude), 30.0f, 0.0f,
            TodCurves::CURVE_LINEAR
        );
        aTargetY += TodAnimateCurveFloat(
            BUNGEE_ZOMBIE_HEIGHT, BUNGEE_ZOMBIE_HEIGHT - 400, static_cast<int>(mAltitude), -600.0f, 0.0f,
            TodCurves::CURVE_LINEAR
        );
    }
...
void Zombie::DrawDancerReanim(Graphics *g) {
    ...
    if (mZombiePhase != ZombiePhase::PHASE_DANCER_DANCING_IN &&
        ...
        case 0: aSpotLightColor = Color(250, 250, 160); break;
        case 1: aSpotLightColor = Color(114, 234, 170); break;
        case 2: aSpotLightColor = Color(216, 126, 202); break;
        case 3: aSpotLightColor = Color(90, 110, 140); break;
        case 4: aSpotLightColor = Color(240, 90, 130); break;
        }
...
void Zombie::DrawReanim(Graphics *g, const ZombieDrawPosition &theDrawPos, int theBaseRenderGroup) {
    ...
    if (theDrawPos.mClipHeight > CLIP_HEIGHT_LIMIT) {
        const float aDrawHeight = 120.0f - theDrawPos.mClipHeight + 71.0f;
        g->SetClipRect(
    ...
    if (mZombiePhase == ZombiePhase::PHASE_ZOMBIE_BURNED) {
        ...
        const int aGrayness =
            TodAnimateCurve(0, 39, mBoard->mBoardData.mMainCounter % 40, 155, 255, TodCurves::CURVE_BOUNCE);
        if (mChilledCounter > 0 || mIceTrapCounter > 0) {
            const int aColdColor =
    ...
    if (mJustGotShotCounter > 0 && !IsBobsledTeamWithSled()) {
        const int aGrayness = mJustGotShotCounter * 10;
        const Color aHighlightColor(aGrayness, aGrayness, aGrayness, 255);
    ...
    if (mShieldType != ShieldType::SHIELDTYPE_NONE) {
        if (mZombiePhase == ZombiePhase::PHASE_ZOMBIE_BURNED) {
            ...
            const int aGrayness = mShieldJustGotShotCounter * 10;
...
void Zombie::DrawBungeeCord(Graphics *g, int theOffsetX) {
    const int aCordCelHeight = IMAGE_BUNGEECORD->GetCelHeight() * mScaleZombie;
    float aPosX, aPosY;
    GetTrackPosition("Zombie_bungi_body", aPosX, aPosY);

    bool aSetClip = false;
    if (IsOnBoard() && mApp->IsFinalBossLevel()) {
        const Zombie *aBoss = mBoard->GetBossZombie();

        int aClipAmount = 55;
        if (aBoss->mZombiePhase == ZombiePhase::PHASE_BOSS_BUNGEES_LEAVE) {
            const Reanimation *aBossReanim = mApp->ReanimationGet(aBoss->mBodyReanimID);
            aClipAmount =
...
void Zombie::GetDrawPos(ZombieDrawPosition &theDrawPos) {
    ...
    switch (mZombieType) {
    case ZombieType::ZOMBIE_FOOTBALL: theDrawPos.mImageOffsetY -= 16.0f; break;
    case ZombieType::ZOMBIE_YETI:     theDrawPos.mImageOffsetY -= 20.0f; break;
    case ZombieType::ZOMBIE_CATAPULT:
        theDrawPos.mImageOffsetX -= 25.0f;
        theDrawPos.mImageOffsetY -= 18.0f;
        break;
    case ZombieType::ZOMBIE_POGO:    theDrawPos.mImageOffsetY += 16.0f; break;
    case ZombieType::ZOMBIE_BALLOON: theDrawPos.mImageOffsetY += 17.0f; break;
    case ZombieType::ZOMBIE_POLEVAULTER:
    ...
    case ZombieType::ZOMBIE_ZAMBONI:
        ...
        break;
    case ZombieType::ZOMBIE_GARGANTUAR:
    case ZombieType::ZOMBIE_REDEYE_GARGANTUAR: theDrawPos.mImageOffsetY -= 8.0f; break;
    case ZombieType::ZOMBIE_BOBSLED:           theDrawPos.mImageOffsetY -= 12.0f; break;
    default:                                   break;
    }
    ...
    if (mZombiePhase == ZombiePhase::PHASE_RISING_FROM_GRAVE) {
        ...
        if (mInPool) {
            ...
            const float aHeightLimit = std::min(mPhaseCounter, 40);
...
int Zombie::GetDancerFrame() {
    if (mFromWave == Zombie::ZOMBIE_WAVE_UI || IsImmobilizied()) return 0;
...
void Zombie::DrawIceTrap(Graphics *g, const ZombieDrawPosition &theDrawPos, bool theFront) {
    if (mInPool || mZombieType == ZombieType::ZOMBIE_BOSS) return;
    ...
    case ZombieType::ZOMBIE_BUNGEE:
        ...
        aScale = 1.2f;
        break;
    case ZombieType::ZOMBIE_DIGGER:   aOffsetX -= 27.0f; break;
    case ZombieType::ZOMBIE_CATAPULT: aOffsetX += 32.0f; break;
    case ZombieType::ZOMBIE_BALLOON:
        aOffsetX -= 9.0f;
        aOffsetY += 27.0f;
        break;
...
void Zombie::DrawButter(Graphics *g, const ZombieDrawPosition &theDrawPos) {
    ...
    switch (mZombieType) {
    case ZombieType::ZOMBIE_POGO: aOffsetY -= 5.0f; break;
    case ZombieType::ZOMBIE_GARGANTUAR:
    case ZombieType::ZOMBIE_REDEYE_GARGANTUAR:
        aOffsetX -= 5.0f;
        aOffsetY -= 15.0f;
...
void Zombie::Draw(Graphics *g) {
    ...
    if (mAttachmentID != AttachmentID::ATTACHMENTID_NULL) {
        ...
        if (aDrawPos.mClipHeight > CLIP_HEIGHT_LIMIT) {
            const float aDrawHeight = 120.0f - aDrawPos.mClipHeight + 21.0f;
...
bool Zombie::CanTargetPlant(Plant *thePlant, ZombieAttackType theAttackType) {
    if (mApp->IsWallnutBowlingLevel() && theAttackType != ZombieAttackType::ATTACKTYPE_VAULT) return false;

    if (thePlant->NotOnGround() || thePlant->mSeedType == SeedType::SEED_TANGLEKELP) return false;

    if (!mInPool && mBoard->IsPoolSquare(thePlant->mPlantCol, thePlant->mRow)) return false;
...
Plant *Zombie::FindPlantTarget(ZombieAttackType theAttackType) {
    const Rect aAttackRect = GetZombieAttackRect();
...
Zombie *Zombie::FindZombieTarget() {
    if (mZombiePhase == ZombiePhase::PHASE_DIGGER_TUNNELING) return nullptr;
...
// 0x52EEF0
bool Zombie::IsImmobilizied() { return mIceTrapCounter > 0 || mButteredCounter > 0; }

// 0x52EF10
bool Zombie::IsMovingAtChilledSpeed() {
    if (mChilledCounter > 0) return true;
    ...
    if (mZombieType == ZombieType::ZOMBIE_DANCER || mZombieType == ZombieType::ZOMBIE_BACKUP_DANCER) {
        ...
        if (aLeader) {
            ...
            for (int i = 0; i < NUM_BACKUP_DANCERS; i++) {
                const Zombie *aDancer = mBoard->ZombieTryToGet(aLeader->mFollowerZombieID[i]);
...
void Zombie::UpdateAnimSpeed() {
    ...
    if (mIsEating) {
        ...
        if (ZombieNotWalking() || IsBobsledTeamWithSled() || mZombieType == ZombieType::ZOMBIE_CATAPULT ||
            ...
            const ReanimatorTrack *aTrack =
                &aBodyReanim->mDefinition->mTracks.tracks[aBodyReanim->FindTrackIndex("_ground")];
            const float aDistance =
                aTrack->mTransforms[aBodyReanim->mFrameStart + aBodyReanim->mFrameCount - 1].mTransX -
                aTrack->mTransforms[aBodyReanim->mFrameStart].mTransX;
            if (aDistance >= 1e-6f) {
                const float aOneOverSpeed = aBodyReanim->mFrameCount / aDistance;
                const float aAnimRate = mVelX * aOneOverSpeed * 47.0f / mScaleZombie;
...
void Zombie::StartEating() {
    ...
    if (mZombiePhase == ZombiePhase::PHASE_DIGGER_TUNNELING) return;
...
void Zombie::CheckForPool() {
    ...
    const bool aIsPoolSquare = mBoard->IsPoolSquare(mBoard->PixelToGridX(mX + 75, mY), mRow) &&
...
void Zombie::CheckForHighGround() {
    if (mZombieHeight != ZombieHeight::HEIGHT_ZOMBIE_NORMAL || mZombieType == ZombieType::ZOMBIE_BUNGEE) return;

    const bool aIsHighGround = IsOnHighGround();
...
void Zombie::StartMindControlled() {
    ...
    if (mZombieType == ZombieType::ZOMBIE_DANCER) {
        ...
        if (aLeader) {
            const ZombieID aId = mBoard->ZombieGetID(this);
            for (int i = 0; i < NUM_BACKUP_DANCERS; i++) {
...
void Zombie::EatPlant(Plant *thePlant) {
    ...
    if (mChilledCounter > 0 && mZombieAge % 2 == 1) return;

    if (mApp->IsIZombieLevel() && thePlant->mSeedType == SeedType::SEED_SUNFLOWER) {
        const int aStageBeforeChew = thePlant->mPlantHealth / 40;
        const int aStageAfterChew = (thePlant->mPlantHealth - DAMAGE_PER_EAT) / 40;
        if (aStageAfterChew < aStageBeforeChew ||
            thePlant->mPlantHealth - DAMAGE_PER_EAT <= 0) // 若本次啃食令植物血量下降了至少 1 个阶段
        {
        ...
        if (mBoard->mBoardData.mLevel >= 2 && mBoard->mBoardData.mLevel <= 4 && mApp->IsFirstTimeAdventureMode()) {
            if (thePlant->mPlantCol > 4 && mBoard->mPlants.mSize < 15 &&
                ...
                mBoard->DisplayAdvice(
                    _S("[ADVICE_PEASHOOTER_DIED]"), MessageStyle::MESSAGE_STYLE_HINT_TALL_FAST,
...
bool Zombie::TrySpawnLevelAward() {
    ...
    CoinType aCoinType;
    const int aLevel = mBoard->mBoardData.mLevel;
...
void Zombie::DropLoot() {
    ...
    TrySpawnLevelAward();
    if (mDroppedLoot || mBoard->HasLevelAwardDropped() || !mBoard->CanDropLoot()) return;

    mDroppedLoot = true;
    const int aZombieValue = GetZombieDefinition(mZombieType).mZombieValue;
    ...
    const Rect aZombieRect = GetZombieRect();
    const int aCenterX = aZombieRect.mX + aZombieRect.mWidth / 2;
    const int aCenterY = aZombieRect.mY + aZombieRect.mHeight / 4;
    if (mZombieType == ZombieType::ZOMBIE_YETI) {
        mApp->PlayFoley(FoleyType::FOLEY_SPAWN_SUN);
...
void Zombie::BobsledDie() {
    if (!IsBobsledTeamWithSled() || !IsOnBoard()) return;
...
void Zombie::BobsledBurn() {
    if (!IsBobsledTeamWithSled()) return;
...
void Zombie::DropShield(unsigned int theDamageFlags) {
    ...
    if (mShieldType == ShieldType::SHIELDTYPE_DOOR) {
        ...
        DetachShield();
        if (!TestBit(theDamageFlags, (int)DamageFlags::DAMAGE_DOESNT_LEAVE_BODY)) {
            const float aPosX = mPosX + 31.0f;
            const float aPosY = mPosY + 80.0f;
            TodParticleSystem *aParticle =
...
int Zombie::TakeShieldDamage(int theDamage, unsigned int theDamageFlags) {
    ...
    const int aDamageIndexBeforeDamage = GetShieldDamageIndex();
    const int aDamageActual = std::min(mShieldHealth, theDamage);
    const int aDamageRemaining = theDamage - aDamageActual;
    ...
    const int aDamageIndexAfterDamage = GetShieldDamageIndex();
...
int Zombie::TakeHelmDamage(int theDamage, unsigned int theDamageFlags) {
    ...
    const int aDamageIndexBeforeDamage = GetHelmDamageIndex();
    const int aDamageActual = std::min(mHelmHealth, theDamage);
    const int aDamageRemaining = theDamage - aDamageActual;
    ...
    const int aDamageIndexAfterDamage = GetHelmDamageIndex();
...
int Zombie::TakeFlyingDamage(int theDamage, unsigned int theDamageFlags) {
    ...
    const int aDamageActual = std::min(mFlyingHealth, theDamage);
    const int aDamageRemaining = theDamage - aDamageActual;
...
void Zombie::TakeBodyDamage(int theDamage, unsigned int theDamageFlags) {
    ...
    const int aBodyHealthOrigin = mBodyHealth;
    const int aDamageIndexBeforeDamage = GetBodyDamageIndex();
    mBodyHealth -= theDamage;
    const int aDamageIndexAfterDamage = GetBodyDamageIndex();
    if (mZombieType == ZombieType::ZOMBIE_ZAMBONI) {
        Reanimation *aBodyReanim = mApp->ReanimationGet(mBodyReanimID);
        if (!TestBit(theDamageFlags, (int)DamageFlags::DAMAGE_DOESNT_CAUSE_FLASH)) {
            mApp->PlayFoley(FoleyType::FOLEY_SHIELD_HIT);
        ...
        if (aDamageIndexBeforeDamage != aDamageIndexAfterDamage) {
            if (aDamageIndexAfterDamage == 1) {
                ...
                aBodyReanim->SetImageOverride(
                    "Zombie_gargantuar_outerarm_lower", IMAGE_REANIM_ZOMBIE_GARGANTUAR_OUTERARM_LOWER2
                );
            } else if (aDamageIndexAfterDamage == 2) {
        ...
        if (aBodyHealthOrigin >= mBodyMaxHealth / BOSS_FLASH_HEALTH_FRACTION &&
            ...
            mApp->AddTodParticle(
                770.0f, 260.0f, Board::MakeRenderOrder(RenderLayer::RENDER_LAYER_TOP, 0, 0),
...
void Zombie::TakeDamage(int theDamage, unsigned int theDamageFlags) {
    if (mZombiePhase == ZombiePhase::PHASE_JACK_IN_THE_BOX_POPPING || IsDeadOrDying()) return;
...
bool Zombie::CanBeChilled() {
    if (mZombieType == ZombieType::ZOMBIE_ZAMBONI || IsBobsledTeamWithSled()) return false;
...
bool Zombie::EffectedByDamage(unsigned int theDamageRangeFlags) {
    ...
    const bool submerged = mZombieType == ZombieType::ZOMBIE_SNORKEL && mInPool && !mIsEating;
    if (TestBit(theDamageRangeFlags, (int)DamageRangeFlags::DAMAGES_SUBMERGED) && submerged) {
        return true;
    }

    const bool underground = mZombiePhase == ZombiePhase::PHASE_DIGGER_TUNNELING;
...
TodParticleSystem *Zombie::AddAttachedParticle(int thePosX, int thePosY, ParticleEffect theEffect) {
    if (mDead) return nullptr;

    if (IsFullOfAttachments(mAttachmentID)) return nullptr;
...
// 0x5324B0
bool Zombie::IsTangleKelpTarget() {
    if (mZombieHeight == ZombieHeight::HEIGHT_DRAGGED_UNDER) return true;
...
bool Zombie::IsSquashTarget(const Plant *theExcept) {
    const ZombieID anId = mBoard->ZombieGetID(this);
...
void Zombie::BalloonPropellerHatSpin(bool theSpinning) {
    Reanimation *aBodyReanim = mApp->ReanimationGet(mBodyReanimID);
    const ReanimatorTrackInstance *aHatTrackInstance = aBodyReanim->GetTrackInstanceByName("hat");
...
void Zombie::ApplyButter() {
    if (!mHasHead || !CanBeFrozen()) return;
...
// 0x5327E0
void Zombie::MowDown() {
    if (mDead || mZombiePhase == ZombiePhase::PHASE_ZOMBIE_MOWERED || mZombieType == ZombieType::ZOMBIE_BOSS) return;
...
void Zombie::ApplyBurn() {
    if (mDead || mZombiePhase == ZombiePhase::PHASE_ZOMBIE_BURNED) return;
...
void Zombie::PlayDeathAnim(unsigned int theDamageFlags) {
    ...
    auto aDeathTrackName = "anim_death";
    const int aDeathAnimHit = Rand(100);
    const bool aCanDoSuperLongDeath = mApp->HasFinishedAdventure() || mBoard->mBoardData.mLevel > 5;
...
void Zombie::DoDaisies() {
    ...
    const int aRenderPosition = Board::MakeRenderOrder(RenderLayer::RENDER_LAYER_GRAVE_STONE, mRow, 5);
...
void Zombie::UpdateDeath() {
    ...
    if (!mInPool) {
        ...
        case ZombieType::ZOMBIE_CATAPULT:
        case ZombieType::ZOMBIE_IMP:
        case ZombieType::ZOMBIE_BOSS:          aFallTime = -1.0f; break;
        ...
        case ZombieType::ZOMBIE_POLEVAULTER: aFallTime = 0.68f; break;

        case ZombieType::ZOMBIE_FOOTBALL: aFallTime = 0.52f; break;

        case ZombieType::ZOMBIE_NEWSPAPER: aFallTime = 0.63f; break;

        case ZombieType::ZOMBIE_DANCER:
        case ZombieType::ZOMBIE_BACKUP_DANCER: aFallTime = 0.83f; break;

        case ZombieType::ZOMBIE_BOBSLED: aFallTime = 0.81f; break;

        case ZombieType::ZOMBIE_JACK_IN_THE_BOX: aFallTime = 0.64f; break;

        case ZombieType::ZOMBIE_BALLOON: aFallTime = 0.68f; break;

        case ZombieType::ZOMBIE_DIGGER: aFallTime = 0.85f; break;

        case ZombieType::ZOMBIE_POGO: aFallTime = 0.84f; break;

        case ZombieType::ZOMBIE_YETI: aFallTime = 0.68f; break;

        case ZombieType::ZOMBIE_LADDER: aFallTime = 0.62f; break;

        case ZombieType::ZOMBIE_GARGANTUAR:
        case ZombieType::ZOMBIE_REDEYE_GARGANTUAR: aFallTime = 0.86f; break;

        default: aFallTime = -1.0f; break;
        }
    ...
    if (mZombieType == ZombieType::ZOMBIE_BOSS) {
        if (aBodyReanim->ShouldTriggerTimedEvent(0.1f) || aBodyReanim->ShouldTriggerTimedEvent(0.12f) ||
            ...
            const float aExplosionPosX = RandRangeFloat(600.0f, 750.0f);
            const float aExplosionPosY = RandRangeFloat(50.0f, 300.0f);
            mApp->AddTodParticle(
                aExplosionPosX, aExplosionPosY, (int)RenderLayer::RENDER_LAYER_TOP,
                ParticleEffect::PARTICLE_BOSS_EXPLOSION
            );
            mApp->PlayFoley(FoleyType::FOLEY_BOSS_EXPLOSION_SMALL);
        }
...
void Zombie::UpdateMowered() {
    const Reanimation *aMoweredReanim = mApp->ReanimationTryToGet(mMoweredReanimID);
...
bool Zombie::SetupDrawZombieWon(Graphics *g) {
    if (mFromWave != Zombie::ZOMBIE_WAVE_WINNER) return true;
    ...
    case BackgroundType::BACKGROUND_4_FOG:   g->ClipRect(-172 - mX, -mY, BOARD_WIDTH, BOARD_HEIGHT); break;
    case BackgroundType::BACKGROUND_5_ROOF:
    case BackgroundType::BACKGROUND_6_BOSS:  g->ClipRect(-220 - mX, -mY, BOARD_WIDTH, 187); break;
    default:                                 break;
    }
...
void Zombie::DrawShadow(Graphics *g) {
    ...
    if (mInPool) {
        ...
        TodDrawImageCenterScaledF(
            g, aShadowType == 0 ? IMAGE_PLANTSHADOW : IMAGE_PLANTSHADOW2, aShadowOffsetX, aShadowOffsetY + 92.0f,
            aScale, aScale
        );
    }
...
void Zombie::GetTrackPosition(const char *theTrackName, float &thePosX, float &thePosY) {
    ...
    const int aTrackIndex = aBodyReanim->FindTrackIndex(theTrackName);
...
int Zombie::GetBobsledPosition() {
    ...
    const ZombieID anId = mBoard->ZombieGetID(this);
    const Zombie *aLeaderZombie = mBoard->ZombieGet(mRelatedZombieID);
    for (int i = 0; i < NUM_BOBSLED_FOLLOWERS; i++) {
...
bool Zombie::IsBobsledTeamWithSled() { return GetBobsledPosition() != -1; }
...
void Zombie::BossSpawnContact() {
    ...
    if (mZombieAge < 3500) {
        ...
        int aZombieTypeCount = std::size(gBossZombieList);
        if (mTargetRow == 0) {
            TOD_ASSERT(gBossZombieList[aZombieTypeCount - 1] == ZombieType::ZOMBIE_GARGANTUAR);
            aZombieTypeCount--;
        }

        aZombieType = static_cast<ZombieType>(TodPickFromArray((intptr_t *)gBossZombieList, aZombieTypeCount));
    }
...
bool Zombie::BossAreBungeesDone() {
    ...
    for (int i = 0; i < NUM_BOSS_BUNGEES; i++) {
        const Zombie *aZombie = mBoard->ZombieTryToGet(mFollowerZombieID[i]);
        if (aZombie) {
...
void Zombie::BossDestroyIceballInRow() {
    ...
    if (aFireBallReanim && !mIsFireBall) {
        const float aPosX = aFireBallReanim->mOverlayMatrix.m02 + 80.0f;
        const float aPosY = aFireBallReanim->mOverlayMatrix.m12 + 80.0f;
...
void Zombie::BossDestroyFireball() {
    ...
    if (aFireBallReanim && mIsFireBall) {
        const float aPosX = aFireBallReanim->mOverlayMatrix.m02 + 80.0f;
        const float aPosY = aFireBallReanim->mOverlayMatrix.m12 + 40.0f;
        for (int i = 0; i < 6; i++) {
            const float aAngle = 2 * PI * i / 6 + PI / 2;
            Reanimation *aReanim = mApp->AddReanimation(
                aPosX + 60.0f * sin(aAngle), aPosY + 60.0f * cos(aAngle), 400000, ReanimationType::REANIM_JALAPENO_FIRE
            );
...
void Zombie::BossHeadSpitEffect() {
    if (mIsFireBall) {
        Reanimation *aBodyReanim = mApp->ReanimationGet(mBodyReanimID);
        const int aTrackIndex = aBodyReanim->FindTrackIndex("Boss_jaw");
        ReanimatorTransform aTransform;
        aBodyReanim->GetCurrentTransform(aTrackIndex, &aTransform);
        const float aFlamePosX = mPosX + aTransform.mTransX + 100.0f;
        const float aFlamePosY = mPosY + aTransform.mTransY + 50.0f;
...
void Zombie::BossHeadSpitContact() {
    ...
    const float aPosY = mBoard->GetPosYBasedOnRow(550.0f, mFireballRow) - 90.0f;
...
void Zombie::UpdateBossFireball() {
    Reanimation *aFireballReanim = mApp->ReanimationTryToGet(mBossFireBallReanimID);
    if (aFireballReanim == nullptr) return;

    const float aSpeed = aFireballReanim->GetTrackVelocity("_ground");
    aFireballReanim->mOverlayMatrix.m02 -= aSpeed;
    const float aPosX = aFireballReanim->mOverlayMatrix.m02;
    const float aPosY = mBoard->GetPosYBasedOnRow(aPosX + 75.0f, mFireballRow) - 90.0f;
    ...
    if (mIsFireBall) {
        ...
        if (aFireballReanim->mLoopType == ReanimLoopType::REANIM_LOOP && Rand(10) == 0) {
            const float aBallPosX = aPosX + 100.0f + RandRangeFloat(0.0f, 20.0f);
            const float aBallPosY =
                mBoard->GetPosYBasedOnRow(aBallPosX - 40.0f, mFireballRow) + 90.0f + RandRangeFloat(-50.0f, 0.0f);
            const int aRenderPosition = Board::MakeRenderOrder(RenderLayer::RENDER_LAYER_GRAVE_STONE, mFireballRow, 6);
...
void Zombie::UpdateBoss() {
    ...
    if (mZombiePhase == ZombiePhase::PHASE_BOSS_ENTER) {
        ...
        const int aDamageIndex = GetBodyDamageIndex();
...
void Zombie::PreloadZombieResources(ZombieType theZombieType) {
    ...
    if (theZombieType == ZombieType::ZOMBIE_DIGGER) {
        ...
        for (size_t i = 0; i < std::size(gBossZombieList); i++) {
...
void Zombie::EnableMustache(bool theEnableMustache) {
    if (mFromWave == Zombie::ZOMBIE_WAVE_UI) return;

    if (!mHasHead || Zombie::IsZombotany(mZombieType)) return;

    Reanimation *aBodyReanim = mApp->ReanimationTryToGet(mBodyReanimID);
    if (aBodyReanim == nullptr || !aBodyReanim->TrackExists("Zombie_mustache")) return;
...
void Zombie::EnableFuture(bool theEnableFuture) {
    if (mFromWave == Zombie::ZOMBIE_WAVE_UI || Zombie::IsZombotany(mZombieType)) return;

    Reanimation *aBodyReanim = mApp->ReanimationTryToGet(mBodyReanimID);
    if (aBodyReanim == nullptr || aBodyReanim->mReanimationType != ReanimationType::REANIM_ZOMBIE) return;
...
void Zombie::EnableDance() {
    ...
    if (ZombieNotWalking() || IsDeadOrDying()) return;
