...
Projectile::~Projectile() { AttachmentDie(mAttachmentID); }
...
bool Projectile::PeaAboutToHitTorchwood() {
    if (mMotionType != ProjectileMotion::MOTION_STRAIGHT) return false;
...
bool Projectile::IsSplashDamage(Zombie *theZombie) {
    if (mProjectileType && theZombie && theZombie->IsFireResistant()) return false;
...
void Projectile::Update() {
    mProjectileAge++;
    if (mApp->mGameScene != GameScenes::SCENE_PLAYING && !mBoard->mCutScene->ShouldRunUpsellBoard()) return;
...
void Projectile::DrawShadow(Graphics *g) {
    ...
    case ProjectileType::PROJECTILE_ZOMBIE_PEA: aOffsetX += 3.0f; break;
    ...
    case ProjectileType::PROJECTILE_STAR: aOffsetX += 7.0f; break;
    ...
    case ProjectileType::PROJECTILE_PUFF: return;
    ...
    case ProjectileType::PROJECTILE_FIREBALL: aScale = 1.4f; break;
    default:                                  break;
    }
...
void Projectile::ConvertToFireball(int theGridX) {
    if (mHitTorchwoodGridX == theGridX) return;
