...
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
D3DTestImage::~D3DTestImage() { FreeImage(); }
...
bool D3DTestImage::CompareEqual(const D3DTestImage &theImage) const {
    if (theImage.GetWidth() != GetWidth()) return false;

    if (theImage.GetHeight() != GetHeight()) return false;
...
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
void D3DTestImage::DrawPieceToDevice(
    LPDIRECT3DDEVICE7 theDevice, LPDIRECTDRAWSURFACE7 theTexture, float x, float y, int offx, int offy, int texWidth,
    int texHeight, DWORD theColor
) {
    float maxU = (float)texWidth / gD3DTestTextureWidth;
    float maxV = (float)texHeight / gD3DTestTextureHeight;

    if (gD3DTestHas32BitTexture) CopyToTexture8888(theTexture, offx, offy, texWidth, texHeight);
    else CopyToTexture4444(theTexture, offx, offy, texWidth, texHeight);
    ...
    D3DTester::CheckDXError(
        theDevice->DrawPrimitive(D3DPT_TRIANGLESTRIP, D3DFVF_TLVERTEX, aVertex, 4, D3DDP_WAIT), "DrawPrimitive"
    );
...
bool D3DTestImage::IsUniformColor(DWORD theColor, int &theNumMistakes, int testWidth, int testHeight) const {
    ...
    for (int i = 0; i < testHeight; i++) {
        ...
        for (int j = 0; j < testWidth; j++) {
            ...
            if (ColorDistance(aPixel, theColor) > COLOR_TOLERANCE) theNumMistakes++;
        }
...
bool D3DTester::CheckRegistry() {
    ...
    if (mRegKey == NULL) return false;

    // Check Test Version
    DWORD aVersion = 0;
    aSize = sizeof(aVersion);
    aType = REG_DWORD;
    if (RegQueryValueExA(mRegKey, "Version", 0, &aType, (uchar *)&aVersion, &aSize) != ERROR_SUCCESS) return false;

    if (aVersion != TEST_VERSION) return false;
    ...
    if (aMinVidMemory != mMinVidMemory) return false;
    ...
    if (aRecVidMemory != mRecommendedVidMemory) return false;

    // Check GUID
    UUID aGUID;
    aSize = sizeof(aGUID);
    aType = REG_BINARY;
    if (RegQueryValueExA(mRegKey, "DisplayGUID", 0, &aType, (uchar *)&aGUID, &aSize) != ERROR_SUCCESS) return false;

    if (aSize != sizeof(aGUID)) return false;
    ...
    aType = REG_SZ;
    aSize = 4096;
    if (RegQueryValueExA(mRegKey, "FailureReason", 0, &aType, (uchar *)aBuf, &aSize) != ERROR_SUCCESS) return false;
    ...
    aType = REG_SZ;
    aSize = 4096;
    if (RegQueryValueExA(mRegKey, "Warning", 0, &aType, (uchar *)aBuf, &aSize) != ERROR_SUCCESS) return false;

    mWarning = aBuf;

    mResultsChanged = false;

    if (!mFailureReason.empty()) return false;
    else return true;
}
...
bool D3DTester::FileContains(FILE *theFile, const char *theString) {
    ...
    char aBuf[4096];
    while (!feof(theFile)) {
        if (fgets(aBuf, 4000, theFile) == NULL) break;
...
bool D3DTester::IsSupportedCard(const char *theDisplayDesc) {
    ...
    {
        if (mDriverYear >= 2002) return true;

        if (StrFindNoCase(theDisplayDesc, "nvidia") >= 0) return true;

        if (StrFindNoCase(theDisplayDesc, "radeon") >= 0) return true;

        if (StrFindNoCase(theDisplayDesc, "ati ") >= 0) return true;
...
bool D3DTester::Init(HWND theHWND, LPDIRECTDRAW7 theDDraw) {
    ...
    try {
        ...
        if (theDDraw == NULL) {
            ...
            if (aDirectDrawCreateExFunc == NULL) return Fail("No DirectDrawCreateEx");
        ...
        if (!SUCCEEDED(aResult)) {
            ...
            // int64_t total64bit = dwTotal; // dwTotal will overflow but as long as the vram isn't INT_MAX + 1 it won't
            // heck up total64bit += (aBPP / 8) * aWidth * aHeight; total64bit /= (1024 * 1024); dwTotal +=
            // (aBPP/8)*aWidth*aHeight; dwTotal /= (1024*1024); if (total64bit < mMinVidMemory) 	return Fail("Not enough
            //video memory."); else if (total64bit < mRecommendedVidMemory) 	Warn("Low video memory.");
        }
        ...
        if (aPath.find_first_of("/\\") == std::string::npos) {
            ...
            if (GetSystemDirectoryA(aBuf, sizeof(aBuf) - 1)) aPath = AddTrailingSlash(aBuf, true) + aPath;
        }
        ...
        if (!(aFormat & PixelFormat_A8R8G8B8)) {
            ...
            if (!(aFormat & PixelFormat_A4R4G4B4)) return Fail("A4R4G4B4 and A8R8G8B8 texture formats not supported.");

            gD3DTestHas32BitTexture = false;
        } else gD3DTestHas32BitTexture = true;
...
void D3DTester::CopyPrimaryToTestImage() {
    ...
    if (aDesc.ddpfPixelFormat.dwRGBBitCount == 32) D3DTestPixelConvert<DWORD>(mTestImage, aDesc, 0);
    else if (aDesc.ddpfPixelFormat.dwRGBBitCount == 16) D3DTestPixelConvert<unsigned short>(mTestImage, aDesc, 0);
    else if (aDesc.ddpfPixelFormat.dwRGBBitCount == 24) throw TestException("Can't test 24-bit mode.");
...
bool D3DTester::TestAlphaBlend() {
    ...
    if (aNumErrors == 0) {
        ...
        else return Warn("Alpha blend test not uniform.");
    } else return Fail("Alpha blend Not Supported");
}
...
bool D3DTester::TestAdditiveBlend() {
    ...
    if (aNumErrors == 0) {
        if (isUniform) return true;
        else return Warn("Additive blend test not uniform.");
    } else return Fail("Additive blend not supported.");
}
...
bool D3DTester::TestAlphaAddBlend() {
    ...
    if (aNumErrors == 0) {
        if (isUniform) return true;
        else return Warn("AlphaAdd blend test not uniform.");
    } else return Fail("AlphaAdd blend not supported.");
}
...
bool D3DTester::TestAlphaModulate() {
    ...
    if (aNumErrors == 0) {
        if (isUniform) return true;
        else return Warn("AlphaModulated blend test not uniform.");
    } else return Fail("AlphaModulated blend not supported.");
}
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
bool D3DTester::TestClipProblem() {
    ...
    if (aNumErrors == 0) return true;
    else return Warn("Clip problem detected.");
}
...
bool D3DTester::DoTest() {
    if (!TestAlphaBlend()) return false;

    if (!TestAdditiveBlend()) return false;

    if (!TestAlphaAddBlend()) return false;

    if (!TestAlphaModulate()) return false;

    if (!TestClipProblem()) return false;
...
void D3DTester::TestD3D(HWND theHWND, LPDIRECTDRAW7 theDDraw) {
    if (!gSexyAppBase->mPlayingDemoBuffer) {
        ...
        if (mShouldWriteToRegistry) WriteToRegistry();
