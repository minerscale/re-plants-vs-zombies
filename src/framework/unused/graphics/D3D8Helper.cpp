...
DECLARE_INTERFACE_(IDirect3D8, IUnknown) {
    ...
    (THIS_ UINT Adapter, D3DDEVTYPE CheckType, D3DFORMAT DisplayFormat, D3DFORMAT BackBufferFormat, BOOL Windowed) PURE;
    STDMETHOD(CheckDeviceFormat)
    (THIS_ UINT Adapter, D3DDEVTYPE DeviceType, D3DFORMAT AdapterFormat, DWORD Usage, D3DRESOURCETYPE RType,
     D3DFORMAT CheckFormat) PURE;
    STDMETHOD(CheckDeviceMultiSampleType)
    (THIS_ UINT Adapter, D3DDEVTYPE DeviceType, D3DFORMAT SurfaceFormat, BOOL Windowed,
    ...
    (THIS_ UINT Adapter, D3DDEVTYPE DeviceType, HWND hFocusWindow, DWORD BehaviorFlags,
     D3DPRESENT_PARAMETERS * pPresentationParameters, IDirect3DDevice8 * *ppReturnedDeviceInterface) PURE;
};
...
static bool GetD3D8AdapterInfo2(GUID &theGUID, std::string &theDriver, std::string &theDescription) {
    ...
    if (aDirect3DCreate8Func == NULL) return false;

    gD3D8 = aDirect3DCreate8Func(D3D_SDK_VERSION);
    if (gD3D8 == NULL) return false;
    ...
    if (!SUCCEEDED(aResult)) return false;
