...
bool FModSoundManager::Initialized() { return true; }

bool FModSoundManager::LoadSound(unsigned int theSfxID, const std::string &theFilename) {
    int aLastDotPos = theFilename.rfind('.');
    if (aLastDotPos == -1) {
        if (LoadSound(theSfxID, theFilename + ".wav")) return true;

        if (LoadSound(theSfxID, theFilename + ".mp3")) return true;

        if (LoadSound(theSfxID, theFilename + ".ogg")) return true;

        if (LoadSound(theSfxID, theFilename + ".au")) return true;
...
double FModSoundManager::GetMasterVolume() { return mMasterVolume; }
