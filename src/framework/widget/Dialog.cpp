...
Dialog::Dialog(
    ...
    Widget::SetColors(gDialogColors, NUM_COLORS);
}
...
void Dialog::SetColor(int theIdx, const Color &theColor) {
    ...
    if (theIdx == COLOR_BUTTON_TEXT) {
        if (mYesButton != nullptr) mYesButton->SetColor(DialogButton::COLOR_LABEL, theColor);
        if (mNoButton != nullptr) mNoButton->SetColor(DialogButton::COLOR_LABEL, theColor);
    } else if (theIdx == COLOR_BUTTON_TEXT_HILITE) {
        if (mYesButton != nullptr) mYesButton->SetColor(DialogButton::COLOR_LABEL_HILITE, theColor);
        if (mNoButton != nullptr) mNoButton->SetColor(DialogButton::COLOR_LABEL_HILITE, theColor);
    }
}

void Dialog::SetButtonFont(_Font *theFont) {
    if (mYesButton != nullptr) mYesButton->SetFont(theFont);

    if (mNoButton != nullptr) mNoButton->SetFont(theFont);
}
...
int Dialog::GetPreferredHeight(int theWidth) {
    ...
    if (!mDialogLines.empty()) {
        ...
        aHeight += GetWordWrappedHeight(
            &g,
            theWidth - mContentInsets.mLeft - mContentInsets.mRight - mBackgroundInsets.mLeft -
...
void Dialog::Draw(Graphics *g) {
    ...
    Rect aBoxRect(
        mBackgroundInsets.mLeft, mBackgroundInsets.mTop, mWidth - mBackgroundInsets.mLeft - mBackgroundInsets.mRight,
    ...
    Rect aRect(
        mBackgroundInsets.mLeft + mContentInsets.mLeft + 2, aCurY,
...
void Dialog::AddedToManager(WidgetManager *theWidgetManager) {
    ...
    if (mYesButton != nullptr) theWidgetManager->AddWidget(mYesButton);
    if (mNoButton != nullptr) theWidgetManager->AddWidget(mNoButton);
}

void Dialog::RemovedFromManager(WidgetManager *theWidgetManager) {
    Widget::RemovedFromManager(theWidgetManager);

    if (mYesButton != nullptr) theWidgetManager->RemoveWidget(mYesButton);
    if (mNoButton != nullptr) theWidgetManager->RemoveWidget(mNoButton);
}

void Dialog::OrderInManagerChanged() {
    Widget::OrderInManagerChanged();
    if (mYesButton != nullptr) mWidgetManager->PutInfront(mYesButton, this);
    if (mNoButton != nullptr) mWidgetManager->PutInfront(mNoButton, this);
}
...
void Dialog::Resize(int theX, int theY, int theWidth, int theHeight) {
    ...
    if ((mYesButton != nullptr) && (mNoButton != nullptr)) {
        int aBtnWidth = (mWidth - mContentInsets.mLeft - mContentInsets.mRight - mBackgroundInsets.mLeft -
                         mBackgroundInsets.mRight - mButtonSidePadding * 2 - mButtonHorzSpacing) /
                        2;
        int aBtnHeight = mButtonHeight;

        mYesButton->Resize(
            mX + mBackgroundInsets.mLeft + mContentInsets.mLeft + mButtonSidePadding,
        ...
        mYesButton->Resize(
            mX + mContentInsets.mLeft + mBackgroundInsets.mLeft,
            mY + mHeight - mContentInsets.mBottom - mBackgroundInsets.mBottom - aBtnHeight,
            mWidth - mContentInsets.mLeft - mContentInsets.mRight - mBackgroundInsets.mLeft - mBackgroundInsets.mRight,
            aBtnHeight
        );
    }
}
...
void Dialog::MouseDrag(int x, int y) {
    if (mDragging) {
        ...
        if (aNewX < -8) aNewX = -8;
        else if (aNewX + mWidth > mWidgetManager->mWidth + 8) aNewX = mWidgetManager->mWidth - mWidth + 8;

        if (aNewY < -8) aNewY = -8;
        else if (aNewY + mHeight > mWidgetManager->mHeight + 8) aNewY = mWidgetManager->mHeight - mHeight + 8;

        mDragMouseX = mX + x - aNewX;
        mDragMouseY = mY + y - aNewY;

        if (mDragMouseX < 8) mDragMouseX = 8;
        else if (mDragMouseX > mWidth - 9) mDragMouseX = mWidth - 9;

        if (mDragMouseY < 8) mDragMouseY = 8;
        else if (mDragMouseY > mHeight - 9) mDragMouseY = mHeight - 9;
...
bool Dialog::IsModal() { return mIsModal; }
...
void Dialog::ButtonPress(int theId) {
    if ((theId == ID_YES) || (theId == ID_NO)) mDialogListener->DialogButtonPress(mId, theId);
}
