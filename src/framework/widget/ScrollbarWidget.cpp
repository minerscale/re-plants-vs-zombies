...
bool ScrollbarWidget::AtBottom() { return mMaxValue - mPageSize - mValue <= 1.0; }
...
int ScrollbarWidget::GetTrackSize() {
    if (mHorizontal) return mWidth - 2 * mUpButton->mWidth;
    else return mHeight - 2 * mUpButton->mWidth;
}
...
void ScrollbarWidget::Draw(Graphics *g) {
    ...
    if (mHorizontal) {
        if (mUpdateMode == UPDATE_MODE_PGUP) g->SetColor(Color(48, 48, 48));
        else g->SetColor(Color(232, 232, 232));
        g->FillRect(0, 0, aThumbPosition, mHeight);

        if (aThumbSize > 0) DrawThumb(g, aThumbPosition, 0, aThumbSize, mHeight);

        if (mUpdateMode == UPDATE_MODE_PGDN) g->SetColor(Color(48, 48, 48));
        else g->SetColor(Color(232, 232, 232));
        ...
        if (aThumbSize > 0) DrawThumb(g, 0, aThumbPosition, mWidth, aThumbSize);
...
void ScrollbarWidget::ClampValue() {
    ...
    if (mValue > mMaxValue - mPageSize) mValue = mMaxValue - mPageSize;
    if (mValue < 0.0) mValue = 0.0;
...
void ScrollbarWidget::ButtonPress(int theId) {
    ...
    if (theId == 0) SetValue(mValue - 1);
    else SetValue(mValue + 1);
}
...
int ScrollbarWidget::ThumbCompare(int x, int y) {
    ...
    if (mHorizontal) aTrackPosition = x;
    else aTrackPosition = y;

    if (aTrackPosition < GetThumbPosition()) return -1;

    if (aTrackPosition >= GetThumbPosition() + GetThumbSize()) return 1;
...
void ScrollbarWidget::MouseDrag(int x, int y) {
    ...
    if (mPressedOnThumb) {
        if (mHorizontal) SetThumbPosition(mMouseDownThumbPos + x - mMouseDownX);
        else SetThumbPosition(mMouseDownThumbPos + y - mMouseDownY);
    }
