...
void ButtonWidget::DrawButtonImage(Graphics *g, Image *theImage, const Rect &theRect, int x, int y) {
    if (theRect.mWidth != 0) g->DrawImage(mButtonImage, x, y, theRect);
    else g->DrawImage(theImage, x, y);
}
    ...
    */
    ...
    if ((mButtonImage == nullptr) && (mDownImage == nullptr)) {
        ...
        if (isDown) {
            ...
            else g->SetColor(mColors[COLOR_LABEL]);
...
void ButtonWidget::SetDisabled(bool isDisabled) {
    ...
    if (HaveButtonImage(mDisabledImage, mDisabledRect)) MarkDirty();
}

void ButtonWidget::MouseEnter() {
    Widget::MouseEnter();

    if (mOverAlphaFadeInSpeed == 0 && mOverAlpha > 0) mOverAlpha = 0;
...
void ButtonWidget::MouseLeave() {
    ...
    if (mOverAlphaSpeed == 0 && mOverAlpha > 0) mOverAlpha = 0;
...
void ButtonWidget::MouseUp(int theX, int theY, int theBtnNum, int theClickCount) {
    ...
    if (mIsOver && mWidgetManager->mHasFocus) mButtonListener->ButtonDepress(mId);
...
void ButtonWidget::Update() {
    ...
    if (mIsDown && mIsOver) mButtonListener->ButtonDownTick(mId);
