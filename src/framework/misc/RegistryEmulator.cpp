...
size_t RegistryEmulator::GetNextKey(size_t theKeyIdx) {
    ...
    while (mRegVec[theKeyIdx] != '\0')
        theKeyIdx++; // theValueName
...
bool RegistryEmulator::Read(
    ...
    while (mRegVec[aKeyIdx] != '\0')
        aKeyIdx++; // theValueName
    aKeyIdx++;     // '\0'
...
void RegistryEmulator::DeleteKey(size_t theKeyIdx) {
    ...
    if (aNextKeyIdx == SIZE_MAX) mRegVec.resize(theKeyIdx);
    else mRegVec.erase(mRegVec.begin() + theKeyIdx, mRegVec.begin() + aNextKeyIdx);
}
