...
bool XMLParser::GetUTF8Char(wchar_t *theChar, bool *error) {
    ...
    *error = true;
    ...
    if (p_fread(&aTempChar, 1, 1, mFile) == 1) {
        if ((aTempChar & 0x80) != 0) {
            if ((aTempChar & 0xC0) != 0xC0) return false;
            ...
            *aBytesReadPtr++ = aTempChar;
            ...
            while (aLen > 0) {
                ...
                if ((anExtraChar & 0xC0) != 0x80) return false;
                // sanity check: high bit set, and next highest bit NOT set.
            ...
            bool valid = true;
            switch (aTotalLen) {
            case 2: valid = !((aBytesRead[0] & 0x3E) == 0); break;
            case 3: valid = !((aBytesRead[0] & 0x1F) == 0 && (aBytesRead[1] & 0x20) == 0); break;
            case 4: valid = !((aBytesRead[0] & 0x0F) == 0 && (aBytesRead[1] & 0x30) == 0); break;
            case 5: valid = !((aBytesRead[0] & 0x07) == 0 && (aBytesRead[1] & 0x38) == 0); break;
            case 6: valid = !((aBytesRead[0] & 0x03) == 0 && (aBytesRead[1] & 0x3C) == 0); break;
            }
        ...
        if (aTempChar == 0xFEFF && mFirstChar)
        // zero-width non breaking space as the first char is a byte order marker.
        {
            mFirstChar = false;
...
bool XMLParser::GetUTF16Char(wchar_t *theChar, bool *error) {
    ...
    if (mByteSwap) aTempChar = static_cast<wchar_t>((aTempChar << 8) | (aTempChar >> 8));

    if ((aTempChar & 0xD800) == 0xD800) {
        wchar_t aNextChar = 0;
        if (p_fread(&aNextChar, 2, 1, mFile) != 1) return false;

        if (mByteSwap) aNextChar = static_cast<wchar_t>((aNextChar << 8) | (aNextChar >> 8));
        if ((aNextChar & 0xDC00) == 0xDC00) {
            *theChar = static_cast<wchar_t>((((aTempChar & ~0xD800) << 10) | (aNextChar & ~0xDC00)) + 0x10000);
        } else return false;
    ...
    if (mFile == NULL) {
        ...
        if ((mGetCharFunc = &XMLParser::GetAsciiChar)) {
            if (aFileLen >= 3) // UTF-8?
            {
                ...
                if (aChar1 == 0xEF && aChar2 == 0xBB && aChar3 == 0xBF) mGetCharFunc = &XMLParser::GetUTF8Char;
...
void XMLParser::SetStringSource(const std::string &theString) { SetStringSource(StringToWString(theString)); }
...
bool XMLParser::NextElement(XMLElement *theElement) {
    for (;;) {
        ...
        for (;;) {
            ...
            if (aVal == 1) {
                ...
                if (theElement->mType == XMLElement::TYPE_COMMENT) {
                    ...
                    *aStrPtr += static_cast<SexyChar>(c);
                            ...
                            if (theElement->mType == XMLElement::TYPE_START) {
                                ...
                                if (aAttributeKey == L"/") {
                                    ...
                                    if (aAttributeKey.length() > 0) {
                                        ...
                                        AddAttribute(
                                            theElement, WStringToSexyString(aLastAttributeKey),
                                ...
                                if (insertEnd) {
                                    ...
                                    for (int i = 0; i < anAddLength; i++)
                                        mBufferedText[anOldSize + i] =
                                            static_cast<wchar_t>(anAddString[anAddLength - i - 1]);
                                ...
                                if (aLastSectionName != theElement->mValue) {
                                    Fail(
                                        _S("End '") + theElement->mValue + _S("' Doesn't Match Start '") +
                                        aLastSectionName + _S("'")
                                    );
                                    return false;
                                }

                                if (aLastSlash == -1) mSection.erase(mSection.begin(), mSection.end());
                                else mSection.erase(mSection.begin() + aLastSlash, mSection.end());
                        ...
                        if (theElement->mType == XMLElement::TYPE_START) {
                            ...
                            if (!doingAttribute) {
                                ...
                                if (c == L'=') {
                                    ...
                                    if (!AttributeVal) aStrPtr = &aAttributeKey;
                                    else aStrPtr = &aAttributeValue;
                                }
                            }
                ...
                if (theElement->mType != XMLElement::TYPE_NONE) Fail(_S("Unexpected End of File"));
        ...
        // Ignore comments
        if ((theElement->mType != XMLElement::TYPE_COMMENT) || mAllowComments) return true;
    }
}
