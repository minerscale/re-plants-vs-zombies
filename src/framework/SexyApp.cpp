...
SexyApp::SexyApp() {
    ...
    if (mBuildNum != 0) mBuildDate = BUILD_INFO_MARKER + 8;
}
...
void SexyApp::URLOpenSucceeded(const std::string &theURL) {
    ...
    if (mShutdownOnURLOpen) mSkipAd = true;
}
...
bool SexyApp::ZylomUpdateCheckNeeded() { return ZylomGS_StandAlone_UpdateCheckNeeded(); }

void SexyApp::ZylomShowAd() { ZylomGS_StandAlone_ShowAd(mIsRegistered); }
