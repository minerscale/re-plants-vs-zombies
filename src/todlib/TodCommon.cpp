...
// 0x511510
bool TodHasUsedCheatKeys() { return false; }
...
float TodCurveQuad(float theTime) { return theTime * theTime; }

float TodCurveInvQuad(float theTime) { return 2 * theTime - theTime * theTime; }

// 0x5118C0
float TodCurveS(float theTime) { return 3 * theTime * theTime - 2 * theTime * theTime * theTime; }
...
// 0x511970
float TodCurveBounce(float theTime) { return 1 - fabs(2 * theTime - 1); }
...
float TodCurveCubic(float theTime) { return theTime * theTime * theTime; }

float TodCurveInvCubic(float theTime) { return (theTime - 1.0f) * (theTime - 1.0f) * (theTime - 1.0f) + 1.0f; }
...
float TodCurvePoly(float theTime, float thePoly) { return (float)pow(theTime, thePoly); }

float TodCurveInvPoly(float theTime, float thePoly) { return (float)pow(theTime - 1.0f, thePoly) + 1.0f; }
...
void TodDrawStringViewMatrix(
    ...
    const auto aFont = dynamic_cast<ImageFont *>(theFont);
...
void TodDrawStringMatrix(
    ...
    const auto aExpectedSize = simdutf::utf32_length_from_utf8(theString.c_str(), theString.length());
    const auto aUTF32String = std::unique_ptr<char32_t[]>(new char32_t[aExpectedSize]);
    const auto aUTF32StringLength =
        simdutf::convert_utf8_to_utf32(theString.c_str(), theString.length(), aUTF32String.get());
    const auto aU32StringView = std::u32string_view(aUTF32String.get(), aUTF32StringLength);

    TodDrawStringViewMatrix(g, theFont, theMatrix, aU32StringView, theColor);
}
...
bool TodResourceManager::TodLoadResources(const std::string &theGroup) {
    ...
    while (!gSexyAppBase->mShutdown && TodLoadNextResource())
        ;
    if (gSexyAppBase->mShutdown) return false;
