...
public:
    ...
    void AddHead(const T &theHead) {
        ...
        if (mHead) mHead->mPrev = aNode; // 已存在头部节点时，令原头部节点的上一个节点指向该新节点
        else mTail = aNode;              // 不存在头部节点时，尾部节点也为该新节点
        mSize++;
        mHead = aNode;
    }
    ...
    inline T RemoveHead() {
        ...
        mHead = aSecNode;
        if (aSecNode) aSecNode->mPrev = nullptr;
        else mTail = nullptr;
    ...
    inline TodListNode<T> *RemoveAt(TodListNode<T> *theNode) {
        if (theNode->mPrev != nullptr) theNode->mPrev->mNext = theNode->mNext;
        else mHead = theNode->mNext;

        if (theNode->mNext != nullptr) theNode->mNext->mPrev = theNode->mPrev;
        else mTail = theNode->mPrev;
