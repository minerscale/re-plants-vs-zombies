...
/*
    [为通俗理解以下内容，在此规定]
        ...
        例如，ReanimatorDefinition 作为 Reanimation 类（动画类）的定义数据类，TodParticleDefinition 作为
   TodParticleSystem 类（粒子系统类）的定义数据类等。
*/
...
public:
    ...
    // 若 _MemVar 为标志或枚举类型的数据，则 mExtraData 为指向其各标志数据的 DefSymbol 数组的指针；否则，mExtraData
    // 为空指针。 虽然借助一个 _DefClass 类的定义结构图就已经可以通过相关函数读取该 _DefClass
    // 的全部数据（即进行浅拷贝）， 但是 _DefClass
    // 中的部分指针变量指向的数据仍然需要进一步依靠相应类型的定义结构图进行递归读取（即进行深拷贝）。
...
public:
    ...
    // 例如 TodParticleDefinition 下的 *mEmitterDefs 和 mEmitterDefCount、以及 TodEmitterDefinition 下的
    // *mParticleFields 和 mParticleFieldCount 等。 在读取时，作为 mArrayCount
    // 的一项数据总是能在初次读取时就被正确读取（因为是整数类型），故其也会在后续 mArrayData 的修复过程中成为校验参考
};
