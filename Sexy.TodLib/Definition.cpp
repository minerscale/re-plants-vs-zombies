...
bool DefReadFromCacheArray(void *&theReadPtr, DefinitionArrayDef *theArray, DefMap *theDefMap) {
    ...
    for (int i = 0; i < theArray->mArrayCount; i++)
        if (!DefMapReadFromCache(
                theReadPtr, theDefMap, (void *)((int)pData + theDefMap->mDefSize * i)
            )) // 最后一个参数表示 pData[i]
            return false;
    return true;
}
...
bool DefMapReadFromCache(void *&theReadPtr, DefMap *theDefMap, void *theDefinition) {
    ...
    for (DefField *aField = theDefMap->mMapFields; *aField->mFieldName != '\0'; aField++) {
        ...
        case DefFieldType::DT_FONT:  aSucceed = DefReadFromCacheFont(theReadPtr, (Font **)aDest); break;
...
bool DefinitionReadImageField(XMLParser *theXmlParser, Image **theImage) {
    ...
    std::string aMessgae = StrFormat(
        "Failed to find image '%s' in %s", SexyStringToStringFast(aStringValue).c_str(), theXmlParser->GetFileName()
    );
...
bool DefinitionReadFontField(XMLParser *theXmlParser, Font **theFont) {
    ...
    std::string aMessgae = StrFormat(
        "Failed to find font '%s' in %s", SexyStringToStringFast(aStringValue).c_str(), theXmlParser->GetFileName()
    );
...
bool DefinitionReadField(XMLParser *theXmlParser, DefMap *theDefMap, void *theDefinition, bool *theDone) {
    ...
    for (DefField *aField = theDefMap->mMapFields; *aField->mFieldName != '\0'; aField++) {
        ...
        if (stricmp(aXMLElement.mValue.c_str(), aField->mFieldName) == 0) // 判断 aXMLElement 定义的是否为该成员变量
        {
            ...
            case DefFieldType::DT_FONT:  aSuccess = DefinitionReadFontField(theXmlParser, (Font **)pVar); break;
...
void DefinitionFreeArrayField(DefinitionArrayDef *theArray, DefMap *theDefMap) {
    for (int i = 0; i < theArray->mArrayCount; i++)
        DefinitionFreeMap(
            theDefMap, (void *)((int)theArray->mArrayData + theDefMap->mDefSize * i)
        ); // 最后一个参数表示 pData[i]
