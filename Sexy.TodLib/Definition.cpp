...
// @Patoke implement
unsigned int DefGetSizeString(char **theValue) { return strlen(*theValue) + sizeof(intptr_t); }
...
bool DefReadFromCacheArray(void *&theReadPtr, DefinitionArrayDef *theArray, DefMap *theDefMap) {
    ...
    for (int i = 0; i < theArray->mArrayCount; i++)
        if (!DefMapReadFromCache(
                theReadPtr, theDefMap, (void *)((intptr_t)pData + theDefMap->mDefSize * i)
            )) // 最后一个参数表示 pData[i]
            return false;
    return true;
}
        ...
        if (aReadCompressedFailed) // 判断是否读取成功
        {
            ...
            if (aUncompressedBuffer) {
                ...
                if (aUncompressedSize <
                    theDefMap->mDefSize +
                        sizeof(uint)) // 检测解压数据的长度是否足够“定义数据 + 一个校验值记录数据”的长度
                    ...
                    if (aCashHash != aDefHash) // 判断校验值是否一致，若不一致则说明数据发生错误
                        ...
                        // ☆ 正式开始读取定义数据 ☆
                        // 初次粗略读取 theDefinition 原类型的定义数据，囫囵吞枣地将所有记录的数据全部读入到
                        // theDefinition 中 此时 theDefinition
                        // 原本的非指针类型的数据将全部被正确读取，但其指针类型的变量会被读取并赋值为野指针
...
void DefWriteToCacheImage(void *theWritePtr, Image *theValue) {
    ResourceId aImageId{};
    if (theValue) aImageId = GetIdByImage(theValue);
    else aImageId = (ResourceId)(0xFFFFFFFF); // -1
...
void DefWriteToCacheFont(void *theWritePtr, _Font **theValue) {
    ...
    if (*theValue) TodFindFontPath(*theValue, &aFontName);
