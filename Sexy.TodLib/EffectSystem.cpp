...
void EffectSystem::ProcessDeleteQueue() {
    ...
    while (mTrailHolder->mTrails.IterateNext(aTrail))
        if (aTrail->mDead) mTrailHolder->mTrails.DataArrayFree(aTrail);

    Reanimation *aReanim = nullptr;
    while (mReanimationHolder->mReanimations.IterateNext(aReanim))
        if (aReanim->mDead) mReanimationHolder->mReanimations.DataArrayFree(aReanim);

    Attachment *aAttachment = nullptr;
    while (mAttachmentHolder->mAttachments.IterateNext(aAttachment))
        if (aAttachment->mDead) mAttachmentHolder->mAttachments.DataArrayFree(aAttachment);
}
...
void EffectSystem::Update() {
    ...
    Trail *aTrail = nullptr;
    while (mTrailHolder->mTrails.IterateNext(aTrail))
        if (!aTrail->mIsAttachment) aTrail->Update();

    Reanimation *aReanim = nullptr;
    while (mReanimationHolder->mReanimations.IterateNext(aReanim))
        if (!aReanim->mIsAttachment) aReanim->Update();
}
...
static inline int Tod_clipShape(
    ...
    if (Tod_teClip(dst, buf, top) < 3) return 0;
...
void TodTriangleGroup::DrawGroup(Graphics *g) {
    if (mImage && mTriangleCount) {
        ...
        g->mDestImage->BltTrianglesTex(
            mImage, mVertArray, mTriangleCount, Rect(0, 0, BOARD_WIDTH, BOARD_HEIGHT), Color::White, mDrawMode, 0.0f,
            0.0f, g->mLinearBlend
        );
...
// 0x446300
void TodTriangleGroup::AddTriangle(
    ...
    if (mTriangleCount > 0 && (mDrawMode != theDrawMode || mImage != theImage)) DrawGroup(g);
    mImage = theImage;
