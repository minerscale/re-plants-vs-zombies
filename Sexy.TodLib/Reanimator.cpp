...
void Reanimation::ReanimBltMatrix(
    ...
    if (!gSexyAppBase->Is3DAccelerated() && // 未开启 3D 硬件加速
        TestBit(
            gReanimationParamArray[(int)mReanimationType].mReanimParamFlags,
            (int)ReanimFlags::REANIM_FAST_DRAW_IN_SW_MODE
        ) && // 动画允许使用软件渲染
        FloatApproxEqual(theTransform.m01, 0.0f) &&
        FloatApproxEqual(theTransform.m10, 0.0f) &&           // 横向和纵向的倾斜值均为 0
    ...
    } else TodBltMatrix(g, theImage, theTransform, theClipRect, theColor, theDrawMode, theSrcRect);
}
...
void ReanimatorEnsureDefinitionLoaded(ReanimationType theReanimType, bool theIsPreloading) {
    ...
    } else // < 以下部分仅内测版执行 >
    {
        if (gAppHasUsedCheatKeys())
            ...
            TodTraceAndLog(
                "Non-cheater failed to preload '%s' on %s", aReanimParams->mReanimFileName,
