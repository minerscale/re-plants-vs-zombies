...
void TodStringRemoveReturnChars(std::string &theString) {
    for (int i = 0; i < theString.size();) {
        if (theString[i] == '\r') theString.replace(i, 1, "", 0); // 原版中此处的“1”和“""”已内联至函数内部
        else i++;
    }
}
...
void TodWriteStringSetFormat(const char *theFormat, TodStringListFormat &theCurrentFormat) {
    for (int i = 0; i < gTodStringFormatCount; i++) {
        ...
        if (strncmp(theFormat, aFormat.mFormatName, strlen(aFormat.mFormatName)) == 0) {
            if (aFormat.mNewFont != nullptr) theCurrentFormat.mNewFont = aFormat.mNewFont;
            if (aFormat.mNewColor != Color(0, 0, 0, 0)) theCurrentFormat.mNewColor = aFormat.mNewColor;
...
int TodWriteString(
    ...
    if (drawString) // 如果需要实际绘制
    {
        ...
        case DrawStringJustification::DS_ALIGN_RIGHT_VERTICAL_MIDDLE:  theX += aSpareX; break;
        case DrawStringJustification::DS_ALIGN_CENTER:
        case DrawStringJustification::DS_ALIGN_CENTER_VERTICAL_MIDDLE: theX += aSpareX / 2; break;
        case DrawStringJustification::DS_ALIGN_LEFT:
        case DrawStringJustification::DS_ALIGN_LEFT_VERTICAL_MIDDLE:   break;
        }
    }

    if (theLength < 0 || theOffset + theLength > theString.size()) theLength = theString.size();
    ...
    for (int i = theOffset; i < theLength; i++) {
        if (theString[i] == '{') {
            ...
            if (TestBit(
                    theCurrentFormat.mFormatFlags, TodStringFormatFlag::TOD_FORMAT_IGNORE_NEWLINES
                )) // 如果将换行符视作空格
            {
                if (CharIsSpaceInFormat(theString[i], theCurrentFormat)) // 如果当前字符是空格
                {
                    ...
                    continue;
                } else aPrevCharWasSpace = false; // 确保字符串中至多只能连续出现 1 个空格字符
            }
...
int TodDrawStringWrappedHelper(
    ...
    while (aCurPos < theText.size()) {
        ...
        if (aCurWidth > theRect.mWidth) // 如果当前宽度超出了限制区域的宽度，则进行换行的处理
        {
            ...
            if (aSpacePos != -1) // 如果本行前面的字符中存在空格字符
            {
                ...
                if (aCurPos < aLineFeedPos + 1) aCurPos++; // 确保每行至少有 1 个字符
            ...
            if (aLineWidth > aMaxWidth) aMaxWidth = aLineWidth; // 更新最大行宽度
    ...
    if (aLineFeedPos < theText.size()) {
        ...
        if (aLastLineLength >= 0) aYOffset += aLineSpacing;
    } else aYOffset += aLineSpacing;
