...
void StoreScreenOverlay::Draw(Graphics *g) { mParent->DrawOverlay(g); }
...
void StoreScreen::UpdateMouse() {
    ...
    for (int aItemPos = 0; aItemPos < MAX_PAGE_SPOTS; aItemPos++) {
        ...
        if (aItemType != STORE_ITEM_INVALID && !IsItemUnavailable(aItemType)) {
            ...
            if (Rect(aItemX, aItemY, 50, 87).Contains(aMouseX, aMouseY)) {
                ...
                if (mApp->mCrazyDaveMessageIndex != aMessageIndex) SetBubbleText(aMessageIndex, 100, false);
    ...
    mApp->SetCursor(
        mBackButton->mIsOver || mPrevButton->mIsOver || mNextButton->mIsOver || aShowFinger ? CURSOR_HAND
                                                                                            : CURSOR_POINTER
    );
}
    ...
    if (!CanAffordItem(theStoreItem)) {
        ...
        if (aComfirmResult == ID_OK) {
            ...
            for (int i = 0; i <= STORE_ITEM_PLANT_IMITATER; i++) {
                if (mApp->IsTrialStageLocked() || mApp->mPlayerInfo->mPurchases[i] <= 0) aGiveAchievement = false;
                ;
            }
    ...
    for (int aItemPos = 0; aItemPos < MAX_PAGE_SPOTS; aItemPos++) {
        ...
        if (Rect(aItemX, aItemY, 50, 87).Contains(x, y)) {
            if (IsFullVersionOnly(aItemType)) {
                mWaitForDialog = true;
                mApp->LawnMessageBox(
                    DIALOG_MESSAGE, "[GET_FULL_VERSION_TITLE]", "[FULL_VERSION_TO_BUY]", "[DIALOG_BUTTON_OK]", "",
                    BUTTONS_FOOTER
                );
                mWaitForDialog = false;
