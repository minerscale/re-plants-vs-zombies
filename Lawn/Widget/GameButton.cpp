...
static Color gGameButtonColors[6] = {Color(0, 0, 0),       Color(0, 0, 0),       Color(0, 0, 0),
                                     Color(255, 255, 255), Color(132, 132, 132), Color(212, 212, 212)};
...
void GameButton::Draw(Graphics *g) {
    ...
    if (mFont) {
        if (mLabelJustify == GameButton::BUTTON_LABEL_CENTER) aFontX += (mWidth - mFont->StringWidth(mLabel)) / 2;
        else if (mLabelJustify == GameButton::BUTTON_LABEL_RIGHT) aFontX += mWidth - mFont->StringWidth(mLabel);
    ...
    if (!isDown) {
        if (mDisabled && HaveButtonImage(mDisabledImage, mDisabledRect))
            ...
            DrawButtonImage(g, mOverImage, mOverRect, 0, 0);
        else if (HaveButtonImage(mButtonImage, mNormalRect)) DrawButtonImage(g, mButtonImage, mNormalRect, 0, 0);
        ...
        if (HaveButtonImage(mDownImage, mDownRect)) DrawButtonImage(g, mDownImage, mDownRect, 0, 0);
        else if (HaveButtonImage(mOverImage, mOverRect)) DrawButtonImage(g, mOverImage, mOverRect, 1, 1);
        else DrawButtonImage(g, mButtonImage, mNormalRect, 1, 1);
...
void GameButton::Update() {
    ...
    if (!mIsDown && !mIsOver && mOverAlpha > 0) {
        ...
        if (mOverAlpha < 0) mOverAlpha = 0;
    } else if (mIsOver && mOverAlphaFadeInSpeed > 0 && mOverAlpha < 1) {
        if (mOverAlphaFadeInSpeed > 0) {
            mOverAlpha += mOverAlphaFadeInSpeed;
            if (mOverAlpha > 1) mOverAlpha = 1;
        } else mOverAlpha = 1;
    }
}

// 0x448470
void GameButton::SetLabel(const SexyString &theLabel) { mLabel = TodStringTranslate(theLabel); }

// 0x4484E0
void NewLawnButton::SetLabel(const SexyString &theLabel) { mLabel = TodStringTranslate(theLabel); }

// 0x448550
void LawnStoneButton::SetLabel(const SexyString &theLabel) { mLabel = TodStringTranslate(theLabel); }
...
void NewLawnButton::Draw(Graphics *g) {
    ...
    if (mFont) {
        if (mLabelJustify == ButtonWidget::BUTTON_LABEL_CENTER) aFontX += (mWidth - mFont->StringWidth(mLabel)) / 2;
        else if (mLabelJustify == ButtonWidget::BUTTON_LABEL_RIGHT) aFontX += mWidth - mFont->StringWidth(mLabel);
    ...
    if (!isDown) {
        ...
        else DrawButtonImage(g, mButtonImage, mNormalRect, mButtonOffsetX + mTranslateX, mButtonOffsetY + mTranslateY);
