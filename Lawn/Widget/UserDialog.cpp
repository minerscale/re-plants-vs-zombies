...
UserDialog::UserDialog(LawnApp *theApp)
    : LawnDialog(
          theApp, Dialogs::DIALOG_USERDIALOG, true, _S("WHO ARE YOU?"), _S(""), _S(""), Dialog::BUTTONS_OK_CANCEL
      ) {
...
void UserDialog::Resize(int theX, int theY, int theWidth, int theHeight) {
    ...
    mRenameButton->Layout(
        LayoutFlags::LAY_SameLeft | LayoutFlags::LAY_Above | LayoutFlags::LAY_SameHeight | LayoutFlags::LAY_SameWidth,
...
void UserDialog::ButtonDepress(int theId) {
    ...
    if (!aSelName.empty()) {
        ...
        case UserDialog::UserDialog_DeleteUser: mApp->DoConfirmDeleteUserDialog(aSelName); break;
        }
    }
}
