...
#define ColorMatch(p, q)                                                                                               \
    (((p)->red == (q)->red) && ((p)->green == (q)->green) && ((p)->blue == (q)->blue) &&                               \
     ((p)->opacity) == ((q)->opacity))
...
typedef struct _PixelPacket {
    uchar blue, green, red, opacity;
} PixelPacket;

typedef struct _NodeInfo {
    struct _NodeInfo *parent, *child[8];

    double number_unique, total_red, total_green, total_blue, quantize_error;

    unsigned long color_number;

    unsigned char id, level, census;
} NodeInfo;
...
typedef struct _CubeInfo {
    ...
    PixelPacket color, *colormap;

    double distance, pruning_threshold, next_threshold;

    unsigned long nodes, free_nodes, color_number;
    ...
    double weights[ExceptionQueueLength];
...
static void *CloneMemory(void *destination, const void *source, const size_t size) {
    ...
    if ((p <= q) || ((p + size) >= q)) return (memcpy(destination, source, size));
    /*
...
static void ReacquireMemory(void **memory, const size_t size) {
    ...
    if (allocation == (void *)NULL) LiberateMemory((void **)memory);
    *memory = allocation;
}
...
static NodeInfo *GetNodeInfo(CubeInfo *cube_info, const unsigned int id, const unsigned int level, NodeInfo *parent) {
    ...
    if (cube_info->free_nodes == 0) {
        ...
        if (nodes == (Nodes *)NULL) return ((NodeInfo *)NULL);
...
static CubeInfo *GetCubeInfo(QuantizeInfo *theQuantizeInfo, unsigned int depth) {
    ...
    if (cube_info == (CubeInfo *)NULL) return ((CubeInfo *)NULL);

    memset(cube_info, 0, sizeof(CubeInfo));

    if (depth > MaxTreeDepth) depth = MaxTreeDepth;
    ...
    if (cube_info->root == (NodeInfo *)NULL) return ((CubeInfo *)NULL);
...
static void PruneChild(CubeInfo *cube_info, const NodeInfo *node_info) {
    ...
    if (node_info->census != 0)
        for (id = 0; id < MaxTreeDepth; id++)
            if (node_info->census & (1 << id)) PruneChild(cube_info, node_info->child[id]);
...
static void PruneLevel(CubeInfo *cube_info, const NodeInfo *node_info) {
    ...
    if (node_info->census != 0) {
        for (id = 0; id < MaxTreeDepth; id++) {
            if (node_info->census & (1 << id)) PruneLevel(cube_info, node_info->child[id]);
        }
    }

    if (node_info->level == cube_info->depth) PruneChild(cube_info, node_info);
}
    ...
    for (y = 0; y < (long)theHeight; y++) {
        ...
        for (x = 0; x < (long)theWidth; x += count) {
            ...
            for (level = 1; level <= cube_info->depth; level++) {
                bisect *= 0.5;
                id = (unsigned int)((((p->red) >> index) & 0x01) << 2 | (((p->green) >> index) & 0x01) << 1 |
                                    (((p->blue) >> index) & 0x01));
                ...
                if (node_info->child[id] == (NodeInfo *)NULL) {
                    ...
                    if (level == cube_info->depth) cube_info->colors++;
                }
...
static void Reduce(CubeInfo *cube_info, const NodeInfo *node_info) {
    ...
    if (node_info->census != 0) {
        for (id = 0; id < MaxTreeDepth; id++) {
            if (node_info->census & (1 << id)) Reduce(cube_info, node_info->child[id]);
        }
    }
    ...
    if (node_info->quantize_error <= cube_info->pruning_threshold) {
        ...
        if (node_info->number_unique > 0) cube_info->colors++;
...
static void ClosestColor(CubeInfo *cube_info, const NodeInfo *node_info) {
    if (cube_info->distance != 0.0) {
        ...
        if (node_info->census != 0) {
            for (id = 0; id < MaxTreeDepth; id++) {
                if (node_info->census & (1 << id)) ClosestColor(cube_info, node_info->child[id]);
            }
        }
...
static void DefineColormap(CubeInfo *cube_info, NodeInfo *node_info) {
    ...
    if (node_info->census != 0) {
        for (id = 0; id < MaxTreeDepth; id++) {
            if (node_info->census & (1 << id)) DefineColormap(cube_info, node_info->child[id]);
        }
    }
...
static unsigned int Assignment(
    CubeInfo *cube_info, PixelPacket *thePixelPackets, int theWidth, int theHeight, uchar *theIndices,
    PixelPacket *theColorTable
) {
    uchar index;
    long count, y;
    ...
    for (y = 0; y < (long)theHeight; y++) {
        ...
        for (x = 0; x < (long)theWidth; x += count) {
            ...
            for (count = 1; (x + count) < (long)theWidth; count++)
                if (!ColorMatch(q, q + count)) break;

            node_info = cube_info->root;
            for (index = MaxTreeDepth - 1; (long)index > 0; index--) {
                id = (unsigned int)((((q->red) >> index) & 0x01) << 2 | (((q->green) >> index) & 0x01) << 1 |
                                    (((q->blue) >> index) & 0x01));

                if ((node_info->census & (1 << id)) == 0) break;
...
void Sexy::Quantize8Bit(
    ...
    if (cube_info == (CubeInfo *)NULL) return;
    ...
    if (status != false) {
        ...
        // 296 ms
        Assignment(
            cube_info, (PixelPacket *)theSrcBits, theWidth, theHeight, (uchar *)theDestColorIndices,
            (PixelPacket *)theDestColorTable
        );
        aTiming = timeGetTime() - aThing;
    }
...
void Sexy::Quantize8Bit2(
    ...
    for (int aRow = 0; aRow < theHeight; aRow++) {
        for (int aCol = 0; aCol < theWidth; aCol++) {
            ...
            ushort aHashIndex =
                (ushort)(((src & 0xFF000000) >> (3 * 8 - 3 * 4)) | ((src & 0x00FF0000) >> (2 * 8 - 2 * 4)) |
                         ((src & 0x0000FF00) >> (1 * 8 - 1 * 4)) | ((src & 0x000000FF) >> (0 * 8 - 0 * 4)));
    ...
    while (aColorTableSize < 256) {
        ...
        if (aBestColorRecord == NULL) break;
        ...
        for (int aPoolIdx = 0; aPoolIdx < aPoolSize; aPoolIdx++) {
            ...
            ulong aDistance = gSignedPowerTable[((aColor >> 24)) - ((aBestColor >> 24)) + 256] +
    ...
    for (int aRow = 0; aRow < theHeight; aRow++) {
        for (int aCol = 0; aCol < theWidth; aCol++) {
            ...
            for (int aColorIdx = 0; aColorIdx < aColorTableSize; aColorIdx++) {
                ...
                ulong aDistance = gSignedPowerTable[((aColor >> 24)) - ((src >> 24)) + 256] +
