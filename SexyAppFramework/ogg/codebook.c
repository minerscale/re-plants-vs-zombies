...
int vorbis_staticbook_unpack(oggpack_buffer *opb, static_codebook *s) {
    ...
    case 2:
...

        {
            int quantvals = 0;
            switch (s->maptype) {
            case 1: quantvals = _book_maptype1_quantvals(s); break;
            case 2: quantvals = s->entries * s->dim; break;
            }
...
long vorbis_book_decode(codebook *book, oggpack_buffer *b) {
    ...
    if (packed_entry >= 0) return (book->dec_index[packed_entry]);
...
long vorbis_book_decodevv_add(
    codebook *book, ogg_int32_t **a, long offset, int ch, oggpack_buffer *b, int n, int point
) {
    long i, j, entry;
    int chptr = 0;
    int shift = point - book->binarypoint;
