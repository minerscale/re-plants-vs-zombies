...
static int _bisect_forward_serialno(
    OggVorbis_File *vf, ogg_int64_t begin, ogg_int64_t searched, ogg_int64_t end, ogg_uint32_t currentno, long m
) {
...
static int
_fetch_headers(OggVorbis_File *vf, vorbis_info *vi, vorbis_comment *vc, ogg_uint32_t *serialno, ogg_page *og_ptr) {
    ogg_page og = {0, 0, 0, 0};
...
static int _ov_open1(void *f, OggVorbis_File *vf, char *initial, long ibytes, ov_callbacks callbacks) {
    ...
    if ((ret = _fetch_headers(vf, vf->vi, vf->vc, &vf->current_serialno, NULL)) < 0) {
        vf->datasource = NULL;
        ov_clear(vf);
    } else if (vf->ready_state < PARTOPEN) vf->ready_state = PARTOPEN;
    return (ret);
}
            ...
            if (vf->ready_state >= STREAMSET) {
                ...
                if (result > 0) {
                    ...
                    if (vf->vi[vf->current_link].codec_setup) {
                        ...
                        lastblock = thisblock;
                        continue;
                    } else ogg_stream_packetout(vf->os, NULL);
                }
            }
    ...
    /* new search algorithm by HB (Nicholas Vinen) */
    {
        ...
        while (begin < end) {
            ...
            while (begin < end) {
                ...
                if (result < 0) {
                    if (bisect <= begin + 1) end = begin; /* found it */
                    else {
           ...
           raw_seek, don't keep packets preceeding granulepos. */
        {
            ...
            while (1) {
                ...
                if (op.granulepos != -1) {
                    ...
                    vf->pcm_offset += total;
                    break;
                } else result = ogg_stream_packetout(vf->os, NULL);
            }
        }
    }
...
vorbis_info *ov_info(OggVorbis_File *vf, int link) {
    if (vf->seekable) {
        if (link < 0)
            if (vf->ready_state >= STREAMSET) return vf->vi + vf->current_link;
            else return vf->vi;
        else if (link >= vf->links) return NULL;
        else return vf->vi + link;
    } else {
        return vf->vi;
    }
}
...
long ov_read(OggVorbis_File *vf, char *buffer, int bytes_req, int *bitstream) {
    ...
    if (samples > 0) {
        ...
        if (channels == 1) {
            if (samples > (bytes_req / 2)) samples = bytes_req / 2;
        } else {
            if (samples > (bytes_req / 4)) samples = bytes_req / 4;
        }
