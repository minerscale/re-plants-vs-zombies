...
static vorbis_look_mapping *mapping0_look(vorbis_dsp_state *vd, vorbis_info_mode *vm, vorbis_info_mapping *m) {
    int i;
    ...
    for (i = 0; i < info->submaps; i++) {
        ...
        look->residue_look[i] = look->residue_func[i]->look(vd, vm, ci->residue_param[resnum]);
    }
...
static vorbis_info_mapping *mapping0_unpack(vorbis_info *vi, oggpack_buffer *opb) {
    ...
    if (oggpack_read(opb, 1)) info->submaps = oggpack_read(opb, 4) + 1;
    else info->submaps = 1;
...
static int mapping0_inverse(vorbis_block *vb, vorbis_look_mapping *l) {
    ...
    for (i = 0; i < vi->channels; i++) {
        ...
        floormemo[i] = look->floor_func[submap]->inverse1(vb, look->floor_look[submap]);
    ...
    for (i = 0; i < vi->channels; i++) {
        ogg_int32_t *pcm = vb->pcm[i];
        if (nonzero[i]) _vorbis_apply_window(pcm, b->window, ci->blocksizes, vb->lW, vb->W, vb->nW);
        else
            for (j = 0; j < n; j++)
                pcm[j] = 0;
    }
...
vorbis_func_mapping mapping0_exportbundle = {
    &mapping0_unpack, &mapping0_look, &mapping0_free_info, &mapping0_free_look, &mapping0_inverse
};
