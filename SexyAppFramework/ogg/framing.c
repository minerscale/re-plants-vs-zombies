...
static void _ogg_buffer_mark_one(ogg_reference * or) { or->buffer->refcount++; }
...
static ogg_reference *ogg_buffer_sub(ogg_reference * or, long begin, long length) {
    ...
    while (or &&length) {
        ...
        head->length = length;
        if (head->length > or->length - begin) head->length = or->length - begin;
...
/* split a reference into two references; 'return' is a reference to
   ...
   'head/tail' are NULL */
static ogg_reference *ogg_buffer_split(ogg_reference **tail, ogg_reference **head, long pos) {
    ...
    while (!(os->body_fill & FINFLAG)) {
        ...
        if (!os->header_tail) {
            ...
            if (pageno != os->pageno) {
                ...
                else os->holeflag = 2; /* set for external reporting */
...
int ogg_stream_packetout(ogg_stream_state *os, ogg_packet *op) { return _packetout(os, op, 1); }

int ogg_stream_packetpeek(ogg_stream_state *os, ogg_packet *op) { return _packetout(os, op, 0); }
