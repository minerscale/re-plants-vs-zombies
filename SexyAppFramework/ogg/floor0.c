...
STIN ogg_int32_t vorbis_coslook2_i(long a) {
    ...
    if (a > 0x10000) a = 0x20000 - a;
    {
        int i = a >> COS_LOOKUP_I_SHIFT;
        int d = a & COS_LOOKUP_I_MASK;
        a = ((COS_LOOKUP_I[i] << COS_LOOKUP_I_SHIFT) - d * (COS_LOOKUP_I[i] - COS_LOOKUP_I[i + 1])) >>
...
static const int barklook[28] = {0,    100,  200,  301,   405,   516,   635,   766,  912,  1077,
                                 1263, 1476, 1720, 2003,  2333,  2721,  3184,  3742, 4428, 5285,
                                 6376, 7791, 9662, 12181, 15624, 20397, 27087, 36554};
...
static vorbis_look_floor *floor0_look(vorbis_dsp_state *vd, vorbis_info_mode *mi, vorbis_info_floor *i) {
    int j;
    ...
    for (j = 0; j < look->n; j++) {
        ...
        int val = (look->ln * ((toBARK(info->rate / 2 * j / look->n) << 11) / toBARK(info->rate / 2))) >> 11;
...
/* export hooks */
vorbis_func_floor floor0_exportbundle = {&floor0_unpack,    &floor0_look,     &floor0_free_info,
