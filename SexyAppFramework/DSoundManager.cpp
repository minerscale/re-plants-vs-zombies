...
#define SOUND_FLAGS                                                                                                    \
    (DSBCAPS_CTRLPAN | DSBCAPS_CTRLVOLUME | DSBCAPS_STATIC | DSBCAPS_LOCSOFTWARE | DSBCAPS_GLOBALFOCUS |               \
    ...
    if (theHWnd != NULL) {
        ...
        if (aDirectSoundCreateFunc != NULL && aDirectSoundCreateFunc(NULL, &mDirectSound, NULL) == DS_OK) {
            ...
            if (SUCCEEDED(aResult)) {
                ...
                aBufferDesc.dwFlags =
                    DSBCAPS_PRIMARYBUFFER; //| DSBCAPS_CTRL3D; // Need default controls (pan, volume, frequency).
...
DSoundManager::~DSoundManager() {
    ...
    if (mPrimaryBuffer) mPrimaryBuffer->Release();

    if (mDirectSound != NULL) {
        if (mHaveFMod) gFMod->FSOUND_Close();

        mDirectSound->Release();

        if (mHaveFMod) FreeFModDLL();
    }
}
...
int DSoundManager::FindFreeChannel() {
    ...
    for (int i = 0; i < MAX_CHANNELS; i++) {
        if (mPlayingSounds[i] == NULL) return i;
...
void DSoundManager::SetVolume(double theVolume) {
    ...
    for (int i = 0; i < MAX_CHANNELS; i++)
        if (mPlayingSounds[i] != NULL) mPlayingSounds[i]->RehupVolume();
}
...
bool DSoundManager::LoadWAVSound(unsigned int theSfxID, const std::string &theFilename) {
    ...
    while (!p_feof(fp)) {
        p_fread(aChunkType, 1, 4, fp);
        if (p_fread(&aChunkSize, 4, 1, fp) == 0) return false;
...
int ov_pak_open(PFILE *f, OggVorbis_File *vf, char *initial, long ibytes) {
    ov_callbacks callbacks = {
        (size_t(*)(void *, size_t, size_t, void *))p_fread, (int (*)(void *, ogg_int64_t, int))p_fseek64_wrap,
...
#else
bool DSoundManager::LoadOGGSound(unsigned int theSfxID, const std::string &theFilename) { return false; }
#endif
...
bool DSoundManager::LoadAUSound(unsigned int theSfxID, const std::string &theFilename) {
    ...
    } else memcpy(lpvPtr, aSrcBuffer, aDataSize);
...
bool DSoundManager::LoadSound(unsigned int theSfxID, const std::string &theFilename) {
    ...
    if (!mDirectSound) return true; // sounds just	won't play, but this is not treated as a failure condition
    ...
    if ((aFilename.length() > 2) && (aFilename[0] != '\\') && (aFilename[0] != '/') && (aFilename[1] != ':')) {
        ...
        if (LoadWAVSound(theSfxID, aCachedName)) return true;
        MkDir(GetFileDir(aCachedName));
    }

    if (LoadWAVSound(theSfxID, aFilename + ".wav")) return true;
...
int DSoundManager::GetFreeSoundId() {
    for (int i = 0; i < MAX_SOURCE_SOUNDS; i++) {
        if (mSourceSounds[i] == NULL) return i;
    }
...
int DSoundManager::GetNumSounds() {
    int aCount = 0;
    for (int i = 0; i < MAX_SOURCE_SOUNDS; i++) {
        if (mSourceSounds[i] != NULL) aCount++;
    }
...
bool DSoundManager::GetTheFileTime(const std::string &theDepFile, FILETIME *theFileTime) {
    ...
    if (aDepFileHandle == INVALID_HANDLE_VALUE) return false;
...
bool DSoundManager::WriteWAV(unsigned int theSfxID, const std::string &theFilename, const std::string &theDepFile) {
    if ((theFilename.length() == 0) || (theSfxID < 0) || (theSfxID >= MAX_SOURCE_SOUNDS)) return false;

    ulong aDataSize = mSourceDataSizes[theSfxID];

    void *lpvPtr;
    DWORD dwBytes;
    if (mSourceSounds[theSfxID]->Lock(0, aDataSize, &lpvPtr, &dwBytes, NULL, NULL, 0) != DS_OK) return false;
...
SoundInstance *DSoundManager::GetSoundInstance(unsigned int theSfxID) {
    ...
    if (aFreeChannel < 0) return NULL;

    if (mDirectSound == NULL) {
        mPlayingSounds[aFreeChannel] = new DSoundInstance(this, NULL);
    } else {
        if (mSourceSounds[theSfxID] == NULL) return NULL;
...
void DSoundManager::SetCooperativeWindow(HWND theHWnd) {
    if (mDirectSound != NULL) mDirectSound->SetCooperativeLevel(theHWnd, DSSCL_NORMAL);
    /*
