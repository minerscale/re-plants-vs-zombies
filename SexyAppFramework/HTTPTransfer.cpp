...
bool HTTPTransfer::SocketWait(bool checkRead, bool checkWrite) {
    while (!mExiting) {
        ...
        if (checkRead) FD_SET(mSocket, &aReadSet);

        if (checkWrite) FD_SET(mSocket, &aWriteSet);
        ...
        if (FD_ISSET(mSocket, &aReadSet)) return true;

        if (FD_ISSET(mSocket, &aWriteSet)) return true;
    }
...
void HTTPTransfer::GetThreadProc() {
    ...
    if (mSocket == 0) Fail(RESULT_SOCKET_ERROR);
    ...
    if (!mExiting) {
        ...
        if (anAddr == INADDR_NONE) {
            ...
            if (aHostEnt != NULL) memcpy(&anAddr, aHostEnt->h_addr_list[0], 4);
        }
        ...
        if (anAddr == INADDR_NONE) {
            ...
            if (::connect(mSocket, (sockaddr *)&aSockAddrIn, sizeof(SOCKADDR_IN)) != 0) {
                if (WSAGetLastError() != WSAEWOULDBLOCK) Fail(RESULT_CONNECT_FAIL);

                // Wait for socket to become writable to know we connected
                if (!SocketWait(false, true)) Fail(RESULT_CONNECT_FAIL);
            }
            ...
            while (!mExiting) {
                ...
                if (!gotHeader) {
                    ...
                    if (anEndLPos == -1) continue; // Not enough for a full line yet
                    ...
                    if (aLine.length() == 0) gotHeader = true;
                } else {
                    if (chunked) {
                        ...
                        if (aChunkLengthLeft == 0) {
                            ...
                            if (aChunkLengthLeft == 0) break; // Zero-size chunk marks end of chunked transfer
    ...
    else if (mResult == RESULT_NOT_COMPLETED) mResult = RESULT_DONE;

    mThreadRunning = false;
    mTransferPending = false;
}

void HTTPTransfer::GetThreadProcStub(void *theArg) { ((HTTPTransfer *)theArg)->GetThreadProc(); }
...
void HTTPTransfer::PrepareTransfer(const std::string &theURL) {
    ...
    if (aSSPos != -1) aPos = aSSPos + 2;

    if (aSSPos != -1) mProto = mURL.substr(0, aSSPos - 1);
...
void HTTPTransfer::StartTransfer() {
    ...
    if ((gSexyAppBase != NULL) && (gSexyAppBase->mPlayingDemoBuffer)) return;
...
void HTTPTransfer::UpdateStatus() {
    ...
    if (gSexyAppBase != NULL) {
        if (gSexyAppBase->mPlayingDemoBuffer) {
            ...
            if (gSexyAppBase->PrepareDemoCommand(false)) {
                if ((!gSexyAppBase->mDemoIsShortCmd) && (gSexyAppBase->mDemoCmdNum == DEMO_HTTP_RESULT)) {
                    ...
                    if (aTransferId == mTransferId) {
                        ...
                        // mTransferId, mResult, gSexyAppBase->mUpdateCount).c_str());
                    } else {
                        // Not us, go back
            ...
            if (mResult == RESULT_NOT_COMPLETED) gSexyAppBase->mDemoBuffer.WriteString("");
            else gSexyAppBase->mDemoBuffer.WriteString(mContent);
...
std::string HTTPTransfer::GetContent() {
    if (mResult == RESULT_NOT_COMPLETED) return "";
