...
Checkbox::Checkbox(Image *theUncheckedImage, Image *theCheckedImage, int theId, CheckboxListener *theCheckboxListener)
    : mListener(theCheckboxListener), mId(theId), mChecked(false), mUncheckedImage(theUncheckedImage),
      mCheckedImage(theCheckedImage), mOutlineColor(Color::White), mBkgColor(Color(80, 80, 80)),
...
void Checkbox::SetChecked(bool checked, bool tellListener) {
    mChecked = checked;
    if (tellListener && mListener) mListener->CheckboxChecked(mId, mChecked);
    MarkDirty();
}

bool Checkbox::IsChecked() { return mChecked; }
...
void Checkbox::MouseDown(int x, int y, int theBtnNum, int theClickCount) {
    ...
    mChecked = !mChecked;
    if (mListener) mListener->CheckboxChecked(mId, mChecked);
    MarkDirty();
}
