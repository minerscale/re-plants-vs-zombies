...
DSoundInstance::~DSoundInstance() {
    if (mSoundBuffer != NULL) mSoundBuffer->Release();
}
...
void DSoundInstance::RehupPan() {
    if (mSoundBuffer != NULL) mSoundBuffer->SetPan(mBasePan + mPan);
}
...
bool DSoundInstance::Play(bool looping, bool autoRelease) {
    ...
    if (looping) {
        if (mSoundBuffer->Play(0, 0, DSBPLAY_LOOPING) != DS_OK) return false;
    } else {
...
void DSoundInstance::AdjustPitch(double theNumSteps) {
    if (mSoundBuffer != NULL) {
        ...
        if (aNewFrequency < DSBFREQUENCY_MIN) aNewFrequency = DSBFREQUENCY_MIN;
        if (aNewFrequency > DSBFREQUENCY_MAX) aNewFrequency = DSBFREQUENCY_MAX;
...
bool DSoundInstance::IsPlaying() {
    if (!mHasPlayed) return false;

    if (mSoundBuffer == NULL) return false;
