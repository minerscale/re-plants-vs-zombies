...
int Image::GetCelHeight() { return mHeight / mNumRows; }
...
void AnimInfo::Compute(int theNumCels, int theBeginFrameTime, int theEndFrameTime) {
    ...
    mNumCels = theNumCels;
    if (mNumCels <= 0) mNumCels = 1;

    if (mFrameDelay <= 0) mFrameDelay = 1;
    ...
    if (theEndFrameTime > 0) SetPerFrameDelay(mNumCels - 1, theEndFrameTime);
    ...
    if (!mPerFrameDelay.empty()) {
        ...
        for (i = 0; i < mNumCels; i++) {
            if (mPerFrameDelay[i] <= 0) mPerFrameDelay[i] = mFrameDelay;

            mTotalAnimTime += mPerFrameDelay[i];
        }
    } else mTotalAnimTime = mFrameDelay * mNumCels;

    if (!mFrameMap.empty()) mFrameMap.resize(mNumCels);
}

int AnimInfo::GetPerFrameCel(int theTime) {
    for (int i = 0; i < mNumCels; i++) {
        theTime -= mPerFrameDelay[i];
        if (theTime < 0) return i;
    }
...
int AnimInfo::GetCel(int theTime) {
    if (mAnimType == AnimType_Once && theTime >= mTotalAnimTime) {
        if (!mFrameMap.empty()) return mFrameMap[mFrameMap.size() - 1];
        else return mNumCels - 1;
    }

    theTime = theTime % mTotalAnimTime;

    int aFrame;
    if (!mPerFrameDelay.empty()) aFrame = GetPerFrameCel(theTime);
    else aFrame = (theTime / mFrameDelay) % mNumCels;

    if (mFrameMap.empty()) return aFrame;
...
bool Image::PolyFill3D(const Point *, int, const Rect *, const Color &, int, int, int) { return false; }
...
void Image::BltMatrix(
    Image *, float, float, const SexyMatrix3 &, const Rect &, const Color &, int, const Rect &, bool
) {}
void Image::BltTrianglesTex(
    Image *, const TriVertex (*)[3], int, const Rect &, const Color &, int, float, float, bool
) {}
