...
void flushCommandBuffer() {
    if (inRecording) {
        ...
        auto time_passed = std::chrono::duration_cast<std::chrono::duration<double>>(now_time - begin_time).count();
        if (time_passed > display_interval_sec) {
            printf(
                "\nblit/flush: %f\npass/frame: %f\nblts/frame: %f\n framerate: %f\n", (double)num_flushes / num_blits,
                num_passes / (gFramerate * time_passed), num_blits / (gFramerate * time_passed), gFramerate
            );
            begin_time = now_time;
...
void VkImage::
    ...
    if (cacheMiss) {
        ...
        vkCmdBindDescriptorSets(
            ...
            &imageDescriptors[imageBufferIdx], 0, nullptr
        );
    ...
    if (thisLayoutSuboptimal || otherLayoutSuboptimal) {
        ...
        vkCmdPipelineBarrier(
            imageCommandBuffers[imageBufferIdx], VK_PIPELINE_STAGE_ALL_COMMANDS_BIT, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT,
            0, 0, nullptr, 0, nullptr, (thisLayoutSuboptimal && otherLayoutSuboptimal) ? 2 : 1,
            &barriers[otherLayoutSuboptimal && (!thisLayoutSuboptimal)]
        );
    }
...
void VkImage::BltF(
    Image * /*theImage*/, float /*theX*/, float /*theY*/, const Rect & /*theSrcRect*/, const Rect & /*theClipRect*/,
