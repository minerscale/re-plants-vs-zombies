...
void createTextureImage() {
    ...
    // width:  u16
    // height: u16
    // RGBA:   u32*width*height
    FILE *fp = fopen("textures/texture.data", "rb");
    if (!fp) {
        throw std::runtime_error("Couldn't open texture file");
    }
    fread(texDim, sizeof(texDim[0]), sizeof(texDim) / sizeof(texDim[0]), fp);

    VkDeviceSize imageSize = texDim[0] * texDim[1] * sizeof(uint32_t);

    uint32_t *imdata = (uint32_t *)malloc(imageSize);
    fread(imdata, imageSize, 1, fp);
    fclose(fp);
    ...
    memcpy(data, imdata, static_cast<size_t>(imageSize));
    vkUnmapMemory(device, stagingBufferMemory);

    free(imdata);

    createImage(
        texDim[0], texDim[1], VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_TILING_OPTIMAL,
    ...
    copyBufferToImage(stagingBuffer, textureImage, texDim[0], texDim[1]);
...
static void windowResizeCallback(GLFWwindow *, int focused) { windowFocused = focused; }

VkInterface::VkInterface(int width, int height) {
    // Init glfw
    glfwInit();
    ...
    glfwSetWindowFocusCallback(window, windowResizeCallback);
...
bool VkInterface::IsFocused() { return windowFocused; }

bool VkInterface::ShouldClose() { return glfwWindowShouldClose(window); }

void VkInterface::ReleaseMouseCapture() { glfwSetInputMode(window, GLFW_CURSOR, GLFW_CURSOR_DISABLED); }
