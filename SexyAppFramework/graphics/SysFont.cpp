...
SysFont::SysFont(
    SexyAppBase *theApp, const std::string &theFace, int thePointSize, int theScript, bool bold, bool italics,
    bool underline
) {
...
void SysFont::Init(
    ...
    mHFont = CreateFontA(
        aHeight, 0, 0, 0, bold ? FW_BOLD : FW_NORMAL, italics, underline, false, theScript, OUT_TT_PRECIS,
        CLIP_DEFAULT_PRECIS, ANTIALIASED_QUALITY, DEFAULT_PITCH | FF_DONTCARE, theFace.c_str()
    );
...
SysFont::~SysFont() { DeleteObject(mHFont); }
...
ImageFont *SysFont::CreateImageFont() {
    ...
    for (i = 0; i < 256; i++) {
        ...
        aFontLayer->GetCharData(aChar)->mImageRect =
            Rect(aChar * anImageCharWidth, 0, anImageCharWidth, anImage->mHeight);
...
void SysFont::DrawString(
    ...
    if (aDDImage != NULL) {
        ...
        if (aSurface != NULL) {
            ...
            if (aDDImage->mLockCount > 0) aDDImage->mSurface->Unlock(NULL);
            ...
            if (aSurface->GetDC(&aDC) == DD_OK) {
                ...
                SetBkMode(aDC, TRANSPARENT);
                IntersectClipRect(
                    aDC, theClipRect.mX, theClipRect.mY, theClipRect.mX + theClipRect.mWidth,
                    theClipRect.mY + theClipRect.mHeight
                );

                if (mDrawShadow) {
                    SetTextColor(aDC, RGB(0, 0, 0));
                    TextOut(
                        aDC, theX + g->mTransX + 1, theY - mAscent + 1 + g->mTransY + 1, theString.c_str(),
                        theString.length()
                    );
                    if (mSimulateBold)
                        TextOut(
                            aDC, theX + g->mTransX + 2, theY - mAscent + 1 + g->mTransY + 1, theString.c_str(),
                            theString.length()
                        );
                }
                ...
                if (mSimulateBold)
                    TextOut(
                        aDC, theX + g->mTransX + 1, theY - mAscent + 1 + g->mTransY, theString.c_str(),
                        theString.length()
                    );
            ...
            if (aDDImage->mLockCount > 0)
                aDDImage->mSurface->Lock(
                    NULL, &aDDImage->mLockedSurfaceDesc, DDLOCK_SURFACEMEMORYPTR | DDLOCK_WAIT, NULL
                );
        }
    } else if (g->mDestImage != &Graphics::mStaticImage) // DrawString can be called when not drawing onto an image.
    {
        ...
        HBITMAP whiteBitmap =
            (HBITMAP)CreateDIBSection(aDC, (BITMAPINFO *)&bih, DIB_RGB_COLORS, (void **)&whiteBits, NULL, 0);
        HBITMAP blackBitmap =
            (HBITMAP)CreateDIBSection(aDC, (BITMAPINFO *)&bih, DIB_RGB_COLORS, (void **)&blackBits, NULL, 0);
...
#define DRAW_BITMAP(bmp, brush)                                                                                        \
    {                                                                                                                  \
        ...
        if (mDrawShadow) {                                                                                             \
            ...
            if (mSimulateBold) TextOut(aDC, 2, 1, theString.c_str(), theString.length());                              \
        }                                                                                                              \
        SetTextColor(aDC, RGB(theColor.GetRed(), theColor.GetGreen(), theColor.GetBlue()));                            \
        TextOut(aDC, 0, 0, theString.c_str(), theString.length());                                                     \
        if (mSimulateBold) TextOut(aDC, 1, 0, theString.c_str(), theString.length());                                  \
                                                                                                                       \
        ...
        while (aCount > 0) {
            if (*ptr1 == *ptr2) *ptr1 |= 0xFF000000;
            else if ((*ptr1 & 0xFFFFFF) != 0xFFFFFF || (*ptr2 & 0xFFFFFF) != 0x000000) // if not the background of
                                                                                       // either, it's a 'blend'
            {
...
_Font *SysFont::Duplicate() { return new SysFont(*this); }
