...
bool Sexy::Quantize8Bit(
    ...
    for (int anIdx = 1; anIdx < aSize; anIdx++) {
        ...
        for (;;) {
            ...
            if (aColor < aCheckColor) aRightPos = aMiddlePos - 1;
            else if (aColor > aCheckColor) aLeftPos = aMiddlePos + 1;
            else {
            ...
            if (aLeftPos > aRightPos) {
                ...
                memmove(
                    aSearchTable + anInsertPos + 1, aSearchTable + anInsertPos,
                    (aColorTableSize - anInsertPos) * sizeof(ulong)
                );
                aSearchTable[anInsertPos] = aColor;

                memmove(
                    aTranslationTable + anInsertPos + 1, aTranslationTable + anInsertPos,
