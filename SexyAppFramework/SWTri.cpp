...
static int FixedFloor(int x) {
    if (x > 0) return x & 0xFFFF0000;
    else return (x & 0xFFFF0000) - 0x10000;
}
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
static inline void
lClip(SWHelper::XYZStruct &dst, const SWHelper::XYZStruct &on, const SWHelper::XYZStruct &off, const float edge) {
...
static inline void
rClip(SWHelper::XYZStruct &dst, const SWHelper::XYZStruct &on, const SWHelper::XYZStruct &off, const float edge) {
    float delta = (edge - off.mX) / (on.mX - off.mX);
    dst.mX = off.mX + (on.mX - off.mX) * delta;
    dst.mY = off.mY + (on.mY - off.mY) * delta;
    dst.mU = off.mU + (on.mU - off.mU) * delta;
...
static inline void
tClip(SWHelper::XYZStruct &dst, const SWHelper::XYZStruct &on, const SWHelper::XYZStruct &off, const float edge) {
...
static inline void
bClip(SWHelper::XYZStruct &dst, const SWHelper::XYZStruct &on, const SWHelper::XYZStruct &off, const float edge) {
    float delta = (edge - off.mY) / (on.mY - off.mY);
    dst.mX = off.mX + (on.mX - off.mX) * delta;
    dst.mY = off.mY + (on.mY - off.mY) * delta;
    dst.mU = off.mU + (on.mU - off.mU) * delta;
...
static inline int clipShape(
    SWHelper::XYZStruct **dst, SWHelper::XYZStruct **src, const float left, const float right, const float top,
    const float bottom
) {
...
void SWHelper::SWDrawShape(
    XYZStruct *theVerts, int theNumVerts, MemoryImage *theImage, const Color &theColor, int theDrawMode,
    const Rect &theClipRect, void *theSurface, int thePitch, int thePixelFormat, bool blend, bool vertexColor
) {
    ...
    if (theImage) theImage->CommitBits();
    ...
    for (;;) {
        ...
        if (vCount) {
            ...
            SWDrawTriangle(
                textured, talpha, vertexColor, globalargb, pVerts, pFrameBuffer, thePitch, &textureInfo, globalDiffuse,
                thePixelFormat, blend
            );
            ...
            if (vCount > 3) {
                for (unsigned int extraVert = 2; extraVert < vCount - 1; ++extraVert) {
                    ...
                    SWDrawTriangle(
                        textured, talpha, vertexColor, globalargb, pVerts, pFrameBuffer, thePitch, &textureInfo,
...
void Sexy::SWTri_AddDrawTriFunc(
    bool textured, bool talpha, bool mod_argb, bool global_argb, int thePixelFormat, bool blend, DrawTriFunc theFunc
) {
...
void SWHelper::SWDrawTriangle(
    bool textured, bool talpha, bool mod_argb, bool global_argb, SWVertex *pVerts, unsigned int *pFrameBuffer,
    const unsigned int bytepitch, const SWTextureInfo *textureInfo, SWDiffuse &globalDiffuse, int thePixelFormat,
    bool blend
) {
    int aType = (blend ? 1 : 0) | (global_argb ? 2 : 0) | (mod_argb ? 4 : 0) | (talpha ? 8 : 0) | (textured ? 16 : 0);
    switch (thePixelFormat) {
    ...
    } else aFunc(pVerts, pFrameBuffer, bytepitch, textureInfo, globalDiffuse);
