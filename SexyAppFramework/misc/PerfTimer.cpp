...
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
int PerfTimer::GetCPUSpeedMHz() { return (int)(gCPUSpeed / 1000000); }
...
struct PerfInfo {
    ...
    PerfInfo(const char *theName)
        : mPerfName(theName), mStartTime(0), mDuration(0), mLongestCall(0), mStartCount(0), mCallCount(0) {}
...
static inline void InsertPerfRecord(PerfRecord &theRecord) {
    if (theRecord.mStart) {
        ...
        if (++anItr->mStartCount == 1) anItr->mStartTime = theRecord.mTime;
    } else {
        ...
        if (anItr != gPerfInfoSet.end()) {
            if (--anItr->mStartCount == 0) {
                ...
                if (aDuration > anItr->mLongestCall) anItr->mLongestCall = (double)aDuration;
            }
        }
    }
}
...
static inline void PushPerfRecord(PerfRecord theRecord) {
    if (gPerfRecordTop >= (int)gPerfRecordVector.size()) gPerfRecordVector.push_back(theRecord);
    else gPerfRecordVector[gPerfRecordTop] = theRecord;
...
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
bool SexyPerf::IsPerfOn() { return gPerfOn; }
...
void SexyPerf::BeginPerf(bool measurePerfOverhead) {
    ...
    if (!measurePerfOverhead) gPerfOn = true;
...
void SexyPerf::StopTiming(const char *theName) {
    if (gPerfOn) {
        ...
        if (--gStartCount == 0) CollatePerfRecords();
    }
}
...
std::string SexyPerf::GetResults() {
    ...
    for (PerfInfoSet::iterator anItr = gPerfInfoSet.begin(); anItr != gPerfInfoSet.end(); ++anItr) {
        const PerfInfo &anInfo = *anItr;
        sprintf(
            aBuf, "%s (%d calls, %%%.2f time): %.2f (%.2f avg, %.2f longest)\n", anInfo.mPerfName, anInfo.mCallCount,
            anInfo.mMillisecondDuration / gDuration * 100, anInfo.mMillisecondDuration,
