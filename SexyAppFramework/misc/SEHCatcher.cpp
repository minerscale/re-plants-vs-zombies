...
std::wstring SEHCatcher::mSubmitMessage =
    L"Please help out by providing as much information as you can about this crash.";
...
SEHCatcher::~SEHCatcher() { SetUnhandledExceptionFilter(mPreviousFilter); }

long __stdcall SEHCatcher::UnhandledExceptionFilter(LPEXCEPTION_POINTERS lpExceptPtr) {
    if (mApp != NULL) mApp->SEHOccured();

    DoHandleDebugEvent(lpExceptPtr);

    if (!mDebugError) SetErrorMode(SEM_NOGPFAULTERRORBOX);
...
bool SEHCatcher::LoadImageHelp() {
    ...
    if (!mSymInitialize(GetCurrentProcess(), pPath, TRUE)) return false;

    return true;
}

void SEHCatcher::UnloadImageHelp() {
    if (mImageHelpLib != NULL) FreeLibrary(mImageHelpLib);
}
...
static bool StrToLongHex(const std::string &aString, DWORD *theValue) {
    *theValue = 0;
    ...
    for (int i = 0; i < (int)aString.length(); i++) {
        ...
        int aValue;
        if ((aChar >= '0') && (aChar <= '9')) aValue = aChar - '0';
        else if ((aChar >= 'A') && (aChar <= 'F')) aValue = (aChar - 'A') + 10;
        else if ((aChar >= 'a') && (aChar <= 'f')) aValue = (aChar - 'a') + 10;
        else return false;
            ...
            if (StrToLongHex(aFindPreStr, &aFindPreVal) && StrToLongHex(aFindPostStr, &aFindPostVal)) {
                ...
                if (aBestDist != -1) {
                    ...
                    if (mUnDecorateSymbolName(
                            aBestName.c_str(), aSymbolName, 4096,
                            UNDNAME_NO_ALLOCATION_MODEL | UNDNAME_NO_ACCESS_SPECIFIERS | UNDNAME_NO_THROW_SIGNATURES |
...
void SEHCatcher::DoHandleDebugEvent(LPEXCEPTION_POINTERS lpEP) {
    ...
    aDebugDump += aBuffer;
    sprintf(
        aBuffer, "CS:%04X SS:%04X DS:%04X ES:%04X FS:%04X GS:%04X\r\n", lpEP->ContextRecord->SegCs,
        lpEP->ContextRecord->SegSs, lpEP->ContextRecord->SegDs, lpEP->ContextRecord->SegEs, lpEP->ContextRecord->SegFs,
    ...
    if (hasImageHelp) GetSymbolsFromMapFile(aDebugDump);
    ...
    if (mShowUI) ShowErrorDialog(anErrorTitle, aDebugDump);
#endif
...
std::string SEHCatcher::ImageHelpWalk(PCONTEXT theContext, int theSkipCount) {
    ...
    for (;;) {
        if (!mStackWalk(
                IMAGE_FILE_MACHINE_I386, GetCurrentProcess(), GetCurrentThread(), &sf, NULL /*theContext*/, NULL,
        ...
        if ((sf.AddrFrame.Offset == 0) || (sf.AddrPC.Offset == 0)) break;
        ...
        if (mSymGetSymFromAddr(GetCurrentProcess(), sf.AddrPC.Offset, &symDisplacement, pSymbol)) {
            char aUDName[256];
            mUnDecorateSymbolName(
                pSymbol->Name, aUDName, 256,
                UNDNAME_NO_ALLOCATION_MODEL | UNDNAME_NO_ALLOCATION_LANGUAGE | UNDNAME_NO_MS_THISTYPE |
                    UNDNAME_NO_ACCESS_SPECIFIERS | UNDNAME_NO_THISTYPE | UNDNAME_NO_MEMBER_TYPE |
...
static LRESULT CALLBACK SEHProgressWindowProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_CLOSE: SEHCatcher::mSubmitReportTransfer.Abort(); return 0;
    case WM_TIMER: {
        if (wParam == 0) {
            int aResult = SEHCatcher::mSubmitReportTransfer.GetResultCode();

            if ((aResult != HTTPTransfer::RESULT_NOT_COMPLETED) && (aResult != HTTPTransfer::RESULT_NOT_STARTED)) {
                if ((aResult != HTTPTransfer::RESULT_DONE) && (aResult != HTTPTransfer::RESULT_ABORTED)) {
                    SEHCatcher::mSubmitReportTransfer.Reset();

                    if (::MessageBoxW(
                            hWnd, SEHCatcher::mSubmitErrorMessage.c_str(), L"Connection Failed",
...
static void CreateProgressWindow() {
    ...
    HWND aHWnd = CreateWindowA(
        ...
        aRect.bottom - aRect.top, NULL, NULL, gHInstance, 0
    );
    ...
    gEditWindow = CreateWindowA(
        "EDIT", "Please Wait",

        WS_VISIBLE | WS_CHILD | ES_READONLY, 24, 10, 240 - 8 - 8, 24, aHWnd, NULL, gHInstance, 0
    );
    if (!gUseDefaultFonts) SendMessage(gEditWindow, WM_SETFONT, (WPARAM)SEHCatcher::mBoldFont, 0);

    SEHCatcher::mNoButtonWindow = CreateWindowA(
        "BUTTON", "Abort", WS_VISIBLE | WS_CHILD | BS_DEFPUSHBUTTON | BS_PUSHBUTTON, (240 - 96) / 2, 64 - 22 - 6, 96,
        22, aHWnd, NULL, gHInstance, 0
    );
    if (!gUseDefaultFonts) SendMessage(SEHCatcher::mNoButtonWindow, WM_SETFONT, (WPARAM)SEHCatcher::mDialogFont, 0);
...
LRESULT CALLBACK SEHCatcher::SEHWindowProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_CLOSE: mExiting = true; return 0;
    }
...
void SEHCatcher::SubmitReportThread(void *theArg) {
    ...
    if (mHasDemoFile) {
        ...
        aContent.insert(
            aContent.end(), (char *)aBuffer.GetDataPtr(), (char *)aBuffer.GetDataPtr() + aBuffer.GetDataLen()
        );
...
void SEHCatcher::ShowSubmitInfoDialog() {
    ...
    HWND aLabelWindow = CreateWindowW(
        L"EDIT", mSubmitMessage.c_str(),
    ...
    HFONT aBoldArialFont = CreateFontA(
        aFontHeight, 0, 0, 0, FW_BOLD, 0, 0, false, ANSI_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS,
        DEFAULT_QUALITY, DEFAULT_PITCH | FF_DONTCARE, "Arial"
    );

    if (!gUseDefaultFonts) SendMessage(aLabelWindow, WM_SETFONT, (WPARAM)aBoldArialFont, 0);

    mEditWindow = CreateWindowA(
        "EDIT", "", WS_VISIBLE | WS_CHILD | ES_MULTILINE | WS_BORDER | WS_VSCROLL, 8, 300 - 168 - 24 - 8 - 8,
        400 - 8 - 8, 168, aHWnd, NULL, gHInstance, 0
    );

    aDC = ::GetDC(mEditWindow);
    aFontHeight = -MulDiv(8, 96, 72);
    ::ReleaseDC(mEditWindow, aDC);
    HFONT aCourierNewFont = CreateFontA(
    ...
    if (mApp == NULL) aWindowStyle |= WS_DISABLED;

    mYesButtonWindow = CreateWindowA(
        "BUTTON", "Continue", aWindowStyle, aCurX, 300 - 24 - 8, aButtonWidth, 24, aHWnd, NULL, gHInstance, 0
    );
    if (!gUseDefaultFonts) SendMessage(mYesButtonWindow, WM_SETFONT, (WPARAM)aBoldArialFont, 0);

    aCurX += aButtonWidth + 8;

    mNoButtonWindow = CreateWindowA(
        "BUTTON", "Abort", WS_VISIBLE | WS_CHILD | BS_DEFPUSHBUTTON | BS_PUSHBUTTON, aCurX, 300 - 24 - 8, aButtonWidth,
        24, aHWnd, NULL, gHInstance, 0
    );
    if (!gUseDefaultFonts) SendMessage(mNoButtonWindow, WM_SETFONT, (WPARAM)aBoldArialFont, 0);
...
void SEHCatcher::ShowErrorDialog(const std::string &theErrorTitle, const std::string &theErrorText) {
    ...
    mDialogFont = ::CreateFontA(
        aHeight, 0, 0, 0, FW_NORMAL, FALSE, FALSE, false, ANSI_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS,
        DEFAULT_QUALITY, DEFAULT_PITCH | FF_DONTCARE, "Tahoma"
    );

    aHeight = -MulDiv(10, 96, 72);
    mBoldFont = ::CreateFontA(
        aHeight, 0, 0, 0, FW_BOLD, FALSE, FALSE, false, ANSI_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS,
        DEFAULT_QUALITY, DEFAULT_PITCH | FF_DONTCARE, "Tahoma"
    );
    ...
    HWND aLabelWindow = CreateWindowW(
        L"EDIT", mCrashMessage.c_str(), WS_VISIBLE | WS_CHILD | ES_MULTILINE | ES_READONLY, 8, 8, 400 - 8 - 8, 84,
        aHWnd, NULL, gHInstance, 0
    );
    ...
    HWND anEditWindow = CreateWindowA(
        "EDIT", theErrorText.c_str(), WS_VISIBLE | WS_CHILD | ES_MULTILINE | WS_BORDER | WS_VSCROLL | ES_READONLY, 8,
    ...
    if (!gUseDefaultFonts) SendMessage(anEditWindow, WM_SETFONT, (WPARAM)aCourierNewFont, 0);
    ...
    if (doDebugButton) {
        ...
        if (!gUseDefaultFonts) SendMessage(mDebugButtonWindow, WM_SETFONT, (WPARAM)aBoldArialFont, 0);

        aCurX += aButtonWidth + 8;
    }

    mNoButtonWindow = CreateWindowA(
        "BUTTON", "Close Now", WS_VISIBLE | WS_CHILD | BS_DEFPUSHBUTTON | BS_PUSHBUTTON, aCurX, 300 - 24 - 8,
        aButtonWidth, 24, aHWnd, NULL, gHInstance, 0
    );
...
std::string SEHCatcher::GetSysInfo() {
    ...
    if (aVersionInfo.dwPlatformId == VER_PLATFORM_WIN32_NT) aDebugDump += "NT ";
    else aDebugDump += "9x ";
