...
SexyStringVector TextWidget::GetLines() { return mLogicalLines; }

void TextWidget::SetLines(SexyStringVector theNewLines) { mLogicalLines = theNewLines; }
...
void TextWidget::DrawColorString(Graphics *g, const SexyString &theString, int x, int y, bool useColors) {
    ...
    for (int i = 0; i < (int)theString.length(); i++) {
        if (theString[i] == 0x100) {
            if (aCurString.length() > 0) g->DrawString(aCurString, x + aWidth, y);

            aWidth += g->GetFont()->StringWidth(aCurString);
            aCurString = _S("");
            if (useColors) g->SetColor(Color(theString[i + 1], theString[i + 2], theString[i + 3]));
            i += 3;
        } else aCurString += theString[i];
    }

    if (aCurString.length() > 0) g->DrawString(aCurString, x + aWidth, y);
}

void TextWidget::DrawColorStringHilited(
    Graphics *g, const SexyString &theString, int x, int y, int theStartPos, int theEndPos
) {
...
int TextWidget::GetStringIndex(const SexyString &theString, int thePixel) {
    ...
    for (int i = 0; i < (int)theString.length(); i++) {
        ...
        if (thePixel >= (aLoLen + aHiLen) / 2) aPos = i + 1;
    }
...
void TextWidget::Resize(int theX, int theY, int theWidth, int theHeight) {
    ...
    double aPageSize = 1;
    if (mHeight > mFont->GetHeight() + 16) aPageSize = (mHeight - 8.0) / mFont->GetHeight();
...
void TextWidget::AddToPhysicalLines(int theIdx, const SexyString &theLine) {
    ...
    if (GetColorStringWidth(theLine) <= mWidth - 8) {
        ...
        while (aCurPos < (int)theLine.length()) {
            ...
            if (aSpacePos == -1) aSpacePos = theLine.length();
            ...
            if (GetColorStringWidth(aNewString) > mWidth - 8) {
                ...
                aCurString = _S("  ") + SexyChar(0xFF) + (SexyChar)aColor.mRed + (SexyChar)aColor.mGreen +
                             (SexyChar)aColor.mBlue + theLine.substr(aNextCheckPos, aSpacePos - aNextCheckPos);
...
void TextWidget::AddLine(const SexyString &theLine) {
    ...
    if (aLine.compare(_S("")) == 0) aLine = _S(" ");
    ...
    if (atBottom) mScrollbar->GoToBottom();
...
void TextWidget::GetSelectedIndices(int theLineIdx, int *theIndices) {
    ...
    for (int aPosIdx = 0; aPosIdx < 2; aPosIdx++) {
        ...
        if (mHiliteArea[aPosIdx][1] < theLineIdx) aVal = 0;
        else if (mHiliteArea[aPosIdx][1] == theLineIdx) aVal = mHiliteArea[aPosIdx][0];
        else aVal = mPhysicalLines[theLineIdx].length();
