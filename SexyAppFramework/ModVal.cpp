...
static const char *
FindFileInStringTable(const std::string &theSearch, const char *theMem, DWORD theLength, const char *theStartPos) {
    const char *aFind = NULL;
    try {
        aFind = std::search(theStartPos, theMem + theLength, theSearch.c_str(), theSearch.c_str() + theSearch.length());
        if (aFind >= theMem + theLength) return NULL;
        else return aFind;
    } catch (...) {
        return NULL;
    }
...
static bool ParseModValString(std::string &theStr, int *theCounter = NULL, int *theLineNum = NULL) {
    ...
    while (aPos > 0) {
        if (!foundComma && theStr[aPos] == ',') {
            aPos--;
            foundComma = true;
        } else if (isdigit(theStr[aPos])) aPos--;
        else break;
    }
...
static void FindModValsInMemory() {
    ...
    if (aMem != NULL) {
        if (FindModValsInMemoryHelper(aMem, aMemLength)) aFound++;
    }
}
...
static ModStorage *CreateFileMods(const char *theFileName) {
    if (gSampleString == NULL) gSampleString = theFileName;
...
int Sexy::ModVal(int theAreaNum, const char *theFileName, int theInt) {
    ...
    if (aModStorage->mChanged) return aModStorage->mInt;
    else return theInt;
}
...
double Sexy::ModVal(int theAreaNum, const char *theFileName, double theDouble) {
    ...
    ModStorage *aModStorage = *(ModStorage **)(theFileName + 1);
    if (aModStorage->mChanged) return aModStorage->mDouble;
...
const char *Sexy::ModVal(int theAreaNum, const char *theFileName, const char *theStr) {
    ...
    ModStorage *aModStorage = *(ModStorage **)(theFileName + 1);
    if (aModStorage->mChanged) return aModStorage->mString.c_str();
    else return theStr;
}

void Sexy::AddModValEnum(const std::string &theEnumName, int theVal) { gStringToIntMap[theEnumName] = theVal; }
...
static bool ModStringToString(const char *theString, std::string &theStrVal) {
    ...
    while (true) {
        ...
        } else if (theString[i] == '\0') return false;
        else aStr += theString[i++];
    }
...
bool Sexy::ReparseModValues() {
    ...
    for (aFileModItr = aMap.begin(); aFileModItr != aMap.end(); ++aFileModItr) {
        ...
        if (aFileList.length() > 0) aFileList += "\r\n  ";
        aFileList += aFileName;
        ...
        if (aStream.is_open()) {
                ...
                while (aString[aCharIdx] != 0) {
                        ...
                        if (aLastChar == '/') {
                            ...
                            while (true) {
                                ...
                                if (aChar == 0) // line continuation
                                {
                                    ...
                                    char aStr[512];
                                    sprintf(
                                        aStr, "ERROR in %s on line %d: Error parsing c++ comment", aFileName.c_str(),
                                        aLineNum
                                    );
                                    MessageBoxA(NULL, aStr, "MODVAL ERROR", MB_OK | MB_ICONERROR);
                                    return false;
                    ...
                    } else if (aChar == '*') // skip C comments
                    {
                        ...
                        if ((aCharIdx >= 2) && (aString[aCharIdx - 1] == 'M') &&
                            ...
                            theAreaNum = 0;
                        } else if ((aCharIdx >= 3) && (aString[aCharIdx - 1] >= '1') && (aString[aCharIdx - 1] <= '9') && (aString[aCharIdx - 2] == 'M') && (!isalpha((unsigned char)aString[aCharIdx - 3]))) {
            ...
            MessageBoxA(
                NULL, (std::string("ERROR: Unable to open ") + aFileName + " for reparsing.").c_str(), "MODVAL ERROR!",
                MB_OK | MB_ICONERROR
            );
            return false;
        }
    ...
    if (!hasNewFiles) {
        if (aFileList.length() == 0) aFileList = "none";
        MessageBoxA(
            NULL, (std::string("WARNING: No file changes detected.  Files parsed: \r\n  ") + aFileList).c_str(),
            "MODVAL WARNING!", MB_OK | MB_ICONWARNING
        );
        return false;
    }
