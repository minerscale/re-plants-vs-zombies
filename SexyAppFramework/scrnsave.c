    ...
    if (vi.dwPlatformId == VER_PLATFORM_WIN32_WINDOWS) {
        ...
        if (RegOpenKey(HKEY_CURRENT_USER, REGSTR_PATH_SCREENSAVE, &hKey) == ERROR_SUCCESS) {
            ...
            if (check_pwd && res == ERROR_SUCCESS) {
                ...
                if (hPwdLib) VerifyScreenSavePwd = (VERIFYPWDPROC)GetProcAddress(hPwdLib, szVerifyPassword);
            }
            RegCloseKey(hKey);
        }
    }
...
static void LaunchConfig(void) {
    ...
    /* display configure dialog */
    DialogBox(
        hMainInstance, MAKEINTRESOURCE(DLG_SCRNSAVECONFIGURE), GetForegroundWindow(),
...
static int LaunchScreenSaver(HWND hParent) {
    ...
    if (w95 && !fChildPreview) SystemParametersInfo(SPI_SCREENSAVERRUNNING, TRUE, &foo, 0);
    ...
    hMainWindow = CreateWindowEx(
        hParent ? 0 : WS_EX_TOPMOST, CLASS_SCRNSAVE, TEXT("SCREENSAVER"), style, 0, 0, rc.right, rc.bottom, hParent,
        NULL, hMainInstance, NULL
    );
...
restore:
    /* restore system */
    if (w95 && !fChildPreview) SystemParametersInfo(SPI_SCREENSAVERRUNNING, FALSE, &foo, 0);
    FreeLibrary(hPwdLib);
    return msg.wParam;
}
...
static LRESULT WINAPI SysScreenSaverProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    switch (msg) {
    case WM_CREATE:
        if (!fChildPreview) SetCursor(NULL);
    ...
    case WM_TIMER:
        ...
        break;
    case WM_PAINT:
        if (closing) return DefWindowProc(hWnd, msg, wParam, lParam);
        break;
    case WM_SYSCOMMAND:
        if (!fChildPreview) switch (wParam) {
            case SC_CLOSE:
            case SC_SCREENSAVE:
            case SC_NEXTWINDOW:
            case SC_PREVWINDOW: return FALSE;
            }
        break;
    case WM_MOUSEMOVE:
    case WM_LBUTTONDOWN:
...
LONG WINAPI DefScreenSaverProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    if (fChildPreview || closing) return DefWindowProc(hWnd, msg, wParam, lParam);
    ...
    case SCRM_VERIFYPW:
        ...
        if (VerifyScreenSavePwd) return VerifyScreenSavePwd(hWnd);
        else return TRUE;
    case WM_SETCURSOR:
        if (checking_pwd) break;
        SetCursor(NULL);
        return TRUE;
    case WM_NCACTIVATE:
    case WM_ACTIVATE:
    ...
    case WM_MOUSEMOVE: {
        POINT pt;
        GetCursorPos(&pt);
        if (pt.x == pt_orig.x && pt.y == pt_orig.y) break;
    }
    case WM_LBUTTONDOWN:
    case WM_RBUTTONDOWN:
    case WM_MBUTTONDOWN:
    ...
    case WM_SYSKEYDOWN:
        ...
        if (!checking_pwd) PostMessage(hWnd, WM_CLOSE, 0, 0);
        break;
    }
...
static void TerminateScreenSaver(HWND hWnd) {
    ...
    if (checking_pwd || closing) return;
    ...
    if (closing) {
        ...
        DestroyWindow(hWnd);
    } else GetCursorPos(&pt_orig); /* if not: get new mouse position */
}
...
void WINAPI ScreenSaverChangePassword(HWND hParent) {
    ...
    if (hMpr) {
        ...
        if (ChangePassword) ChangePassword(TEXT("SCRSAVE"), hParent, 0, NULL);
