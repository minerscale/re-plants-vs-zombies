...
DDImage::DDImage(DDInterface *theDDInterface) : MemoryImage(theDDInterface->mApp) {
    mDDInterface = theDDInterface;
    Init();
}

DDImage::DDImage() : MemoryImage(gSexyAppBase) {
...
bool DDImage::Check3D(Image *theImage) {
    ...
    if (anImage != NULL) return Check3D(anImage);
    else return false;
}
...
bool DDImage::GenerateDDSurface() {
    ...
    if (mHasAlpha) return false;

    mWantDDSurface = true;

    // Force into non-palletized mode for this
    if (mColorTable != NULL) GetBits();
                ...
                if (mFirstPixelTrans) {
                    ...
                    if (needNewColor) {
                        ...
                        for (i = 0; i < mWidth * mHeight; i++) {
                            ...
                            int anAlpha = val >> 24;
                            if (anAlpha > 0) aUsedColorArray[a16Val / 32] |= (1 << (a16Val % 32));
                        }
                    ...
                    if (mBits != NULL) {
                        ...
                        for (int aRow = 0; aRow < mHeight; aRow++) {
                            for (int aCol = 0; aCol < mWidth; aCol++) {
                                ...
                                int anAlpha = val >> 24;
                                if (anAlpha < 255) *(aDestPtr++) = aTransColor;
                                else *(aDestPtr++) = *a16SrcPtr;
                ...
                if (mFirstPixelTrans) {
                    ...
                    if (mBits != NULL) {
                        ...
                        for (int aRow = 0; aRow < mHeight; aRow++) {
                            for (int aCol = 0; aCol < mWidth; aCol++) {
                                ...
                                *(aDestPtr++) = (((val >> rRightShift) << rLeftShift) & rMask) |
                                                (((val >> gRightShift) << gLeftShift) & gMask) |
                                                (((val >> bRightShift) << bLeftShift) & bMask);
                            }
                    ...
                    if (needNewColor) {
                        ...
                        for (i = 0; i < mWidth * mHeight; i++) {
                            ...
                            if (anAlpha > 0) aUsedColorArray[(val & 0xFFFFFF) / 32] |= (1 << (val % 32));
                        }
...
void DDImage::DeleteDDSurface() {
    if (mSurface != NULL) {
        if ((mColorTable == NULL) && (mBits == NULL) && (mD3DData == NULL)) GetBits();
...
void DDImage::RehupFirstPixelTrans() {
    if (!GenerateDDSurface()) return;

    if ((mNoLock) || (!mHasTrans) || (!mFirstPixelTrans)) return;
...
LPDIRECTDRAWSURFACE DDImage::GetSurface() {
    ...
    if (mSurface == NULL) GenerateDDSurface();
...
void DDImage::FillRect(const Rect &theRect, const Color &theColor, int theDrawMode) {
    ...
    switch (theDrawMode) {
    case Graphics::DRAWMODE_NORMAL:   NormalFillRect(theRect, theColor); break;
    case Graphics::DRAWMODE_ADDITIVE: AdditiveFillRect(theRect, theColor); break;
    }

    DeleteAllNonSurfaceData();
}

void DDImage::NormalDrawLine(
    ...
    if (mLockedSurfaceDesc.ddpfPixelFormat.dwRGBBitCount == 16) {
        if (theColor.mAlpha == 255) {
            ushort aColor = (ushort)(((((theColor.mRed * aRMask) + aRRoundAdd) >> 8) & aRMask) |
            ...
            if (fabs(dv) < fabs(dh)) {
                ...
                ushort *aDestPixels =
                    ((ushort *)mLockedSurfaceDesc.lpSurface) + ((int)theStartY * aRowWidth) + (int)theStartX;
                *aDestPixels = aColor;
                aDestPixels++;
                ...
                while (aCurX <= theEndX) {
                    if (G > 0) {
                        ...
                        if (aCurX < aMinX || aCurY < aMinY || aCurX > aMaxX || aCurY > aMaxY) break;
                    } else G += DeltaG2;
                ...
                ushort *aDestPixels = ((ushort *)mLockedSurfaceDesc.lpSurface) +
                                      ((int)theStartY * mLockedSurfaceDesc.lPitch / 2) + (int)theStartX;
                *aDestPixels = aColor;
                aDestPixels += aRowAdd;
            ...
            ushort src = ((((((theColor.mRed * theColor.mAlpha + 0x80) >> 8) * aRMask) + aRRoundAdd) >> 8) & aRMask) +
        ...
        if (theColor.mAlpha == 255) {
            ulong aColor = ((((theColor.mRed * aRMask) + aRRoundAdd) >> 8) & aRMask) |
                           ((((theColor.mGreen * aGMask) + aGRoundAdd) >> 8) & aGMask) |
                           ((((theColor.mBlue * aBMask) + aBRoundAdd) >> 8) & aBMask);
            ...
            if (abs(dv) < abs(dh)) {
                ...
                ulong *aDestPixels =
                    ((ulong *)mLockedSurfaceDesc.lpSurface) + ((int)theStartY * aRowWidth) + (int)theStartX;
                *aDestPixels = aColor;
                aDestPixels++;
            ...
            ulong src = ((((((theColor.mRed * theColor.mAlpha + 0x80) >> 8) * aRMask) + aRRoundAdd) >> 8) & aRMask) +
                        ((((((theColor.mGreen * theColor.mAlpha + 0x80) >> 8) * aGMask) + aGRoundAdd) >> 8) & aGMask) +
                        ((((((theColor.mBlue * theColor.mAlpha + 0x80) >> 8) * aBMask) + aBRoundAdd) >> 8) & aBMask);
            int oma = 256 - theColor.mAlpha;
...
void DDImage::AdditiveDrawLine(
    ...
    if (mLockedSurfaceDesc.ddpfPixelFormat.dwRGBBitCount == 16) {
        ...
        if (abs(dv) < abs(dh)) {
            ...
            *(aDestPixels++) = (r << aRedShift) | (g << aGreenShift) | (b << aBlueShift);
        ...
        if (abs(dv) < abs(dh)) {
            ...
            *(aDestPixels++) = (r << aRedShift) | (g << aGreenShift) | (b << aBlueShift);
            ...
            ulong *aDestPixels = ((ulong *)mLockedSurfaceDesc.lpSurface) +
                                 ((int)theStartY * mLockedSurfaceDesc.lPitch / 4) + (int)theStartX;
...
void DDImage::DrawLine(
    ...
    switch (theDrawMode) {
    case Graphics::DRAWMODE_NORMAL:   NormalDrawLine(theStartX, theStartY, theEndX, theEndY, theColor); break;
    case Graphics::DRAWMODE_ADDITIVE: AdditiveDrawLine(theStartX, theStartY, theEndX, theEndY, theColor); break;
    }

    DeleteAllNonSurfaceData();
}

void DDImage::NormalDrawLineAA(
    double theStartX, double theStartY, double theEndX, double theEndY, const Color &theColor
) {
    LPDIRECTDRAWSURFACE aSurface = GetSurface();
    (void)aSurface; // unused
    ...
    ulong aGMask = mLockedSurfaceDesc.ddpfPixelFormat.dwGBitMask;
    ulong aBMask = mLockedSurfaceDesc.ddpfPixelFormat.dwBBitMask;
    ulong color = (((theColor.mRed * aRMask) >> 8) & aRMask) | (((theColor.mGreen * aGMask) >> 8) & aGMask) |
...
#define BLEND_PIXEL(p)                                                                                                 \
    *(p) = ((((color & 0xFF00FF) * a + (dest & 0xFF00FF) * oma) >> 8) & 0xFF00FF) |                                    \
...
#define CALC_WEIGHT_A(w) (((w) * (theColor.mAlpha + 1)) >> 8)
#define BLEND_PIXEL(p)                                                                                                 \
    *(p) = ((((color & aRMask) * a + (dest & aRMask) * oma) >> 8) & aRMask) |                                          \
...
ulong *DDImage::GetBits() {
    if (mBits == NULL) {
        if (mSurface == NULL) return MemoryImage::GetBits();
...
void DDImage::NormalFillRect(const Rect &theRect, const Color &theColor) {
    ...
    if (mLockedSurfaceDesc.ddpfPixelFormat.dwRGBBitCount == 16) {
        if (theColor.mAlpha == 255) {
            ushort aColor = ((((theColor.mRed * aRMask) + aRRoundAdd) >> 8) & aRMask) |
                            ((((theColor.mGreen * aGMask) + aGRoundAdd) >> 8) & aGMask) |
                            ((((theColor.mBlue * aBMask) + aBRoundAdd) >> 8) & aBMask);

            ushort *aDestPixelsRow =
                ((ushort *)mLockedSurfaceDesc.lpSurface) + (theRect.mY * mLockedSurfaceDesc.lPitch / 2) + theRect.mX;
        ...
        if (theColor.mAlpha == 255) {
            ulong aColor = ((((theColor.mRed * aRMask)) >> 8) & aRMask) |
                           ((((theColor.mGreen * aGMask)) >> 8) & aGMask) |
                           ((((theColor.mBlue * aBMask)) >> 8) & aBMask);

            ulong *aDestPixelsRow =
                ((ulong *)mLockedSurfaceDesc.lpSurface) + (theRect.mY * mLockedSurfaceDesc.lPitch / 4) + theRect.mX;
            ...
            ulong src = ((((((theColor.mRed * theColor.mAlpha + 0x7F) >> 8) * aRMask) + aRRoundAdd) >> 8) & aRMask) +
            ...
            for (int y = 0; y < theRect.mHeight; y++) {
                ...
                for (int x = 0; x < theRect.mWidth; x++) {
                    ...
                    *(aDestPixels++) = src + (((((dest & aRMask) * oma)) >> 8) & aRMask) +
...
void DDImage::NormalBlt(Image *theImage, int theX, int theY, const Rect &theSrcRect, const Color &theColor) {
    ...
    if (aMemoryImage != NULL) {
        ...
        // TODO:
        if ((aMemoryImage->mIsVolatile) && ((aDDImage == NULL) || (aDDImage->mSurface == NULL)) && (!mNoLock) &&
...
#undef PEEK_SRC_COLOR
            ...
            if (mLockCount > 0) mSurface->Unlock(NULL);
...
void DDImage::AdditiveBltMirror(
    Image *theImage, int theX, int theY, const Rect &theSrcRectOrig, const Color &theColor
) {
    theImage->mDrawn = true;
...
void DDImage::Blt(Image *theImage, int theX, int theY, const Rect &theSrcRect, const Color &theColor, int theDrawMode) {
    ...
    if (Check3D(this)) {
        ...
        if ((aDDImage != NULL) && (aDDImage->mSurface != NULL) &&
            (mDDInterface->mD3DInterface->mTransformStack.empty()) && (theDrawMode == Graphics::DRAWMODE_NORMAL) &&
    ...
    if ((mDrawToBits) || (mHasAlpha) || ((mHasTrans) && (!mFirstPixelTrans)) ||
        (mDDInterface->mIs3D && this != mDDInterface->mNewCursorAreaImage && this != mDDInterface->mOldCursorAreaImage
        )) {
    ...
    switch (theDrawMode) {
    case Graphics::DRAWMODE_NORMAL:   NormalBlt(theImage, theX, theY, theSrcRect, theColor); break;
    case Graphics::DRAWMODE_ADDITIVE: AdditiveBlt(theImage, theX, theY, theSrcRect, theColor); break;
    }

    DeleteAllNonSurfaceData();
}

void DDImage::BltMirror(
    ...
    switch (theDrawMode) {
    case Graphics::DRAWMODE_NORMAL:   NormalBltMirror(theImage, theX, theY, theSrcRect, theColor); break;
    case Graphics::DRAWMODE_ADDITIVE: AdditiveBltMirror(theImage, theX, theY, theSrcRect, theColor); break;
    }
    ...
    if (Check3D(this)) {
        ...
        if (anIntersect.mWidth != aDestRect.mWidth || anIntersect.mHeight != aDestRect.mHeight) {
            if (anIntersect.mWidth != 0 && anIntersect.mHeight != 0)
                mDDInterface->mD3DInterface->BltClipF(
                    theImage, theX, theY, theSrcRect, &theClipRect, theColor, theDrawMode
                );
        } else mDDInterface->mD3DInterface->Blt(theImage, theX, theY, theSrcRect, theColor, theDrawMode, true);

        return;
    } else BltRotated(theImage, theX, theY, theSrcRect, theClipRect, theColor, theDrawMode, 0, 0, 0);
}

// 0x57A610
void DDImage::BltRotated(
    Image *theImage, float theX, float theY, const Rect &theSrcRect, const Rect &theClipRect, const Color &theColor,
    ...
    if (Check3D(this)) {
        mDDInterface->mD3DInterface->BltRotated(
            theImage, theX, theY, &theClipRect, theColor, theDrawMode, theRot, theRotCenterX, theRotCenterY, theSrcRect
        );
        return;
    }
...
#undef READ_COLOR
                ...
                ulong *aColorTable = aMemoryImage->mColorTable;
                uchar *aSrcBits =
                    aMemoryImage->mColorIndices + theSrcRect.mX + theSrcRect.mY * aMemoryImage->GetWidth();
...
void DDImage::StretchBlt(
    Image *theImage, const Rect &theDestRectOrig, const Rect &theSrcRectOrig, const Rect &theClipRect,
    ...
    if (Check3D(this)) {
        mDDInterface->mD3DInterface->StretchBlt(
            theImage, theDestRectOrig, theSrcRectOrig, &theClipRect, theColor, theDrawMode, fastStretch
        );
        return;
    }
    ...
    Rect theDestRect;
    FRect theSrcRect;
    if (!StretchBltClipHelper(theSrcRectOrig, theClipRect, theDestRectOrig, theSrcRect, theDestRect)) return;
    ...
    if (fastStretch) {
        if ((aSrcDDImage != NULL) && (theColor == Color::White) && (theDrawMode == Graphics::DRAWMODE_NORMAL) &&
            ...
            if (aSrcDDImage->mHasTrans) aFlags |= DDBLT_KEYSRC;

            RECT aDestRect = {
                theDestRect.mX, theDestRect.mY, theDestRect.mX + theDestRect.mWidth,
...
#undef _PLUSPLUS
        ...
        if (fastStretch) aTempImage.FastStretchBlt(theImage, aTempRect, theSrcRect, theColor, 0);
        else aTempImage.SlowStretchBlt(theImage, aTempRect, theSrcRect, theColor, 0);
...
void DDImage::StretchBltMirror(
    ...
    if (Check3D(this)) {
        mDDInterface->mD3DInterface->StretchBlt(
            theImage, theDestRectOrig, theSrcRectOrig, &theClipRect, theColor, theDrawMode, fastStretch, true
        );
        return;
    }
    ...
    if (!StretchBltMirrorClipHelper(theSrcRectOrig, theClipRect, theDestRectOrig, theSrcRect, theDestRect)) return;
...
void DDImage::BltMatrix(
    ...
    if (Check3D(this)) {
        mDDInterface->mD3DInterface->BltTransformed(
    ...
    if (mLockedSurfaceDesc.ddpfPixelFormat.dwRGBBitCount == 32) aPixelFormat = 0x888;
        ...
        aPixelFormat = 0x555;
    else DBG_ASSERTE(FALSE);

    BltMatrixHelper(
        theImage, x, y, theMatrix, theClipRect, theColor, theDrawMode, theSrcRect, mLockedSurfaceDesc.lpSurface,
        mLockedSurfaceDesc.lPitch, aPixelFormat, blend
    );
...
void DDImage::BltTrianglesTex(
    ...
    if (Check3D(this)) {
        mDDInterface->mD3DInterface->DrawTrianglesTex(
    ...
    BltTrianglesTexHelper(
        theTexture, theVertices, theNumTriangles, theClipRect, theColor, theDrawMode, mLockedSurfaceDesc.lpSurface,
        mLockedSurfaceDesc.lPitch, aPixelFormat, tx, ty, blend
    );
    UnlockSurface();
...
void DDImage::FillScanLinesWithCoverage(
    ...
    if (Check3D(this)) { // ugh!#@$
        ...
        aTempImage.FillScanLinesWithCoverage(
            theSpans, theSpanCount, theColor, theDrawMode, theCoverage, theCoverX - l, theCoverY - t, theCoverWidth,
            theCoverHeight
        );
    ...
    if (mLockedSurfaceDesc.ddpfPixelFormat.dwRGBBitCount == 16) {
        ...
        ushort src = (((theColor.mRed * aRMask) >> 8) & aRMask) | (((theColor.mGreen * aGMask) >> 8) & aGMask) |
                     (((theColor.mBlue * aBMask) >> 8) & aBMask);
        ...
        for (int i = 0; i < theSpanCount; ++i) {
            ...
            for (int w = 0; w < aSpan->mWidth; ++w) {
                ...
                *(aDestPixels++) = ((((dest & aRMask) * oma + (src & aRMask) * a) >> 8) & aRMask) |
        ...
        ulong src = (((theColor.mRed * aRMask) >> 8) & aRMask) | (((theColor.mGreen * aGMask) >> 8) & aGMask) |
                    (((theColor.mBlue * aBMask) >> 8) & aBMask);
