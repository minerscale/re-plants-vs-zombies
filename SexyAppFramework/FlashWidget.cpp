    ...
    if (cwch = ::MultiByteToWideChar(CP_ACP, 0, pSrc, -1, NULL, 0)) // get size minus NULL terminator
    {
        ...
        if (wsOut) {
            if (!::MultiByteToWideChar(CP_ACP, 0, pSrc, -1, wsOut, cwch)) {
                if (ERROR_INSUFFICIENT_BUFFER == ::GetLastError()) return wsOut;
...
struct __declspec(uuid("d27cdb6c-ae6d-11cf-96b8-444553540000")) IShockwaveFlash : IDispatch {
    ...
    virtual HRESULT __stdcall get_ReadyState(
        /*[out,retval]*/ long *thestate
    ) = 0;
    virtual HRESULT __stdcall get_TotalFrames(
        /*[out,retval]*/ long *numframes
    ) = 0;
...
struct __declspec(uuid("a6ef9860-c720-11d0-9337-00a0c90dcaa9")) IDispatchEx : IDispatch {
    ...
    HRESULT RemoteInvokeEx(
        long id, unsigned long lcid, unsigned long dwFlags, DISPPARAMS *pdp, VARIANT *pvarRes, EXCEPINFO *pei,
        IServiceProvider *pspCaller, unsigned int cvarRefArg, unsigned int *rgiRefArg, VARIANT *rgvarRefArg
    );
...
struct __declspec(uuid("d27cdb72-ae6d-11cf-96b8-444553540000")) IFlashObjectInterface : IDispatchEx {};
...
public:
    ...
    HRESULT Init(FlashWidget *theFlashWidget) {
        ...
        if ((mFlashWidget->mFlashInterface->QueryInterface(IID_IConnectionPointContainer, (void **)&aConnectionPoint) ==
             S_OK) &&
        ...
        if (aConnectionPoint != NULL) aConnectionPoint->Release();
    ...
    virtual HRESULT STDMETHODCALLTYPE Invoke(
        DISPID dispIdMember, REFIID riid, LCID lcid, WORD wFlags, ::DISPPARAMS __RPC_FAR *pDispParams,
...
public:
    ...
    virtual ~ControlSite() {
        if (mFlashWidget != NULL) mFlashWidget->mCOMCount--;
    }
    ...
    HRESULT STDMETHODCALLTYPE GetWindowContext(
        ...
        /* [out] */ IOleInPlaceUIWindow __RPC_FAR *__RPC_FAR *ppDoc, /* [out] */ LPRECT lprcPosRect,
        /* [out] */ LPRECT lprcClipRect, /* [out][in] */ LPOLEINPLACEFRAMEINFO lpFrameInfo
    ) {
    ...
    GetDC(/* [in] */ LPCRECT pRect, /* [in] */ DWORD grfFlags, /* [out] */ HDC __RPC_FAR *phDC) {
        return E_INVALIDARG;
    }
    ...
    HRESULT STDMETHODCALLTYPE InvalidateRect(/* [in] */ LPCRECT pRect, /* [in] */ BOOL fErase) {
        if (pRect == NULL) {
            ...
            mFlashWidget->mDirtyRect =
                Rect(pRect->left, pRect->top, pRect->right - pRect->left, pRect->bottom - pRect->top);
            mFlashWidget->mFlashDirty = true;
        } else {
            mFlashWidget->mDirtyRect = mFlashWidget->mDirtyRect.Union(
                Rect(pRect->left, pRect->top, pRect->right - pRect->left, pRect->bottom - pRect->top)
            );
        }
    ...
    HRESULT STDMETHODCALLTYPE
    ScrollRect(/* [in] */ INT dx, /* [in] */ INT dy, /* [in] */ LPCRECT pRectScroll, /* [in] */ LPCRECT pRectClip) {
        return S_OK;
    }
    ...
    HRESULT STDMETHODCALLTYPE OnDefWindowMessage(
        /* [in] */ UINT msg, /* [in] */ WPARAM wParam, /* [in] */ LPARAM lParam, /* [out] */ LRESULT __RPC_FAR *plResult
    ) {
        return S_FALSE;
    }
};
...
FlashWidget::FlashWidget(int theId, FlashListener *theFlashListener) {
    ...
    if (mFlashLibHandle != NULL) {
        ...
        CoCreateInstance(CLSID_ShockwaveFlash, NULL, CLSCTX_INPROC_SERVER, IID_IOleObject, (void **)&mOleObject);
    }
...
FlashWidget::~FlashWidget() {
    ...
    if (mWindowlessObject != NULL) mWindowlessObject->Release();
    if (mFlashInterface != NULL) mFlashInterface->Release();
    ...
    if (mOleObject != NULL) mOleObject->Release();

    if (mControlSite != NULL) mControlSite->Release();

    // Make sure all our COM objects were actually destroyed
    DBG_ASSERTE(mCOMCount == 0);

    if (mFlashLibHandle != NULL) FreeLibrary(mFlashLibHandle);
}
...
double FlashWidget::GetFlashVersion() {
    ...
    if (FAILED(CoCreateInstance(CLSID_ShockwaveFlash, NULL, CLSCTX_INPROC_SERVER, IID_IOleObject, (void **)&anOleObject)
        ))
        return 0.0;

    IShockwaveFlash *aFlashInterface = NULL;
    if (FAILED(anOleObject->QueryInterface(__uuidof(IShockwaveFlash), (LPVOID *)&aFlashInterface))) return 0.0;
...
void FlashWidget::DrawFlashBackground(Graphics *g) {
    ...
    if (mBkgImage != NULL) {
        ...
        else g->DrawImage(mBkgImage, 0, 0);
    }
}
...
bool FlashWidget::StartAnimation(const std::string &theFileName) {
    ...
    if ((mPauseCount == 0) && (mFlashInterface->Play() != S_OK)) return false;
...
void FlashWidget::Pause() {
    ...
    if ((mPauseCount == 1) && (mFlashInterface != NULL) && (mState != STATE_STOPPED)) mFlashInterface->StopPlay();
}
...
bool FlashWidget::IsPlaying() {
    ...
    if (mFlashInterface != NULL) mFlashInterface->IsPlaying(&aBool);
    return aBool != 0;
}

int FlashWidget::GetCurrentFrame() {
    long aCurrentFrame = -1;
    if (mFlashInterface != NULL) mFlashInterface->CurrentFrame(&aCurrentFrame);
    return aCurrentFrame;
}

std::string FlashWidget::GetCurrentLabel(const std::string &theTimeline) {
    BSTR aBStr = L"";
    if (mFlashInterface != NULL) mFlashInterface->TCurrentLabel(_bstr_t(theTimeline.c_str()), &aBStr);
    return (const char *)_bstr_t(aBStr);
}

void FlashWidget::CallFrame(const std::string &theTimeline, int theFrameNum) {
    if (mFlashInterface != NULL) mFlashInterface->TCallFrame(_bstr_t(theTimeline.c_str()), theFrameNum);
}

void FlashWidget::CallLabel(const std::string &theTimeline, const std::string &theLabel) {
    if (mFlashInterface != NULL) mFlashInterface->TCallLabel(_bstr_t(theTimeline.c_str()), _bstr_t(theLabel.c_str()));
}

std::string FlashWidget::GetVariable(const std::string &theName) {
    BSTR aBStr = L"";
    if (mFlashInterface != NULL) mFlashInterface->GetVariable(_bstr_t(theName.c_str()), &aBStr);
    return (const char *)_bstr_t(aBStr);
}

void FlashWidget::SetVariable(const std::string &theName, const std::string &theValue) {
    if (mFlashInterface != NULL) mFlashInterface->SetVariable(_bstr_t(theName.c_str()), _bstr_t(theValue.c_str()));
}
...
void FlashWidget::Update() {
    ...
    if (mState == STATE_PLAYING) {
        ...
        if (!isPlaying) {
            ...
            if ((mFlashListener != NULL) && (mPauseCount == 0)) mFlashListener->FlashAnimEnded(mId);
        }
    }

    if (mFlashDirty) MarkDirty();
}
...
void FlashWidget::Draw(Graphics *g) {
    if (mFlashDirty) {
        ...
        HDC aDC = NULL;
        if (aSurface->GetDC(&aDC) != S_OK) return;
        ...
        if (aViewObject != NULL) {
            ...
            HRGN aRgn = CreateRectRgn(
                mDirtyRect.mX - anAbsPos.mX, mDirtyRect.mY - anAbsPos.mY,
                mDirtyRect.mX + mDirtyRect.mWidth - anAbsPos.mX, mDirtyRect.mY + mDirtyRect.mHeight - anAbsPos.mY
            );
...
void FlashWidget::MouseDown(int x, int y, int theBtnNum, int theClickCount) {
    if (theBtnNum == 0) {
        LRESULT aResult;
        Point anAbsPos = GetAbsPos();
        mWindowlessObject->OnWindowMessage(
            WM_LBUTTONDOWN, MK_LBUTTON, MAKELPARAM(x + anAbsPos.mX, y + anAbsPos.mY), &aResult
        );
        CheckCursor();
    }
}
...
void FlashWidget::MouseDrag(int x, int y) {
    if (mWidgetManager->IsLeftButtonDown()) {
        ...
        Point anAbsPos = GetAbsPos();
        mWindowlessObject->OnWindowMessage(
            WM_MOUSEMOVE, MK_LBUTTON, MAKELPARAM(x + anAbsPos.mX, y + anAbsPos.mY), &aResult
        );
        CheckCursor();
    }
}
