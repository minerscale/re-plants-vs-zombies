...
std::string DDInterface::ResultToString(int theResult) {
    ...
    case RESULT_DD_CREATE_FAIL:     return "RESULT_DD_CREATE_FAIL";
    case RESULT_SURFACE_FAIL:       return "RESULT_SURFACE_FAIL";
    case RESULT_EXCLUSIVE_FAIL:     return "RESULT_EXCLUSIVE_FAIL";
    case RESULT_DISPCHANGE_FAIL:    return "RESULT_DISPCHANGE_FAIL";
    case RESULT_INVALID_COLORDEPTH: return "RESULT_INVALID_COLORDEPTH";
...
DDImage *DDInterface::GetScreenImage() { return mScreenImage; }
...
HRESULT DDInterface::CreateSurface(DDSURFACEDESC2 *theDesc, LPDIRECTDRAWSURFACE *theSurface, void *) {
    ...
    if (mDD7 != NULL) {
        ...
        if (!SUCCEEDED(aResult)) return aResult;
    ...
    // If things are stored blue low, green middle, red high, we can optimize a lot of our software rendering based on
    // bit patterns. This of course does not matter for native data which is already in the correct order (and can be
    // optimized similarly).
    gOptimizeSoftwareDrawing = aDesc.ddpfPixelFormat.dwBBitMask < aDesc.ddpfPixelFormat.dwGBitMask &&
                               aDesc.ddpfPixelFormat.dwGBitMask < aDesc.ddpfPixelFormat.dwRBitMask;
#endif
...
bool DDInterface::Do3DTest(HWND theHWND) {
    if (mD3DTester == NULL) {
        if (mApp->mTest3D || mApp->mAutoEnable3D) {
            ...
            if (mApp->mAutoEnable3D && mD3DTester->Is3DRecommended()) mIs3D = true;

            if (!mD3DTester->Is3DSupported()) mIs3D = false;

            if (mD3DTester->ResultsChanged() && !mD3DTester->Is3DRecommended()) mIs3D = false;
...
int DDInterface::Init(HWND theWindow, bool IsWindowed) {
    ...
    if (gDirectDrawCreateExFunc != NULL) {
        ...
        if (GotDXError(aResult, "DirectDrawCreateEx")) return RESULT_DD_CREATE_FAIL;

        aResult = mDD7->QueryInterface(IID_IDirectDraw, (LPVOID *)&mDD);
        if (GotDXError(aResult, "QueryrInterface IID_IDirectDraw")) return RESULT_DD_CREATE_FAIL;
    } else {
        aResult = gDirectDrawCreateFunc(NULL, &mDD, NULL);
    }

    if (Do3DTest(theWindow)) {
        if (mD3DTester->ResultsChanged()) mApp->Done3dTesting();
    }
    ...
    OutputDebug(
        _S("Application requests %4lu x %4lu [%2d:%2d]\n"), mWidth, mHeight, mAspect.mNumerator, mAspect.mDenominator
    );

    if (GotDXError(aResult, "DirectDrawCreate")) return RESULT_DD_CREATE_FAIL;

    if (IsWindowed) {
        OutputDebug(
            _S("Hack aspect is                   [%2d:%2d]\n"), mApp->mWindowAspect.mNumerator,
            mApp->mWindowAspect.mDenominator
        );
        ...
        if (mApp->mEnableWindowAspect && mAspect < mApp->mWindowAspect) {
            ...
            ::MoveWindow(
                theWindow, std::max((LONG)0, rc.left), std::max((LONG)0, rc.top), rc.right - rc.left,
        ...
        OutputDebug(
            _S("Window is            %4lu x %4lu [%2d:%2d]\n"), mDisplayWidth, mDisplayHeight,
            mDisplayAspect.mNumerator, mDisplayAspect.mDenominator
        );
        ...
        if (mIs3D) aDesc.ddsCaps.dwCaps |= DDSCAPS_3DDEVICE;
        ...
        else if (GotDXError(aResult, "CreateSurface Windowed Primary")) return RESULT_SURFACE_FAIL;

        ZeroMemory(&aDesc, sizeof(aDesc));
        aDesc.dwSize = sizeof(aDesc);
        aDesc.dwFlags = DDSD_CAPS | DDSD_HEIGHT | DDSD_WIDTH;
        aDesc.ddsCaps.dwCaps = DDSCAPS_OFFSCREENPLAIN | DDSCAPS_SYSTEMMEMORY;
        if (mIs3D) aDesc.ddsCaps.dwCaps = DDSCAPS_OFFSCREENPLAIN | DDSCAPS_VIDEOMEMORY | DDSCAPS_3DDEVICE;

        aDesc.dwWidth = mWidth;
        aDesc.dwHeight = mHeight;

        aResult = CreateSurface(&aDesc, &mDrawSurface, NULL);
        if (GotDXError(aResult, "CreateSurface Windowed DrawSurface")) return RESULT_SURFACE_FAIL;
        ...
        if (SUCCEEDED(aResult)) aResult = mPrimarySurface->SetClipper(aClipper);

        aClipper->Release();
    } else {
        OutputDebug(
            _S("Desktop is           %4lu x %4lu [%2d:%2d]\n"), mDesktopWidth, mDesktopHeight,
            mDesktopAspect.mNumerator, mDesktopAspect.mDenominator
        );
        ...
        OutputDebug(
            _S("Display is           %4lu x %4lu [%2d:%2d]\n"), mDisplayWidth, mDisplayHeight,
            mDisplayAspect.mNumerator, mDisplayAspect.mDenominator
        );

        aResult = mDD->SetCooperativeLevel(theWindow, DDSCL_EXCLUSIVE | DDSCL_FULLSCREEN);
        if (GotDXError(aResult, "SetCooperativeLevel FullScreen")) return RESULT_EXCLUSIVE_FAIL;

        aResult = mDD->SetDisplayMode(mDisplayWidth, mDisplayHeight, mFullscreenBits);

        if (GotDXError(aResult, "SetDisplayMode FullScreen")) return RESULT_DISPCHANGE_FAIL;

        ZeroMemory(&aDesc, sizeof(aDesc));
        aDesc.dwSize = sizeof(aDesc);
        aDesc.dwFlags = DDSD_CAPS | DDSD_BACKBUFFERCOUNT;
        aDesc.ddsCaps.dwCaps = DDSCAPS_PRIMARYSURFACE | DDSCAPS_FLIP | DDSCAPS_COMPLEX /* |
                                                                        ,DDSCAPS_FRONTBUFFER*/
            ;
        aDesc.dwBackBufferCount = 1;
        ...
        aResult = CreateSurface(&aDesc, &mPrimarySurface, NULL);
        if (GotDXError(aResult, "CreateSurface FullScreen Primary")) return RESULT_SURFACE_FAIL;
        ...
        if (GotDXError(aResult, "GetAttachedSurface")) return RESULT_SURFACE_FAIL;
        ...
        aResult = CreateSurface(&aDesc, &mDrawSurface, NULL);
        if (GotDXError(aResult, "CreateSurface FullScreen DrawSurface")) return RESULT_SURFACE_FAIL;
        ...
        if (!mApp->mFullScreenPageFlip) mDD->FlipToGDISurface();
    }

    OutputDebug(
        _S("Draw buffer is       %4lu x %4lu [%2d:%2d]\n"), mWidth, mHeight, mAspect.mNumerator, mAspect.mDenominator
    );
    ...
    if (GotDXError(aResult, "CreateSurface OldCursorArea")) return RESULT_SURFACE_FAIL;

    aResult = CreateSurface(&aDesc, &mNewCursorArea, NULL);
    if (GotDXError(aResult, "CreateSurface NewCursorArea")) return RESULT_SURFACE_FAIL;
    ...
    if (mPrimarySurface != NULL) {
        ...
        if ((aDesc.ddpfPixelFormat.dwRGBBitCount != 16) && (aDesc.ddpfPixelFormat.dwRGBBitCount != 32))
            return RESULT_INVALID_COLORDEPTH;
        ...
        {
            if (((mRedMask >> i) & 1) != 0) mRedShift = i;
            if (((mGreenMask >> i) & 1) != 0) mGreenShift = i;
            if (((mBlueMask >> i) & 1) != 0) mBlueShift = i;
        }

        for (i = 0; i < 32; i++) {
            if ((i + mRedShift < 32) && ((mRedMask >> (i + mRedShift)) != 0)) mRedBits = i + 1;
            if ((i + mGreenShift < 32) && ((mGreenMask >> (i + mGreenShift)) != 0)) mGreenBits = i + 1;
            if ((i + mBlueShift < 32) && ((mBlueMask >> (i + mBlueShift)) != 0)) mBlueBits = i + 1;
        }
...
void DDInterface::SetVideoOnlyDraw(bool videoOnlyDraw) {
    ...
    if (mSecondarySurface == NULL) {
        ...
        if (FAILED(aResult)) mVideoOnlyDraw = false;
    }
...
ulong DDInterface::GetColorRef(ulong theRGB) {
    ...
    ulong aColor;
    aColor =
        ((DWORD(((LONGLONG)bRed * (LONGLONG)aDesc.ddpfPixelFormat.dwRBitMask) / 255) & aDesc.ddpfPixelFormat.dwRBitMask
         ) |
         (DWORD(((LONGLONG)bGreen * (LONGLONG)aDesc.ddpfPixelFormat.dwGBitMask) / 255) &
          aDesc.ddpfPixelFormat.dwGBitMask) |
...
void DDInterface::RemoveDDImage(DDImage *theDDImage) {
    ...
    if (anItr != mDDImageSet.end()) mDDImageSet.erase(anItr);
}
    ...
    if (hdcImage != NULL) {
        ...
        if (SUCCEEDED(hr)) {
            ...
            if (theWidth == aDesc.dwWidth && theHeight == aDesc.dwHeight) {
                ...
                StretchBlt(
                    hdc, 0, 0, aDesc.dwWidth, aDesc.dwHeight, hdcImage, theX, theY, theWidth, theHeight, SRCCOPY
                );
            }
    ...
    if (mIsWindowed) {
        ...
        if (!mVideoOnlyDraw) {
            ...
            if ((mApp->mWaitForVSync) && (!mApp->mSoftVSyncWait)) {
                ...
                if (mScanLineFailCount >= 3) scanLineFail = true;
                    ...
                    if (!scanLineFail) {
                        ...
                        for (;;) {
                            if (SUCCEEDED(mDD->GetScanLine(&aScanLine))) {
                                ...
                                if ((aScanLine < aLastScanLine) || (aHalfMarkDist <= 0)) break;
                            }
                        }
            ...
            if ((mApp->mWaitForVSync) && (!mApp->mSoftVSyncWait)) mDD->WaitForVerticalBlank(DDWAITVB_BLOCKBEGIN, 0);
...
void DDInterface::RestoreOldCursorAreaFrom(LPDIRECTDRAWSURFACE theSurface, bool adjust) {
    if ((mHasOldCursorArea) && (mPrimarySurface != NULL)) {
        Rect aSexyScreenRect(
            mCursorX - (mCursorWidth / 2), mCursorY - (mCursorHeight / 2), mCursorWidth, mCursorHeight
        );

        Rect aClippedScreenRect = aSexyScreenRect.Intersection(Rect(0, 0, mWidth, mHeight));

        Rect aSexyLocalRect(
            aClippedScreenRect.mX - aSexyScreenRect.mX, aClippedScreenRect.mY - aSexyScreenRect.mY,
            aClippedScreenRect.mWidth, aClippedScreenRect.mHeight
        );
...
void DDInterface::DrawCursorTo(LPDIRECTDRAWSURFACE theSurface, bool adjust) {
    if ((mCursorImage != NULL) && (mPrimarySurface != NULL)) {
        ...
        if (theSurface == mDrawSurface && !mIs3D) {
            ...
            g.DrawImage(
                mCursorImage, mCursorX - (mCursorWidth / 2) + (mCursorWidth - mCursorImage->mWidth) / 2,
            ...
            aNewCursorAreaG.DrawImage(
                mCursorImage, (mCursorWidth - mCursorImage->mWidth) / 2, (mCursorHeight - mCursorImage->mHeight) / 2
            );

            // From mNewCursorArea to theSurface
            aResult = theSurface->Blt(&aScreenRect, mNewCursorArea, &aLocalRect, DDBLT_WAIT, &aBltFX);
        }
    } else mHasOldCursorArea = false;
}
...
void DDInterface::MoveCursorTo(LPDIRECTDRAWSURFACE theSurface, bool adjust, int theNewCursorX, int theNewCursorY) {
    ...
    if ((mCursorImage != NULL) && (mPrimarySurface != NULL)) {
        ...
        Rect aNewSexyScreenRect(
            theNewCursorX - (mCursorWidth / 2), theNewCursorY - (mCursorHeight / 2), mCursorWidth, mCursorHeight
        );

        Rect aNewClippedScreenRect = aNewSexyScreenRect.Intersection(mPresentationRect);
        Rect aNewSexyLocalRect(
            aNewClippedScreenRect.mX - aNewSexyScreenRect.mX, aNewClippedScreenRect.mY - aNewSexyScreenRect.mY,
            aNewClippedScreenRect.mWidth, aNewClippedScreenRect.mHeight
        );
        if (adjust) {
            POINT aPoint = {0, 0};
            ClientToScreen(mHWnd, &aPoint);
        ...
        if ((aSexySrcAreaRect.mWidth > 0) || (aSexySrcAreaRect.mHeight > 0)) {
            Rect aSexyDestAreaRect(
                aSexySrcAreaRect.mX - aSexyOrigSrcAreaRect.mX, aSexySrcAreaRect.mY - aSexyOrigSrcAreaRect.mY,
            ...
            aOldCursorAreaG.DrawImage(
                mCursorImage, theNewCursorX - mCursorX + (mCursorWidth - mCursorImage->mWidth) / 2,
