...
FModMusicInterface::~FModMusicInterface() { FreeFModDLL(); }
...
FSOUND_SAMPLE *FModMusicInterface::LoadFMODSample(const std::string &theFileName) {
    ...
    if (aLastSlashPos < 0) aLastSlashPos = 0;
...
bool FModMusicInterface::LoadSample(
    int theSongId, const std::string &theIntroFileName, const std::string &theRepeatFileName, bool repeat
) {
    ...
    if ((anIntroSample == NULL) || (aLoopSample == NULL)) return false;
    ...
    if ((anIntroFreq != aLoopFreq) || (anIntroMode != aLoopMode)) return false;

    // Create new sound to combine these two into
    FSOUND_SAMPLE *aSample = gFMod->FSOUND_Sample_Alloc(
        FSOUND_FREE, anIntroLen + aLoopLen, anIntroMode | FSOUND_LOOP_NORMAL, anIntroFreq, 255, 128, 0
    );
    ...
    if (gFMod->FSOUND_Sample_Lock(
            aSample, 0, (anIntroLen + aLoopLen) * aSampleBytes, &aSamplePtr1, &aSamplePtr2, &aSampleLen1, &aSampleLen2
        )) {
        void *aSrcPtr1;
        void *aSrcPtr2;
        uint aSrcLen1;
        ...
        if (gFMod->FSOUND_Sample_Lock(
                anIntroSample, 0, anIntroLen * aSampleBytes, &aSrcPtr1, &aSrcPtr2, &aSrcLen1, &aSrcLen2
            )) {
        ...
        if (gFMod->FSOUND_Sample_Lock(
                aLoopSample, anIntroLen * aSampleBytes, aLoopLen * aSampleBytes, &aSrcPtr1, &aSrcPtr2, &aSrcLen1,
                &aSrcLen2
            )) {
...
void FModMusicInterface::PlayMusic(int theSongId, int theOffset, bool noLoop) {
    ...
    if (anItr != mMusicMap.end()) {
        ...
        if (aMusicInfo->mHMusic != NULL) {
            gFMod->FMUSIC_SetMasterVolume(
                aMusicInfo->mHMusic, (int)(mMasterVolume * aMusicInfo->mVolume * mMaxMusicVolume)
            );
...
void FModMusicInterface::FadeIn(int theSongId, int theOffset, double theSpeed, bool noLoop) {
    ...
    if (anItr != mMusicMap.end()) {
        ...
        if (aMusicInfo->mHMusic != NULL) {
            ...
            if (!gFMod->FMUSIC_IsPlaying(aMusicInfo->mHMusic)) gFMod->FMUSIC_PlaySong(aMusicInfo->mHMusic);

            if (theOffset != -1) gFMod->FMUSIC_SetOrder(aMusicInfo->mHMusic, theOffset);
        } else {
