...
int memory_output_module_probe(const char *name) {
    ...
    if (aByteVector == NULL) return XA_ERROR_INVALID_PARAMETERS;

    return XA_SUCCESS;
}

int memory_output_module_query(XA_OutputModuleQuery *query, unsigned long query_mask) { return XA_SUCCESS; }

int memory_output_new(
    XA_OutputInstance **output, const char *name, XA_OutputModuleClassInfo *class_info, XA_DecoderInfo *decoder
) {
...
int memory_output_delete(XA_OutputInstance *output) { return XA_SUCCESS; }

int memory_output_open(XA_OutputInstance *output) { return XA_SUCCESS; }

int memory_output_close(XA_OutputInstance *output) { return XA_SUCCESS; }

int memory_output_write(
    XA_OutputInstance *output, const void *buffer, unsigned long size, unsigned int bytes_per_sample,
    ...
    aMemoryOutput->mByteVector.insert(
        aMemoryOutput->mByteVector.end(), (const unsigned char *)buffer, (const unsigned char *)buffer + size
    );
