...
void count_filters(png_structp png_ptr, png_row_infop row_info, png_bytep data) {
    if (png_ptr != NULL && row_info != NULL) ++filters_used[*(data - 1)];
}
#endif
...
static void png_default_warning(png_structp png_ptr, png_const_charp message) {
    ...
    if (png_ptr != NULL && png_ptr->error_ptr != NULL) name = png_ptr->error_ptr;
...
extern PNG_EXPORT(png_voidp, png_debug_malloc) PNGARG((png_structp png_ptr, png_uint_32 size));
extern PNG_EXPORT(void, png_debug_free) PNGARG((png_structp png_ptr, png_voidp ptr));
...
png_voidp png_debug_malloc(png_structp png_ptr, png_uint_32 size) {
    ...
    if (size == 0) return (png_voidp)(NULL);
       ...
       buffer and once to get a new free list entry. */
    {
        ...
        if (current_allocation > maximum_allocation) maximum_allocation = current_allocation;
...
void png_debug_free(png_structp png_ptr, png_voidp ptr) {
    if (png_ptr == NULL) fprintf(STDERR, "NULL pointer to png_debug_free.\n");
    if (ptr == 0) {
        ...
        for (;;) {
            ...
            if (pinfo->pointer == ptr) {
                *ppinfo = pinfo->next;
                ...
                if (current_allocation < 0) fprintf(STDERR, "Duplicate free of memory\n");
...
int test_one_file(PNG_CONST char *inname, PNG_CONST char *outname) {
    ...
    read_ptr = png_create_read_struct_2(
        PNG_LIBPNG_VER_STRING, (png_voidp)NULL, (png_error_ptr)NULL, (png_error_ptr)NULL, (png_voidp)NULL,
    ...
    png_set_write_fn(
        write_ptr, (png_voidp)fpout, png_default_write_data,
#if defined(PNG_WRITE_FLUSH_SUPPORTED)
        png_default_flush
    );
#else
    ...
    png_debug(0, "Transferring info struct\n");
    {
        ...
        if (png_get_IHDR(
                read_ptr, read_info_ptr, &width, &height, &bit_depth, &color_type, &interlace_type, &compression_type,
                &filter_type
            )) {
            png_set_IHDR(
                write_ptr, write_info_ptr, width, height, bit_depth,
...
#if defined(PNG_READ_cHRM_SUPPORTED) && defined(PNG_WRITE_cHRM_SUPPORTED)
    {
        ...
        if (png_get_cHRM(
                read_ptr, read_info_ptr, &white_x, &white_y, &red_x, &red_y, &green_x, &green_y, &blue_x, &blue_y
            )) {
...
#if defined(PNG_READ_pCAL_SUPPORTED) && defined(PNG_WRITE_pCAL_SUPPORTED)
    {
        ...
        if (png_get_pCAL(read_ptr, read_info_ptr, &purpose, &X0, &X1, &type, &nparams, &units, &params)) {
    ...
    for (;;) {
        ...
        if (num_in != num_out) {
            ...
            if (wrote_question == 0) {
                fprintf(STDERR, "   Was %s written with the same chunk size (8k),", inname);
...
int main(int argc, char *argv[]) {
    ...
    if (!multiple && argc == 3 + verbose) outname = argv[2 + verbose];
    ...
    if (multiple) {
...
#if defined(PNG_READ_USER_TRANSFORM_SUPPORTED)
                for (k = 0; k < 256; k++)
                    if (filters_used[k]) fprintf(STDERR, " Filter %d was used %lu times\n", k, filters_used[k]);
#endif
#if defined(PNG_TIME_RFC1123_SUPPORTED)
                if (tIME_chunk_present != 0) fprintf(STDERR, " tIME = %s\n", tIME_string);
        ...
        for (i = 0; i < 3; ++i) {
            ...
            if (i == 0 || verbose == 1 || ierror != 0) fprintf(STDERR, "Testing %s:", inname);
            ...
            if (kerror == 0) {
                ...
                if (verbose == 0 && i != 2) fprintf(STDERR, "Testing %s:", inname);
                fprintf(STDERR, " FAIL\n");
                ierror += kerror;
            }
#ifdef PNG_USER_MEM_SUPPORTED
    ...
    if (ierror == 0) fprintf(STDERR, "libpng passes test\n");
    else fprintf(STDERR, "libpng FAILS test\n");
    return (int)(ierror != 0);
}

/* Generate a compiler error if there is an old png.h in the search path. */
void png_check_pngtest_version(version_1_0_5 png_h_is_not_version_1_0_5) {
