...
void png_set_cHRM(
    png_structp png_ptr, png_infop info_ptr, double white_x, double white_y, double red_x, double red_y, double green_x,
...
void png_set_IHDR(
    ...
    } else info_ptr->rowbytes = (info_ptr->width * info_ptr->pixel_depth + 7) >> 3;
}
...
void png_set_pCAL(
    ...
    info_ptr->pcal_params = (png_charpp)png_malloc(png_ptr, (png_uint_32)((nparams + 1) * sizeof(png_charp)));
...
void png_set_text(png_structp png_ptr, png_infop info_ptr, png_textp text_ptr, int num_text) {
    ...
    if (png_ptr == NULL || info_ptr == NULL || num_text == 0) return;
    ...
    if (info_ptr->num_text + num_text > info_ptr->max_text) {
        if (info_ptr->text != NULL) {
            ...
            info_ptr->text = (png_textp)png_malloc(png_ptr, (png_uint_32)(info_ptr->max_text * sizeof(png_text)));
            png_memcpy(info_ptr->text, old_text, (png_size_t)(old_max * sizeof(png_text)));
    ...
    for (i = 0; i < num_text; i++) {
        ...
        if (text_ptr[i].text == NULL) text_ptr[i].text = (png_charp) "";
...
void png_set_tIME(png_structp png_ptr, png_infop info_ptr, png_timep mod_time) {
    png_debug1(1, "in %s storage function\n", "tIME");
    if (png_ptr == NULL || info_ptr == NULL || (png_ptr->flags & PNG_FLAG_WROTE_tIME)) return;
...
void png_set_tRNS(png_structp png_ptr, png_infop info_ptr, png_bytep trans, int num_trans, png_color_16p trans_values) {
    ...
    if (trans_values != NULL) {
        png_memcpy(&(info_ptr->trans_values), trans_values, sizeof(png_color_16));
