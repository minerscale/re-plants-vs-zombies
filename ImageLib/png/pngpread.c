...
void png_push_read_chunk(png_structp png_ptr, png_infop info_ptr) {
    ...
    if (!png_memcmp(png_ptr->chunk_name, png_IHDR, 4)) {
        ...
        if (png_ptr->mode & PNG_HAVE_IDAT) {
            if (png_ptr->push_length == 0) return;

            if (png_ptr->mode & PNG_AFTER_IDAT) png_error(png_ptr, "Too many IDAT's found");
        }
...
void png_push_crc_finish(png_structp png_ptr) {
    if (png_ptr->skip_length && png_ptr->save_buffer_size) {
        ...
        if (png_ptr->skip_length < (png_uint_32)png_ptr->save_buffer_size) save_size = (png_size_t)png_ptr->skip_length;
...
void png_push_save_buffer(png_structp png_ptr) {
    if (png_ptr->save_buffer_size) {
        if (png_ptr->save_buffer_ptr != png_ptr->save_buffer) {
            ...
            for (i = 0, sp = png_ptr->save_buffer_ptr, dp = png_ptr->save_buffer; i < istop; i++, sp++, dp++) {
                *dp = *sp;
            }
        }
    }
    if (png_ptr->save_buffer_size + png_ptr->current_buffer_size > png_ptr->save_buffer_max) {
        ...
        png_ptr->save_buffer = (png_bytep)png_malloc(png_ptr, (png_uint_32)new_max);
    ...
    if (png_ptr->current_buffer_size) {
        png_memcpy(
            png_ptr->save_buffer + png_ptr->save_buffer_size, png_ptr->current_buffer_ptr, png_ptr->current_buffer_size
        );
...
void png_push_read_IDAT(png_structp png_ptr) {
    ...
    if (png_ptr->idat_size && png_ptr->save_buffer_size) {
        ...
        if (png_ptr->idat_size < (png_uint_32)png_ptr->save_buffer_size) {
            ...
            /* check for overflow */
            if ((png_uint_32)save_size != png_ptr->idat_size) png_error(png_ptr, "save_size overflowed in pngpread");
        } else save_size = png_ptr->save_buffer_size;
...
void png_process_IDAT_data(png_structp png_ptr, png_bytep buffer, png_size_t buffer_length) {
    ...
    if ((png_ptr->flags & PNG_FLAG_ZLIB_FINISHED) && buffer_length) png_error(png_ptr, "Extra compression data");
    ...
    for (;;) {
        ...
        if (ret == Z_STREAM_END) {
            if (png_ptr->zstream.avail_in) png_error(png_ptr, "Extra compressed data");
            ...
            break;
        } else if (ret == Z_BUF_ERROR) break;
        else if (ret != Z_OK) png_error(png_ptr, "Decompression Error");
        if (!(png_ptr->zstream.avail_out)) {
            png_push_process_row(png_ptr);
            png_ptr->zstream.avail_out = (uInt)png_ptr->irowbytes;
            png_ptr->zstream.next_out = png_ptr->row_buf;
...
void png_read_push_finish_row(png_structp png_ptr) {
    ...
    if (png_ptr->interlaced) {
        ...
        do {
            ...
            png_ptr->irowbytes = ((png_ptr->iwidth * png_ptr->pixel_depth + 7) >> 3) + 1;

            if (png_ptr->transformations & PNG_INTERLACE) break;

            png_ptr->num_rows = (png_ptr->height + png_pass_yinc[png_ptr->pass] - 1 - png_pass_ystart[png_ptr->pass]) /
...
void png_push_handle_tEXt(png_structp png_ptr, png_infop info_ptr, png_uint_32 length) {
    ...
    png_ptr->current_text = (png_charp)png_malloc(png_ptr, (png_uint_32)(length + 1));
...
void png_push_read_zTXt(png_structp png_ptr, png_infop info_ptr) {
    ...
    if (!(png_ptr->current_text_left)) {
        ...
        png_ptr->zstream.avail_in = (uInt)(png_ptr->current_text_size - (text - key));
                ...
                if (text == NULL) {
                    ...
                    *(text + text_size) = '\0';
                    ...
                    text = (png_charp)png_malloc(
                        png_ptr, text_size + (png_uint_32)(png_ptr->zbuf_size - png_ptr->zstream.avail_out + 1)
                    );
...
void png_push_have_info(png_structp png_ptr, png_infop info_ptr) {
    if (png_ptr->info_fn != NULL) (*(png_ptr->info_fn))(png_ptr, info_ptr);
}

void png_push_have_end(png_structp png_ptr, png_infop info_ptr) {
    if (png_ptr->end_fn != NULL) (*(png_ptr->end_fn))(png_ptr, info_ptr);
}

void png_push_have_row(png_structp png_ptr, png_bytep row) {
    if (png_ptr->row_fn != NULL) (*(png_ptr->row_fn))(png_ptr, row, png_ptr->row_number, (int)png_ptr->pass);
}
...
png_voidp png_get_progressive_ptr(png_structp png_ptr) { return png_ptr->io_ptr; }
