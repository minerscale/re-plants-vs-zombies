...
use_merged_upsample(j_decompress_ptr cinfo) {
    ...
    if (cinfo->jpeg_color_space != JCS_YCbCr || cinfo->num_components != 3 || cinfo->out_color_space != JCS_RGB ||
...
/* Do computations that are needed before master selection phase */
{
    ...
    if (cinfo->global_state != DSTATE_READY) ERREXIT1(cinfo, JERR_BAD_STATE, cinfo->global_state);
    ...
    for (ci = 0, compptr = cinfo->comp_info; ci < cinfo->num_components; ci++, compptr++) {
        ...
        compptr->downsampled_width = (JDIMENSION)jdiv_round_up(
            (long)cinfo->image_width * (long)(compptr->h_samp_factor * compptr->DCT_scaled_size),
            (long)(cinfo->max_h_samp_factor * DCTSIZE)
        );
        compptr->downsampled_height = (JDIMENSION)jdiv_round_up(
            (long)cinfo->image_height * (long)(compptr->v_samp_factor * compptr->DCT_scaled_size),
    ...
    default: /* else must be same colorspace as in file */ cinfo->out_color_components = cinfo->num_components; break;
    }
...
prepare_for_output_pass(j_decompress_ptr cinfo) {
    ...
    if (master->pub.is_dummy_pass) {
        ...
        if (!cinfo->raw_data_out) {
            ...
            if (cinfo->quantize_colors) (*cinfo->cquantize->start_pass)(cinfo, master->pub.is_dummy_pass);
...
jpeg_new_colormap(j_decompress_ptr cinfo) {
    ...
    /* Prevent application from calling me at wrong times */
    if (cinfo->global_state != DSTATE_BUFIMAGE) ERREXIT1(cinfo, JERR_BAD_STATE, cinfo->global_state);
    ...
    } else ERREXIT(cinfo, JERR_MODE_CHANGE);
}
