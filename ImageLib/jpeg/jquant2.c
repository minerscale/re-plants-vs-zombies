...
prescan_quantize(j_decompress_ptr cinfo, JSAMPARRAY input_buf, JSAMPARRAY output_buf, int num_rows) {
    ...
    for (row = 0; row < num_rows; row++) {
        ...
        for (col = width; col > 0; col--) {
            ...
            histp = &histogram[GETJSAMPLE(ptr[0]) >> C0_SHIFT][GETJSAMPLE(ptr[1]) >> C1_SHIFT]
...
LOCAL(void)
find_best_colors(
    j_decompress_ptr cinfo, int minc0, int minc1, int minc2, int numcolors, JSAMPLE colorlist[], JSAMPLE bestcolor[]
)
...
/* This version performs Floyd-Steinberg dithering */
{
    ...
    for (row = 0; row < num_rows; row++) {
        ...
        for (col = width; col > 0; col--) {
            ...
            if (*cachep == 0) fill_inverse_cmap(cinfo, cur0 >> C0_SHIFT, cur1 >> C1_SHIFT, cur2 >> C2_SHIFT);
            /* Now emit the colormap index for this cell */
            {
...
start_pass_2_quant(j_decompress_ptr cinfo, boolean is_pre_scan) {
    ...
    if (is_pre_scan) {
        ...
        /* Set up method pointers */
        if (cinfo->dither_mode == JDITHER_FS) cquantize->pub.color_quantize = pass2_fs_dither;
        else cquantize->pub.color_quantize = pass2_no_dither;
        cquantize->pub.finish_pass = finish_pass2;

        /* Make sure color count is acceptable */
        i = cinfo->actual_number_of_colors;
        if (i < 1) ERREXIT1(cinfo, JERR_QUANT_FEW_COLORS, 1);
        if (i > MAXNUMCOLORS) ERREXIT1(cinfo, JERR_QUANT_MANY_COLORS, MAXNUMCOLORS);
...
jinit_2pass_quantizer(j_decompress_ptr cinfo) {
    ...
    cquantize->histogram =
        (hist3d)(*cinfo->mem->alloc_small)((j_common_ptr)cinfo, JPOOL_IMAGE, HIST_C0_ELEMS * SIZEOF(hist2d));
    for (i = 0; i < HIST_C0_ELEMS; i++) {
    ...
    if (cinfo->enable_2pass_quant) {
        ...
        if (desired > MAXNUMCOLORS) ERREXIT1(cinfo, JERR_QUANT_MANY_COLORS, MAXNUMCOLORS);
        cquantize->sv_colormap =
            (*cinfo->mem->alloc_sarray)((j_common_ptr)cinfo, JPOOL_IMAGE, (JDIMENSION)desired, (JDIMENSION)3);
