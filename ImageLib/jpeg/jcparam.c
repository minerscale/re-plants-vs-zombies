...
jpeg_add_quant_table(
    j_compress_ptr cinfo, int which_tbl, const unsigned int *basic_table, int scale_factor, boolean force_baseline
)
 ...
 */
{
    ...
    if (which_tbl < 0 || which_tbl >= NUM_QUANT_TBLS) ERREXIT1(cinfo, JERR_DQT_INDEX, which_tbl);
    ...
    for (i = 0; i < DCTSIZE2; i++) {
        ...
        if (force_baseline && temp > 255L) temp = 255L; /* limit to baseline range if requested */
 ...
 */
{
    ...
    static const unsigned int std_chrominance_quant_tbl[DCTSIZE2] = {17, 18, 24, 47, 99, 99, 99, 99, 18, 21, 26, 66, 99,
...
/* Define a Huffman table */
{
    ...
    for (len = 1; len <= 16; len++)
        nsymbols += bits[len];
    if (nsymbols < 1 || nsymbols > 256) ERREXIT(cinfo, JERR_BAD_HUFF_TABLE);
...
jpeg_set_defaults(j_compress_ptr cinfo) {
    ...
    if (cinfo->comp_info == NULL)
        cinfo->comp_info = (jpeg_component_info *)(*cinfo->mem->alloc_small)(
            (j_common_ptr)cinfo, JPOOL_PERMANENT, MAX_COMPONENTS * SIZEOF(jpeg_component_info)
        );
     ...
     */
    if (cinfo->data_precision > 8) cinfo->optimize_coding = TRUE;
...
#define SET_COMP(index, id, hsamp, vsamp, quant, dctbl, actbl)                                                         \
    (compptr = &cinfo->comp_info[index], compptr->component_id = (id), compptr->h_samp_factor = (hsamp),               \
     compptr->v_samp_factor = (vsamp), compptr->quant_tbl_no = (quant), compptr->dc_tbl_no = (dctbl),                  \
    ...
    case JCS_UNKNOWN:
        ...
        break;
    default: ERREXIT(cinfo, JERR_BAD_J_COLORSPACE);
    }
}
...
jpeg_simple_progression(j_compress_ptr cinfo) {
    ...
    if (ncomps == 3 && cinfo->jpeg_color_space == JCS_YCbCr) {
        ...
        if (ncomps > MAX_COMPS_IN_SCAN) nscans = 6 * ncomps; /* 2 DC + 4 AC scans per component */
        else nscans = 2 + 4 * ncomps;                        /* 2 DC scans; 4 AC scans per component */
    }
    ...
    if (cinfo->script_space == NULL || cinfo->script_space_size < nscans) {
        ...
        cinfo->script_space = (jpeg_scan_info *)(*cinfo->mem->alloc_small)(
            (j_common_ptr)cinfo, JPOOL_PERMANENT, cinfo->script_space_size * SIZEOF(jpeg_scan_info)
        );
    }
