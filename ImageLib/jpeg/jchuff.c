...
start_pass_huff(j_compress_ptr cinfo, boolean gather_statistics) {
    ...
    for (ci = 0; ci < cinfo->comps_in_scan; ci++) {
        ...
        if (gather_statistics) {
            ...
            if (dctbl < 0 || dctbl >= NUM_HUFF_TBLS) ERREXIT1(cinfo, JERR_NO_HUFF_TABLE, dctbl);
            if (actbl < 0 || actbl >= NUM_HUFF_TBLS) ERREXIT1(cinfo, JERR_NO_HUFF_TABLE, actbl);
            /* Allocate and zero the statistics tables */
            /* Note that jpeg_gen_optimal_table expects 257 entries in each table! */
            if (entropy->dc_count_ptrs[dctbl] == NULL)
                entropy->dc_count_ptrs[dctbl] =
                    (long *)(*cinfo->mem->alloc_small)((j_common_ptr)cinfo, JPOOL_IMAGE, 257 * SIZEOF(long));
...
jpeg_make_c_derived_tbl(j_compress_ptr cinfo, boolean isDC, int tblno, c_derived_tbl **pdtbl) {
    ...
    if (tblno < 0 || tblno >= NUM_HUFF_TBLS) ERREXIT1(cinfo, JERR_NO_HUFF_TABLE, tblno);
    htbl = isDC ? cinfo->dc_huff_tbl_ptrs[tblno] : cinfo->ac_huff_tbl_ptrs[tblno];
    if (htbl == NULL) ERREXIT1(cinfo, JERR_NO_HUFF_TABLE, tblno);
    ...
    for (p = 0; p < lastp; p++) {
        i = htbl->huffval[p];
        if (i < 0 || i > maxsymbol || dtbl->ehufsi[i]) ERREXIT(cinfo, JERR_BAD_HUFF_TABLE);
...
encode_one_block(working_state *state, JCOEFPTR block, int last_dc_val, c_derived_tbl *dctbl, c_derived_tbl *actbl) {
    ...
    if (!emit_bits(state, dctbl->ehufco[nbits], dctbl->ehufsi[nbits])) return FALSE;
    ...
    for (k = 1; k < DCTSIZE2; k++) {
        if ((temp = block[jpeg_natural_order[k]]) == 0) {
            ...
            while (r > 15) {
                if (!emit_bits(state, actbl->ehufco[0xF0], actbl->ehufsi[0xF0])) return FALSE;
                r -= 16;
            }
            ...
            i = (r << 4) + nbits;
            if (!emit_bits(state, actbl->ehufco[i], actbl->ehufsi[i])) return FALSE;
...
encode_mcu_huff(j_compress_ptr cinfo, JBLOCKROW *MCU_data) {
    ...
    for (blkn = 0; blkn < cinfo->blocks_in_MCU; blkn++) {
        ...
        compptr = cinfo->cur_comp_info[ci];
        if (!encode_one_block(
                &state, MCU_data[blkn][0], state.cur.last_dc_val[ci], entropy->dc_derived_tbls[compptr->dc_tbl_no],
...
encode_mcu_gather(j_compress_ptr cinfo, JBLOCKROW *MCU_data) {
    ...
    for (blkn = 0; blkn < cinfo->blocks_in_MCU; blkn++) {
        ...
        compptr = cinfo->cur_comp_info[ci];
        htest_one_block(
            cinfo, MCU_data[blkn][0], entropy->saved.last_dc_val[ci], entropy->dc_count_ptrs[compptr->dc_tbl_no],
...
jinit_huff_encoder(j_compress_ptr cinfo) {
    ...
    entropy =
        (huff_entropy_ptr)(*cinfo->mem->alloc_small)((j_common_ptr)cinfo, JPOOL_IMAGE, SIZEOF(huff_entropy_encoder));
