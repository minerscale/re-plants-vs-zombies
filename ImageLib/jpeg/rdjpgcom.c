...
static void process_SOFn(int marker) {
    ...
    if (length != (unsigned int)(8 + num_components * 3)) ERREXIT("Bogus SOF marker length");
...
static int scan_JPEG_header(int verbose) {
    ...
    for (;;) {
        ...
        case M_SOF15: /* Differential lossless, arithmetic */
            if (verbose) process_SOFn(marker);
            else skip_variable();
            break;

        case M_SOS: /* stop before hitting compressed data */ return marker;

        case M_EOI: /* in case it's a tables-only JPEG stream */ return marker;

        case M_COM: process_COM(); break;
...
int main(int argc, char **argv) {
    ...
    progname = argv[0];
    if (progname == NULL || progname[0] == 0) progname = "rdjpgcom"; /* in case C library doesn't provide it */
