...
start_pass_phuff(j_compress_ptr cinfo, boolean gather_statistics) {
    ...
    if (cinfo->Ah == 0) {
        if (is_DC_band) entropy->pub.encode_mcu = encode_mcu_DC_first;
        else entropy->pub.encode_mcu = encode_mcu_AC_first;
    } else {
        if (is_DC_band) entropy->pub.encode_mcu = encode_mcu_DC_refine;
        else {
            entropy->pub.encode_mcu = encode_mcu_AC_refine;
            /* AC refinement needs a correction bit buffer */
            if (entropy->bit_buffer == NULL)
                entropy->bit_buffer =
                    (char *)(*cinfo->mem->alloc_small)((j_common_ptr)cinfo, JPOOL_IMAGE, MAX_CORR_BITS * SIZEOF(char));
        }
    }
    if (gather_statistics) entropy->pub.finish_pass = finish_pass_gather_phuff;
    else entropy->pub.finish_pass = finish_pass_phuff;
    ...
    for (ci = 0; ci < cinfo->comps_in_scan; ci++) {
        ...
        if (gather_statistics) {
            ...
            if (tbl < 0 || tbl >= NUM_HUFF_TBLS) ERREXIT1(cinfo, JERR_NO_HUFF_TABLE, tbl);
...
/* Empty the output buffer; we do not support suspension in this module. */
{
    ...
    if (!(*dest->empty_output_buffer)(entropy->cinfo)) ERREXIT(entropy->cinfo, JERR_CANT_SUSPEND);
...
finish_pass_gather_phuff(j_compress_ptr cinfo) {
    ...
    for (ci = 0; ci < cinfo->comps_in_scan; ci++) {
        ...
        if (!did[tbl]) {
            if (is_DC_band) htblptr = &cinfo->dc_huff_tbl_ptrs[tbl];
            else htblptr = &cinfo->ac_huff_tbl_ptrs[tbl];
...
jinit_phuff_encoder(j_compress_ptr cinfo) {
    ...
    entropy =
        (phuff_entropy_ptr)(*cinfo->mem->alloc_small)((j_common_ptr)cinfo, JPOOL_IMAGE, SIZEOF(phuff_entropy_encoder));
