...
#define ASSIGN_STATE(dest, src)                                                                                        \
    ((dest).EOBRUN = (src).EOBRUN, (dest).last_dc_val[0] = (src).last_dc_val[0],                                       \
     (dest).last_dc_val[1] = (src).last_dc_val[1], (dest).last_dc_val[2] = (src).last_dc_val[2],                       \
...
start_pass_phuff_decoder(j_decompress_ptr cinfo) {
     ...
     */
    if (bad) ERREXIT4(cinfo, JERR_BAD_PROGRESSION, cinfo->Ss, cinfo->Se, cinfo->Ah, cinfo->Al);
    ...
    for (ci = 0; ci < cinfo->comps_in_scan; ci++) {
        ...
        for (coefi = cinfo->Ss; coefi <= cinfo->Se; coefi++) {
            ...
            if (cinfo->Ah != expected) WARNMS2(cinfo, JWRN_BOGUS_PROGRESSION, cindex, coefi);
    ...
    if (cinfo->Ah == 0) {
        if (is_DC_band) entropy->pub.decode_mcu = decode_mcu_DC_first;
        else entropy->pub.decode_mcu = decode_mcu_AC_first;
    } else {
        if (is_DC_band) entropy->pub.decode_mcu = decode_mcu_DC_refine;
        else entropy->pub.decode_mcu = decode_mcu_AC_refine;
    }
        ...
        if (EOBRUN == 0) {
            for (; k <= Se; k++) {
                ...
                if (s) {
                    ...
                    if (GET_BITS(1)) s = p1; /* newly nonzero coef is positive */
...
jinit_phuff_decoder(j_decompress_ptr cinfo) {
    ...
    entropy =
        (phuff_entropy_ptr)(*cinfo->mem->alloc_small)((j_common_ptr)cinfo, JPOOL_IMAGE, SIZEOF(phuff_entropy_decoder));
