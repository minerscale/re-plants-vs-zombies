...
/* cinfo->comps_in_scan and cinfo->cur_comp_info[] were set from SOS marker */
{
    ...
    if (cinfo->comps_in_scan == 1) {
        ...
        for (ci = 0; ci < cinfo->comps_in_scan; ci++) {
            ...
            if (cinfo->blocks_in_MCU + mcublks > D_MAX_BLOCKS_IN_MCU) ERREXIT(cinfo, JERR_BAD_MCU_SIZE);
            while (mcublks-- > 0) {
...
METHODDEF(void)
finish_input_pass(j_decompress_ptr cinfo) { cinfo->inputctl->consume_input = consume_markers; }
...
consume_markers(j_decompress_ptr cinfo) {
    ...
    case JPEG_REACHED_EOI: /* Found EOI */
        ...
        if (inputctl->inheaders) { /* Tables-only datastream, apparently */
            if (cinfo->marker->saw_SOF) ERREXIT(cinfo, JERR_SOF_NO_SOS);
        } else {
...
jinit_input_controller(j_decompress_ptr cinfo) {
    ...
    inputctl =
        (my_inputctl_ptr)(*cinfo->mem->alloc_small)((j_common_ptr)cinfo, JPOOL_PERMANENT, SIZEOF(my_input_controller));
