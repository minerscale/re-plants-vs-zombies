...
sep_upsample(
    ...
    if (upsample->next_row_out >= cinfo->max_v_samp_factor) {
        for (ci = 0, compptr = cinfo->comp_info; ci < cinfo->num_components; ci++, compptr++) {
             ...
             */
            (*upsample->methods[ci])(
                cinfo, compptr, input_buf[ci] + (*in_row_group_ctr * upsample->rowgroup_height[ci]),
    ...
    (*cinfo->cconvert->color_convert)(
        cinfo, upsample->color_buf, (JDIMENSION)upsample->next_row_out, output_buf + *out_row_ctr, (int)num_rows
    );
...
h2v2_fancy_upsample(
    ...
    while (outrow < cinfo->max_v_samp_factor) {
        for (v = 0; v < 2; v++) {
            ...
            else /* next nearest is row below */ inptr1 = input_data[inrow + 1];
            outptr = output_data[outrow++];
...
jinit_upsampler(j_decompress_ptr cinfo) {
    ...
    for (ci = 0, compptr = cinfo->comp_info; ci < cinfo->num_components; ci++, compptr++) {
        ...
        if (!compptr->component_needed) {
            ...
            } else upsample->methods[ci] = h2v2_upsample;
