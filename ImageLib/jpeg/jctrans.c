...
jpeg_copy_critical_parameters(j_decompress_ptr srcinfo, j_compress_ptr dstinfo) {
    ...
    if (dstinfo->global_state != CSTATE_START) ERREXIT1(dstinfo, JERR_BAD_STATE, dstinfo->global_state);
    ...
    for (tblno = 0; tblno < NUM_QUANT_TBLS; tblno++) {
        if (srcinfo->quant_tbl_ptrs[tblno] != NULL) {
            ...
            if (*qtblptr == NULL) *qtblptr = jpeg_alloc_quant_table((j_common_ptr)dstinfo);
    ...
    for (ci = 0, incomp = srcinfo->comp_info, outcomp = dstinfo->comp_info; ci < dstinfo->num_components;
         ci++, incomp++, outcomp++) {
...
transencode_coef_controller(j_compress_ptr cinfo, jvirt_barray_ptr *coef_arrays) {
    ...
    buffer =
        (JBLOCKROW)(*cinfo->mem->alloc_large)((j_common_ptr)cinfo, JPOOL_IMAGE, C_MAX_BLOCKS_IN_MCU * SIZEOF(JBLOCK));
