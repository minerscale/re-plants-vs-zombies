...
unsigned long *Image::GetBits() { return mBits; }
...
static void png_pak_read_data(png_structp png_ptr, png_bytep data, png_size_t length) {
     ...
     */
    check = (png_size_t)p_fread(data, (png_size_t)1, length, (PFILE *)png_ptr->io_ptr);
...
#define LSBFirstOrder(x, y) (((y) << 8) | (x))
    ...
    unsigned char background, // 背景色在全局颜色列表中的索引（背景色：图像中没有被指定颜色的像素会被背景色填充）
        c,
        flag,             // 图像标志的压缩字节
        *global_colormap, // 全局颜色列表
        header[1664], magick[12];

    unsigned int delay, dispose,
    ...
    */
    ...
    if (((strncmp((char *)magick, "GIF87", 5) != 0) && (strncmp((char *)magick, "GIF89", 5) != 0))) return NULL;
    ...
    if (BitSet(flag, 0x80)) // 如果存在全局颜色列表
    {
        ...
        */
        global_colors = 1
                        << ((flag & 0x07) + 1
                           ); // 压缩字节的最低 3 位表示全局颜色列表的大小，设其二进制数值为 N，则列表大小 = 2 ^ (N + 1)
        global_colormap = new unsigned char[3 * global_colors]; // 每个颜色占 3 个字节，按 RGB 排列
        if (global_colormap == (unsigned char *)NULL) return NULL;
    ...
    for (;;) {
        if (p_fread(&c, sizeof(char), 1, fp) == 0) break; // 如果读取错误或读取到文件尾则退出，返回空指针
            ...
            */
            ...
            case 0xf9: {
                ...
                */
                ...
                if ((header[0] & 0x01) == 1) opacity = header[3];
                break;
            }
            case 0xfe: {
                char *comments;
            ...
            case 0xff: {
                ...
                */
                loop = false;
                if (ReadBlobBlock(fp, (char *)header) > 0) loop = !strncmp((char *)header, "NETSCAPE2.0", 11);
                while (ReadBlobBlock(fp, (char *)header) > 0)
                    if (loop) iterations = (header[2] << 8) | header[1];
                break;
            }
            default: {
                while (ReadBlobBlock(fp, (char *)header) > 0)
        ...
        colors = !BitSet(flag, 0x80)
                     ? global_colors
                     : 1 << ((flag & 0x07) + 1);               // 判断使用全局颜色列表或使用局部颜色列表，并取得列表大小
...
#define NullCode (-1)
        ...
        int available, bits, code, clear, code_mask, code_size, count, end_of_information, in_code, offset, old_code,
            pass, y;
        ...
        unsigned char data_size, first, *packet, *pixel_stack, *suffix, *top_stack;
        ...
        for (y = 0; y < (int)height; y++) {
            ...
            for (x = 0; x < (int)width;) {
                ...
                *q = colortable[index];
                ...
                if (index == opacity) *q = 0;

                x++;
                q++;
            }

            if (!interlaced) offset++;
            else {
                switch (pass) {
                case 0:
                default: {
...
bool ImageLib::WriteJPEGImage(const std::string &theFileName, Image *theImage) {
    ...
    if ((fp = fopen(theFileName.c_str(), "wb")) == NULL) return false;
...
bool ImageLib::WritePNGImage(const std::string &theFileName, Image *theImage) {
    ...
    png_set_IHDR(
        png_ptr, info_ptr, theImage->mWidth, theImage->mHeight, 8, PNG_COLOR_TYPE_RGB_ALPHA, PNG_INTERLACE_NONE,
...
void jpeg_pak_src(j_decompress_ptr cinfo, PFILE *infile) {
    ...
    if (cinfo->src == NULL) { /* first time for this JPEG object? */
        cinfo->src = (struct jpeg_source_mgr *)(*cinfo->mem->alloc_small)(
            (j_common_ptr)cinfo, JPOOL_PERMANENT, sizeof(pak_source_mgr)
        );
...
void ImageLib::InitJPEG2000() { gJ2KCodec = ::LoadLibrary(_T("j2k-codec.dll")); }
...
void ImageLib::SetJ2KCodecKey(const std::string &theKey) { gJ2KCodecKey = theKey; }

int __stdcall Pak_seek(void *data_source, int offset) { return p_fseek((PFILE *)data_source, offset, SEEK_SET); }
...
Image *GetJPEG2000Image(const std::string &theFileName) {
    if (gJ2KCodec != NULL) {
        ...
        if (loadFuncs) {
            ...
            *((void **)&fJ2K_GetResolutionDimensions) =
                (void *)::GetProcAddress(gJ2KCodec, "_J2K_GetResolutionDimensions@16");
            ...
            *((void **)&fJ2K_getErrorStr) = (void *)::GetProcAddress(gJ2KCodec, "_J2K_getErrorStr@4");
            ...
            // j2k guys didn't use declare the export names. yay! now we have to update these mangled names any time the
            // DLL changes.

            if (!(fJ2K_getVersion != NULL && fJ2K_Unlock != NULL && fJ2K_OpenCustom != NULL && fJ2K_OpenFile != NULL &&
                  fJ2K_Close != NULL && fJ2K_GetInfo != NULL && fJ2K_GetResolutionDimensions != NULL &&
                  fJ2K_Decode != NULL && fJ2K_getLastError != NULL && fJ2K_getErrorStr != NULL)) {
                CloseJPEG2000();
                return NULL;
            }
...
Image *ImageLib::GetImage(const std::string &theFilename, bool lookForAlphaImage) {
    ...
    if (lookForAlphaImage) {
        ...
        // Check ImageName_
        if (anAlphaImage == NULL) anAlphaImage = GetImage(theFilename + "_", false);
    }
