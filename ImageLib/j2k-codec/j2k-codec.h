...
J2K_CODEC_API(void *)
J2K_EasyDecode(char *filename, unsigned char **buffer, int *width, int *height, int *components, char *options);
...
public:
    int width, height, components, hTiles, vTiles, fileType;
    const char *errStr;
    ...
    int open(char *filename) {
        if (image) close();
        image = J2K_Open(filename);
        return _open();
    }
    int open(unsigned char *src_buffer, int src_size) {
        if (image) close();
        image = J2K_Open(src_buffer, src_size);
        return _open();
    }
    int open(int resource_no) {
        if (image) close();

        HRSRC hRes = FindResource(NULL, MAKEINTRESOURCE(resource_no), RT_RCDATA);
        if (!hRes) return error(J2KERR_CANT_OPEN_FILE);
        HGLOBAL res = LoadResource(NULL, hRes);
        if (!res) return error(J2KERR_CANT_OPEN_FILE);
        int res_size = SizeofResource(NULL, hRes);
        if (!res_size) return error(J2KERR_CANT_OPEN_FILE);

        unsigned char *res_data = (unsigned char *)LockResource(res);

        image = J2K_Open(res_data, res_size);
        return _open();
    }

    int open(void *data_source, J2K_Callbacks *cbs) {
        if (image) close();
        image = J2K_Open(data_source, cbs);
        return _open();
    }
    ...
    int getMetaData(int *no, int *type, unsigned char **data, int *size) {
        ...
        if (j2k_err != J2KERR_SUCCESS) return error(j2k_err);
        else return J2KERR_SUCCESS;
    }
    ...
    int decode(char *options = 0) {
        ...
        if (j2k_err == J2KERR_SUCCESS || j2k_err == J2KERR_NO_SUCH_RESOLUTION) {
            j2k_err = J2K_Decode(image, &buffer, &size, options, &pitch);
            if (j2k_err == J2KERR_SUCCESS) return J2KERR_SUCCESS;
        }
    ...
    int easyDecode(char *filename, char *options = 0) {
        int j2k_err = open(filename);
        if (j2k_err == J2KERR_SUCCESS) return decode(options);
        else return error(j2k_err);
    }

    int easyDecode(unsigned char *src_buffer, int src_size, char *options = 0) {
        int j2k_err = open(src_buffer, src_size);
        if (j2k_err == J2KERR_SUCCESS) return decode(options);
        else return error(j2k_err);
    }

    int easyDecode(int resource_no, char *options = 0) {
        int j2k_err = open(resource_no);
        if (j2k_err == J2KERR_SUCCESS) return decode(options);
        else return error(j2k_err);
    }

    void close() {
        if (image) J2K_Close(image);
        reset();
    }
...
class J2K_Frames {
protected:
    int **frame, currFrame, TotalFrames;
    bool isAlpha;
    ...
    int _open(void *image, char *options) {
        ...
        J2K_Info info;
        char new_options[128]; // char *new_options = (char*)alloca(strlen(options)+10);
        ...
        if (!frame) return error(J2KERR_NOT_ENOUGH_MEMORY);

        for (int i = 0; i < TotalFrames; i++) {
            frame[i] = NULL;

            J2K_SelectTiles(image, 0, -1, 0);
            J2K_SelectTiles(image, i, i, 1); // Unselect all tiles and select current tile
            ...
            if (J2K_GetResolutionDimensionsByStr(image, options, &tileWidth, &tileHeight) == J2KERR_SUCCESS) {
                ...
                if (frame[i]) {
                    frame[i][0] = tileWidth;
                    frame[i][1] = tileHeight;
                    frame[i][2] = isAlpha ? 1 : 0;

                    unsigned char *buffer = (unsigned char *)(frame[i] + 3);
                    int size = tileWidth * tileHeight * 4;

                    strcpy(new_options, options);
                    strcat(new_options, ",bpp=4"); // Override user's bpp, because frames have always bpp = 4

                    j2k_err = J2K_Decode(image, &buffer, &size, new_options);
                    if (j2k_err != J2KERR_SUCCESS) {
                        delete frame[i];
                        frame[i] = NULL;
                    }
    ...
    void dec() {
        currFrame--;
        if (currFrame < 0) currFrame = TotalFrames - 1;
    }
...
public:
    ...
    int open(unsigned char *src_buffer, int src_size, char *options) {
        return _open(J2K_Open(src_buffer, src_size), options);
    }
    int open(void *data_source, J2K_Callbacks *cbs, char *options) {
        return _open(J2K_Open(data_source, cbs), options);
    }
    int open(int resource_no, char *options) {
        ...
        unsigned char *res_data = (unsigned char *)LockResource(res);
        return _open(J2K_Open(res_data, res_size), options);
    }
    ...
    __inline unsigned char *getFrame(int no) {
        if (no < 0) no = 0;
        if (no >= TotalFrames) no = TotalFrames - 1;

        if (frame) return (unsigned char *)frame[no];
        else return NULL;
    }
